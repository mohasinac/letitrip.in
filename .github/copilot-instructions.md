...
**NEVER reimplement logic that already exists. Search `@/utils` and `@/helpers` before writing any data, string, or formatting function.**

> **Extend, Don't Duplicate**: if a function exists but lacks a specific case (e.g. `formatCurrency` needs a different locale, `groupBy` needs multi-key support), **add the new parameter or option to the existing function** — never create a parallel copy. Every improvement to a shared util propagates to every call site automatically.
...
### String Utilities (from `@/utils`)

| Need | Use | NOT |
|------|-----|-----|
| Capitalize first letter | `capitalize(str)` | `str.charAt(0).toUpperCase() + str.slice(1)` |
| Capitalize all words | `capitalizeWords(str)` | Manual `.split().map().join()` |
| URL-safe slug | `slugify(text)` | Custom regex replace |
| Truncate to length | `truncate(text, length)` | `` `${text.slice(0, n)}...` `` |
| Strip HTML tags | `stripHtml(html)` | Custom regex |
| Check empty/blank | `isBlank(str)` | `str.trim() === ''` |
| Convert to camelCase | `toCamelCase(str)` | Custom replace chain |
| Convert to snake_case | `toSnakeCase(str)` | Custom replace chain |
| Convert to kebab-case | `toKebabCase(str)` | Custom replace chain |
| Count words | `wordCount(str)` | `str.split(' ').length` |

### Number Utilities (from `@/utils`)

| Need | Use | NOT |
|------|-----|-----|
| Currency display | `formatCurrency(amount, currency?, locale?)` | `amount.toFixed(2)` |
| Thousand-separator number | `formatNumber(num, locale?)` | `num.toLocaleString()` |
| Percentage string | `formatPercentage(decimal, decimals?)` | `` `${(num*100).toFixed(n)}%` `` |
| File size (bytes → KB/MB/GB) | `formatFileSize(bytes)` | Manual division tree |
...
| Need | Use | NOT |
|------|-----|-----|
| String → boolean | `stringToBoolean(value)` | `value === 'true'` |
| Boolean → 'Yes'/'No' | `booleanToString(value, 'yesno')` | Inline ternary |
| Array → keyed object | `arrayToObject(arr, keyField)` | `arr.reduce(...)` manual |
| Object → array of values | `objectToArray(obj)` | `Object.values(obj)` inline |
| Object → `{key, value}[]` | `objectToKeyValueArray(obj)` | `Object.entries(obj).map(...)` |
| Parse cookie string | `parseCookies(cookieStr)` | Manual split/reduce |

### ID Generation (from `@/utils`)

**NEVER hand-craft document IDs.** All Firestore document IDs are SEO-friendly strings generated by the central ID utilities.

| Need | Use |
|------|-----|
| Product document ID | `generateProductId(name, category, condition, seller, count)` |
| Order document ID | `generateOrderId(productCount)` |
| User document ID | `generateUserId(firstName, lastName, email)` |
| Category document ID | `generateCategoryId(name, parentName)` |
| Review document ID | `generateReviewId(productName, userName)` |
| Carousel slide ID | `generateCarouselId(title)` |
| FAQ document ID | `generateFaqId(category, question)` |
| Homepage section ID | `generateHomepageSectionId(type)` |

---

## RULE 6: Use Existing Hooks

**Check `@/hooks` before creating new hooks. If an existing hook nearly covers your use case, add an option or parameter to it — never create a near-duplicate.**

> **Extend, Don't Duplicate**: need products filtered by seller? Add `sellerId?` to `useProducts`. Do NOT create `useSellerProducts` as a separate file copying the same logic. This keeps query keys consistent, avoids stale-cache bugs, and means every fix applies to all callers.
...
| Countdown timer | `useCountdown(targetDate)` — ticks every second; returns `{ days, hours, minutes, seconds, expired }` |
| Real-time notifications | `useNotifications()` — subscribes to Realtime DB notification path for current user |
| Wishlist add/remove | `useWishlistToggle(productId)` — returns `{ isWishlisted, toggle, loading }` |
| Combined gestures | `useGesture(options)` — unified swipe + tap + long-press detection |
| Live bid stream | `useRealtimeBids(auctionId)` — subscribes to Realtime DB bids for an auction |
| Place a bid | `usePlaceBid()` — mutation: validates, calls API, refreshes bid stream |
| Featured products | `useFeaturedProducts()` — curated featured product list |
| Featured auctions | `useFeaturedAuctions()` — curated featured auction list |
| Site settings | `useSiteSettings()` — cached site-wide configuration (name, socials, contact) |
| Promotions / banners | `usePromotions()` — active promotional banners |
| Public events | `usePublicEvents()` — upcoming public event listings |
| Seller storefront | `useSellerStorefront(sellerId)` — seller public page data |
| Blog posts | `useBlogPosts(options?)` — paginated blog post list |
| Contact form submit | `useContactSubmit()` — mutation: submits contact form |
| Hero carousel state | `useHeroCarousel()` — auto-advancing carousel with manual controls |
| Homepage sections | `useHomepageSections()` — ordered homepage section data |
| Homepage reviews | `useHomepageReviews()` — featured review set for homepage |
| Top categories | `useTopCategories()` — top-level category list with product counts |
| Coupon validation | `useCouponValidate()` — validates a coupon code against the API |
| Razorpay checkout | `useRazorpay()` — opens Razorpay modal and handles success/failure callbacks |
| Media upload flow | `useMediaUpload()` — staged file upload (select → preview → submit) |
| FAQ vote | `useFaqVote(faqId)` — up/down vote on an FAQ entry |
| Public profile | `usePublicProfile(userId)` — read-only public user profile |
| Profile stats | `useProfileStats()` — aggregated stats for current user’s profile page |
| Address form | `useAddressForm(options?)` — form state + submit for adding/editing an address |
| Address selector | `useAddressSelector()` — select/deselect from saved addresses |
| Add to cart | `useAddToCart()` — mutation: adds product to cart with optimistic update |
| Checkout flow | `useCheckout()` — multi-step checkout state machine |
| Category selector | `useCategorySelector()` — hierarchical category search + select |
...
**Check `@/components` before creating new UI elements. If a component exists but needs a new variant, size, or prop — add it to the component. Never create a parallel or wrapper component.**

> **Extend, Don't Fork**: adding `variant="ghost"` to `Button` is correct. Creating `GhostButton.tsx` is a violation. The same applies to every primitive — `Input`, `Badge`, `Card`, `Heading`, `Text`, and all others.
...
|-----------|---------|----------|
| `Heading` | Page titles, section headings, card headings | `level={1–6}`, `variant="primary\|secondary\|muted"` |
...
| Component | Covers | Key props |
|-----------|--------|----------|
| `Input` | Text, email, number, password, search | `type`, `error`, `leftIcon`, `rightIcon` |
| `Textarea` | Multi-line text | `rows`, `error` |
| `Select` | Dropdown | `options`, `error` |
| `Checkbox` | Boolean (checkbox style) | `checked`, `indeterminate` |
| `Radio` | Single-choice group | `options` |
| `Toggle` | Boolean (switch style) | `checked`, `size` |
| `Slider` | Range / numeric | `min`, `max`, `step` |
| `FormField` | Full field: label + input + error message | `type`, `label`, `error`, `helperText` |
| `Button` | All clickable actions | `variant="primary\|secondary\|outline\|ghost\|danger\|warning"`, `size="sm\|md\|lg"`, `isLoading` |

```tsx
// WRONG — bare HTML controls
...
See **RULE 31** for the extend-don’t-fork discipline across components, hooks, utils, and Typography.
...
- [ ] Does the existing component / hook / util almost cover my case? → **extend it in-place** (add variant/param/option) — never create a parallel file (Rule 31)
