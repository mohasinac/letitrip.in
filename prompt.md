...
### ✅ LAST COMPLETED — S7-PrizeDraws-2 (2026-05-13)

Public buyer surface + listing-type tabs + reveals-remaining badges landed across 10 commits (5 appkit + 5 root). `npm run check` clean (0 errors / 504 pre-existing warnings) at every step. **No deploys this session per user directive — appkit consumed via `file:./appkit`.** Q1-funcs-dryrun + Q1-ops Firebase Functions deploy + Vercel env wiring carried forward.

- **SB4-F public list** — `PrizeDrawsListingView` (server RSC) + `PrizeDrawsIndexListing` (client toolbar/grid/pagination + filter drawer w/ reveal-status filter) + `MarketplacePrizeDrawCard` (2x2 mini-collage thumb, status pill, entries-remaining, countdown, enter CTA). Public adapter strips `isWon` so buyers stay unspoiled. `src/app/[locale]/prize-draws/page.tsx` shimmed.
- **SB4-G public detail** — `PrizeDrawDetailPageView` (server RSC, reuses PreOrderDetailView grid-2 shell, full collage with `hideWonState`, entries/reveal-window panel, seller card) + `PrizeDrawEntryActions` (client buy panel: "Enter draw" → NonRefundableConsentModal → cart → /user/cart; "View RNG source" link). `[slug]/page.tsx` shimmed.
- **SB6-D allowance badges** — "Limit: N entries per customer" / "Limit: N per customer" pills on PrizeDrawDetailPageView + PreOrderDetailPageView when `maxPerUser` is set. Bundle detail already had this. Post-auth personalised "X/Y entries used" badge deferred.
- **SB7-C category/brand tabs** — `CategoryDetailTabs` + `BrandDetailTabs` get a "Prize Draws" tab rendering `PrizeDrawsIndexListing` scoped by categorySlug/brandName. Bundles tab + centralised `CATEGORY_PAGE_TABS` (SB10-A) deferred to S8.
- **SB7-D store tab** — `StoreDetailLayoutView` gains 4th parallel count + "Prize Draws" tab. `StorePrizeDrawsPageView` RSC + new public route `/stores/[storeSlug]/prize-draws`. `ROUTES.PUBLIC.STORE_PRIZE_DRAWS` added. `PrizeDrawsIndexListing` extended with optional `storeId` prop. Admin / store-dashboard / search-results tabs deferred.
- **SB8-F reveals badge** — `OrderItem` extended with optional `listingType` + `prizeRevealStatus` + `prizeRevealDeadline` + `revealedItemNumber`. `OrderCard` shows fuchsia "N reveals pending" + earliest deadline. User order detail per-item shows reveal-status pill (Pending / Open / Revealed (#N) / Closed). Checkout-side population of these fields deferred to follow-up.
...
| 8 | **S11** | Quality baseline — drive `audit-ssr-in-appkit` baseline 8→0 + TS9 hex sweep (154 hits) + RA-Tier audits applied inline | Tech-debt closeout |
| 9 | **S1-polish** | Optional slot-shell polish slots deferred from S1 (admin alerts/charts/recent-activity, user notifications filters, seller analytics charts/top-products). Feature work — new endpoints + hooks. | Pull when prioritised |
| 10 | **S2-browser-smoke** | User-driven browser smoke for S2 — sign in → cart → consent OTP → COD + Razorpay test card → coupon → auction-add-to-cart-block. Then `vercel --prod`. | One-off post-S2 validation |
