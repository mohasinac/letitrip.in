...
### ✅ LAST COMPLETED — S7-PrizeDraws-3-ops Firebase + Vercel deploys (2026-05-13)

After user OK'd Firebase + Vercel env updates, fired the deploy cohort:

- **Firestore indices** — `node appkit/scripts/firebase-merge.mjs` re-synced root from source, then `firebase deploy --only firestore:indexes` deployed cleanly. 3 prize-draw composites picked up.
- **Firebase Functions** — `cd functions && npm run build` → tsup bundle; `firebase deploy --only functions` created all 7 prep3 prize-draw handlers in `asia-south1` (`triggerEventRaffle` · `assignSpinPrize` · `prizeRevealOpen` · `prizeRevealClose` · `prizeRevealExpiry` · `prizeRevealReminder` · `bundleStockSync`). `listingProcessor` + every other function updated in place.
- **Env wiring** — `.env.local` gains `FIREBASE_FUNCTION_LISTING_URL="https://listingprocessor-nkzuprfdya-el.a.run.app"`. Vercel: `FIREBASE_FUNCTION_LISTING_URL` added to production + preview + development; `LETITRIP_INTERNAL_SECRET` already on prod, mirrored to preview + development.

**Effect:** `/api/products` will route through `listingProcessor` on next request once Vercel picks up the env. Scheduled prize-draw jobs are now firing on the Cloud Scheduler cadence; `bundleStockSync` runs daily 10:00 IST.

`/demo/seed` re-load not needed (no Firestore schema additions, only optional TS fields on `OrderDocumentItem` + `ProductItem`). SB-UNI-Z1/Z2/Z3 media upload reliability remains a separate cohort.

**Code work that landed earlier this session** (collapsed from the prior LAST entry — keep this as one combined block per the cadence rule): SB10-A centralised tab constants + `CategoryDetailTabs`/`BrandDetailTabs` refactor · SB7-C Bundles tab on category + brand via new `BundlesByCategoryListing` · SB7-D Bundles tab on public store (`StoreBundlesPageView` RSC + `/stores/[slug]/bundles/`) · SB7-D Prize Draws filter on `AdminProductsView` + `SellerProductsView` chip · SB7-D `SearchResourceType` widened + `LayoutShellClient` dropdown · SB6-D post-auth "You have used N/M" pill via `currentUserId` + `getServerSessionUser` thread · SB8-F `OrderDocumentItem` schema + COD/Razorpay actions + `orderDocumentToOrder` adapter all stamp `listingType`/`prizeRevealStatus`/`prizeRevealDeadline` on prize-draw lines. 12 commits total this session. `npm run check` clean every step.

**Held items (not blocking next session):** appkit npm publish (still on `file:./appkit`) · `/demo/seed` re-seed (no schema additions hit Firestore — only optional TS fields) · SB-UNI-Z1/Z2/Z3 media upload reliability (Rule #6 violation; separate cohort).

### 🔄 CURRENT — none (awaiting next session)

Next is **SB-UNI-Z1/Z2/Z3** (media upload reliability — Rule #6 fix) or **S-SBUNI** (Tier SB-UNI Phase 0 + Phase 1 — `assertNever` + capability registry + addresses/sublistings/brands/bundles fold into `categories`), depending on what user prioritises.

### ⏳ NEXT UP — single SB-UNI session + Z1–Z3 appended to S7-PrizeDraws-3

| # | Session | Scope | Why this slot |
|---|---------|-------|---------------|
| 1 | **SB-UNI-Z1/Z2/Z3** | Media upload reliability (Rule #6 fix). **Z1** — signed-URL flow replacing the `request.formData()` direct-upload route (currently 4.5 MB Vercel cap violation). **Z2** — MIME widening to allow `3gpp` / `3gpp2` / `x-matroska` alongside existing image/video allowlist. **Z3** — limits centralised in `_internal/shared/media/limits.ts` (currently scattered across 4 routes). Blocks every new form session. | Critical pre-req for S-SBUNI |
