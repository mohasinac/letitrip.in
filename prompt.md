...
### ✅ LAST COMPLETED — S7-PrizeDraws-3 code closeout (2026-05-13)

All S7-PrizeDraws-2 code carry-forwards landed this session per "fix all pending tasks before s8" directive. Deploys + appkit npm publish held (ops, not code). 11 commits (6 appkit + 5 root). `npm run check` clean every step (0 errors / 504 pre-existing warnings). appkit consumed via `file:./appkit`.

- **SB10-A** centralised — new `appkit/src/features/products/constants/listing-tabs.ts` with `CATEGORY_PAGE_TABS`, `STORE_PAGE_TABS`, `SELLER_LISTING_TABS`, `SEARCH_RESULT_TABS`. Each entry maps to either a `products.listingType` filter or a separate collection (`bundles`). `CategoryDetailTabs` + `BrandDetailTabs` refactored to consume `CATEGORY_PAGE_TABS`.
- **SB7-C bundles** — new `BundlesByCategoryListing` (client wrapper over parent-fetched bundles with sort + brand-match filtering). `CategoryDetailPageView` + `BrandDetailPageView` server-fetch bundles in the same `Promise.all` batch (`bundlesRepository.findByCategory` / `findAll`-then-filter-by-brand) and pipe through to the tabs.
- **SB7-D store** — `StoreDetailLayoutView` gets a 5th parallel `findByStore+published` bundle count + Bundles tab. New `StoreBundlesPageView` RSC. `ROUTES.PUBLIC.STORE_BUNDLES` helper + `/stores/[storeSlug]/bundles/page.tsx` shim.
- **SB7-D admin** — `AdminProductsView` `TYPE_OPTIONS` adds "Prize Draws"; filter builder maps it to `listingType==prize-draw`. Products tab now explicitly `listingType==standard`.
- **SB7-D seller dashboard** — `SellerProductsView` `ListingKind` union widens to include `"prize-draw"`; `TypeChips` renders Prize Draw pill; row→kind derivation + KIND_BADGE_VARIANT["prize-draw"]="primary"; edit-row link uses `ROUTES.STORE.PRIZE_DRAWS_EDIT(id)` for prize-draw rows.
- **SB7-D search** — `SearchResourceType` union widened with `prize-draws` + `bundles`. `/search` redirect ROUTE_MAP + `LayoutShellClient` `SEARCH_RESOURCE_TYPES` dropdown + matching `SEARCH_ROUTE_MAP` entries.
- **SB6-D post-auth** — `PrizeDrawDetailPageView` gains optional `currentUserId` prop. When set, server-fetches `orderRepository.countByUserAndProduct` (active-status filter) and renders a fuchsia "You have used N/M" pill next to the existing "Limit: N entries per customer" allowance pill. Page shim threads `getServerSessionUser()?.uid` through.
- **SB8-F population** — `OrderDocumentItem` schema extended with optional `listingType` + `prizeRevealStatus` + `prizeRevealDeadline` + `revealedItemNumber`. COD path (`createCheckoutOrderAction`) + Razorpay path (`verifyAndPlaceRazorpayOrderAction`) both stamp these onto each line in their `orderItems.map`. `orderDocumentToOrder` adapter forwards them so `/api/user/orders` responses carry the data → SB8-F badges from S7-PrizeDraws-2 light up immediately.

**Still held (ops, not code):** Q1-funcs-dryrun + Q1-ops Firebase Functions deploy · Vercel env vars `FIREBASE_FUNCTION_LISTING_URL` + `LETITRIP_INTERNAL_SECRET` · Firestore index deploy · `/demo/seed` re-load. Run on user OK.

### 🔄 CURRENT — none (awaiting next session)

Next is **S-SBUNI** (Tier SB-UNI Phase 0 + Phase 1 — `assertNever` + capability registry + addresses/sublistings/brands/bundles fold into `categories`).

### ⏳ NEXT UP — single SB-UNI session + Z1–Z3 appended to S7-PrizeDraws-3

| # | Session | Scope | Why this slot |
|---|---------|-------|---------------|
| 1 | **S7-PrizeDraws-3-ops** | Code work for S7-PrizeDraws-3 done 2026-05-13 (SB10-A · SB7-C bundles · SB7-D admin/seller/store/search · SB6-D post-auth · SB8-F population). **Remaining is ops only** — `firebase deploy --only firestore:indexes` for prize-draws + entries · `firebase deploy --only functions` for 7 prep3 handlers + `listingProcessor` · Vercel env (`FIREBASE_FUNCTION_LISTING_URL`, `LETITRIP_INTERNAL_SECRET`) · `/demo/seed` re-load against staging. **PLUS** SB-UNI-Z1 + Z2 + Z3 media upload reliability (signed-URL flow replacing `/api/media/upload`; MIME widening 3gpp/3gpp2/x-matroska; limits centralised in `_internal/shared/media/limits.ts` — Rule #6 violation, blocks new form work). | Ops + media upload — single deploy cohort |
