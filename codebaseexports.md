...
| HomepageSectionBoundary | Boundary | Crash containment for ONE section. Wrapped around every section at `renderSection()`'s single choke point — before this, a throw in any of the ~26 independently-fetched sections propagated to `[locale]/error.tsx` and replaced the whole page. Fallback renders `null` (a homepage missing a row beats a homepage that is an apology) and still reports via `trackError`. `sectionId`/`sectionType` are plain strings because a function prop is not serialisable across the Server→Client boundary. Root Cause #77 |
...
| CardStoreLine (2026-09-01) | Text | The `by <seller>` line on a listing card — one implementation for all six card variants (`ProductCard`, `ProductListRow`, `MarketplacePreorderCard`, `MarketplacePrizeDrawCard`, `MarketplaceAuctionCard`). Only `ProductCard` had it, inline. Takes `storeId` as well as `storeName` so a listing that HAS a seller says so even if the name was never denormalized — the old inline block used `safeDisplayName(storeName, "")` and rendered nothing, making "no data" indistinguishable from "no store". Deliberately not a link: the card is already wrapped in one |
...
| ui/forms | FormErrorSummary | Component | `appkit/src/ui/forms/FormErrorSummary.tsx` — reads `FormShellContext` directly (no required props), lists every current live error beside a form's Submit/Save/Publish button; step-tagged + clickable when `fieldToStepIndex` is populated. Supplements (not replaces) per-field inline errors. Every `<FormShell>`/`useFormShellState`/`<Form schema>` callsite must render one — enforced by `audit-form-error-summary`. **2026-08-26 (R1): now gated on `FormShellContextValue.submitAttempted`** — it was un-gated while ~20 views run `validate(draft)` in a mount effect, so every form opened listing its own requirements as errors |
| ui/forms | FormErrorList | Component | 2026-08-26 (W21) — `appkit/src/ui/forms/FormErrorList.tsx` — the `<ul>` of current errors with per-section jump links, extracted so the inline `<FormErrorSummary>` and the mobile "Fix N issues" sheet render from ONE implementation. Takes an optional `ctx` because the sheet is rendered by `BottomActions` high in the layout tree, outside the form's own provider |
| ui/components | OverlayContext / useIsInsideOverlay | Context | 2026-08-26 (W21) — `appkit/src/ui/components/overlay-context.ts`, mounted by `Modal` and `SideDrawer`. Lets route-level chrome suppress itself inside an overlay: a viewport-fixed bar renders *behind* the backdrop, below the dialog it belongs to, and neither portal leaves a detectable DOM trace at the form's position |
| features/layout | useFormBottomActions | Hook | 2026-08-26 (W21) — `appkit/src/features/layout/hooks/useFormBottomActions.tsx` — pins a form's Save/Cancel row and its error sheet into the `BottomChrome` tier via `useBottomActions`. On by default in `<SectionForm>`; opt-in on `<Form bottomBar={…}>`. Mobile-only, so it is ADDITIVE to `<FormErrorSummary>`, never a substitute |
| features/status-history | RecordStatusTimeline | Component | 2026-08-26 (W18) — `appkit/src/features/status-history/components/RecordStatusTimeline.tsx` — the timeline for the seven entities with no fixed phase sequence (tickets, payouts, stores, scammers, bids, shipments, catalogue). Orders/offers keep dedicated wrappers because they pre-render unreached steps; these have nothing to pre-render |
| _internal/shared/history | withHistory · StatusChangeEntry · SerialisedStatusChangeEntry | Primitive | Nine adopters as of W18. `withHistory` is exported publicly ONLY because ads live inside the `siteSettings` singleton and have no repository — every other adopter funnels through its repository's write primitives |
| features/email | renderNotificationEmail · NOTIFICATION_EMAIL_TEMPLATES | Server | 2026-08-26 (W19) — `Record<NotificationType, TemplateDef>`, so a new type cannot compile without an email. `emailHtml` had 0 of 40 callers, so all 28 types shipped `<p>{message}</p>` |
| _internal/shared/features/notifications | resolveNotificationActionUrl · NotificationAudience | Server | 2026-08-26 (W19) — `(relatedType, relatedId, audience) → URL`. `actionUrl` had 5 of 40 callers and the destination is role-dependent. `bid`/`offer`/`review` resolve to a LIST because no per-record page exists in any role |
| features/admin/schemas | NOTIFICATION_TYPE_VALUES · NOTIFICATION_RELATED_TYPE_VALUES | Constant | 2026-08-26 (W19) — the ONE notification-type union (28 values). Four existed; the PUBLIC export was the seed factory's 9-value guess, and between them they invented four values no notification ever had |
...
| products | toProductItem | sync(doc) | Firestore doc → card-grid `ProductItem`; now copies `listingType` (Root Cause #48 fix) so related/grouped cards route correctly, plus `storeId`/`storeSlug` (2026-09-01 — it carried `storeName` alone, the same dropped-field shape one field over, so a related/grouped card could not tell "no seller" from "seller name not denormalized") |
...
| useBottomActions | Actions[] | Bottom action bar items. `desktop?: "hidden" \| "after-scroll" \| "always"` (default `"hidden"`) controls whether the bar may also show above `lg`, where it has always been `lg:hidden`; `"after-scroll"` reveals it once the page scrolls past the hero so the primary CTA comes back. **2026-08-26 (W21): a CLAIM STACK, not last-writer-wins** — each call claims on mount and releases on unmount, only the top claimant publishes, and releasing restores the one underneath instead of blanking. `enabled: false` opts out WITHOUT taking a claim. `infoOpenSignal?: number` force-opens the panel once per increment |
...
| media-url.ts | resolveMediaUrl, **resolveVideoUrl**, getYouTubeVideoId, FIREBASE_STORAGE_HOST, GCS_HOST, ResolveMediaUrlOptions | Normalises a media URL onto the `/media/…` watermark proxy. The GCS branch drops the bucket segment **only when it matches `NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET`** — it used to do so for every `storage.googleapis.com` URL, rewriting foreign buckets into `/media/…` paths that 404 (Root Cause #76-adjacent, fixed 2026-08-26). `resolveVideoUrl` = `resolveMediaUrl(url, { externalProxy: false })`: same mapping for media we own, but a third-party URL is returned **untouched** rather than sent to `/api/media/ext`, which is image-only and 400s on `video/mp4` (Root Cause #27). Use it for anything that becomes a `<video src>`; posters stay on `resolveMediaUrl` |
...
| removeCartOpsFor(productIds) (2026-09-01) | Function — drop the queued ops for specific products after their removal has already been applied server-side. Deliberately NOT `pushCartOp({op:"remove"})`: every call site pushes `add` only, so the queue's `remove` half is dead code whose replay passes a productId to an itemId-keyed route. `clearCartOps` + this are now exported from `appkit/src/client.ts` — without them the cart page could not drain the queue, so a cleared row was re-layered onto the next render and re-POSTed by `useSyncManager` within 30s |
...
| onJobCreated | documentCreated `jobs/{jobId}` (300s timeout, 512MiB) | Async job primitive (2026-08-15) — dispatches by `job.jobType` through `JOB_RUNNERS` (`appkit/src/_internal/server/jobs/core/jobRunners.ts`), pings `bulk_events/{jobId}` RTDB on completion. The single Function that does all heavy async admin work Vercel routes can't. Registered job types: `payoutsWeekly`, `hardBanCascade`, `whatsappNotify`, `resetOtpVerification`, `newsletterExport` (2026-08-19 — builds the subscriber CSV, replacing the prior synchronous in-route build; result surfaces via `useBulkEvent` on `AdminNewsletterView`), `whatsappCatalogImport` (2026-08-22 — see below), `storeNameBackfill` (2026-09-01 — re-syncs the denormalized `storeName`/`storeSlug` onto every product a store owns; enqueued by `updateSellerStore` when a rename actually changes the name. A job rather than an inline write because the update is unbounded in listing count and would blow the 10s route ceiling. Idempotent, so `npm run products:backfill-store-names` can always catch up) — see `enqueueJob()` below. |
...
| audit-client-entry-in-server.mjs | strict-0 | A **Server**-rendered file importing a runtime value from `@mohasinac/appkit/client` (a `"use client"` entry) — the binding is a client-reference proxy, so calling it or property-accessing it throws during the server render as an opaque React #441. Rendering one as JSX is fine and is not flagged. Scope is deliberately narrow — Next.js route entries (`page`/`layout`/`template`/`default`.tsx, `route.ts`) and `async function` components only — because a client-graph helper module (`src/lib/api/*-client.ts`) **must** keep importing from `/client` or it trips `audit-client-server-only-leak` below. The two audits pull in opposite directions; this scope is what keeps them compatible. Suppression `// audit-client-entry-ok: <reason>`. Root Cause #76, 2026-08-26 |
...
| audit-seo-canonical-host.mjs | strict-0 | **2026-08-27.** The canonical host must have exactly ONE definition (`appkit.config.js` `seo.siteUrl`) and every SEO surface must derive from it. Rules: `HOST_DISAGREEMENT` (two owners resolve it differently), `HARDCODED_HOST_LITERAL` (a bare site-host URL outside the owner), `UNSAFE_ENV_FALLBACK` (`?? ""` / `?? "App"` / localhost — renders a valid-looking but wrong entity instead of failing), `REGISTRY_STALE`. `appkit.config.js` hardcoded the apex while `seo.server.ts` read env; once the Vercel env pointed at www, robots.txt/sitemap/`metadataBase` said apex and every page canonical said www, so all 182 sitemap URLs 307-redirected and the site left Google (root-cause #81). No suppression marker |
| audit-seo-sitemap-parity.mjs | strict-0 | **2026-08-27.** A sitemap section returning zero URLs is indistinguishable from a site that genuinely has none of that entity. Rules: `DISCRIMINATOR_NOT_IN_UNION` (a fetcher filters on a literal absent from its own TS union — `categoryType == "listing"` is not a `CategoryType` and hid ~47 category pages), `SILENT_EMPTY_SECTION` (a catch that swallows to `[]` unlogged), `NO_TESTDATA_GUARD` (a `.select()` missing the test-data field, so 34 tester-sandbox URLs reached the public sitemap), `REGISTRY_STALE` (the per-section count digest lost a section). No suppression marker |
