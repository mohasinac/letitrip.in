...
type FooterSection = {
  ...
  content: 
    ...
    | { type: 'social'; socials: Array<{ name: string; href: string; icon: ReactNode }> };
};
  ...
  return (
    <footer id="footer" className={`mt-auto mb-16 md:mb-0 ${layout.footerBg}`}>
      <div className="container mx-auto px-6 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          {footerSections.map((section) => (
            <div key={section.title}>
              <h3 className={`font-bold text-lg mb-4 ${colors.footer.title}`}>{section.title}</h3>
              
              {section.content.type === 'text' && (
                <p className={`${colors.footer.text} text-sm`}>{section.content.text}</p>
              )}
              ...
              {section.content.type === 'links' && (
                <ul className="space-y-2 text-sm">
                  {section.content.links.map((link) => (
                    <li key={link.href}>
                      <a href={link.href} className={`${colors.footer.text} ${colors.footer.textHover}`}>
                        {link.label}
                      </a>
                    </li>
                  ))}
        ...
        <div className={`border-t ${colors.footer.border} mt-8 pt-6 text-center text-sm ${colors.footer.copyright}`}>
