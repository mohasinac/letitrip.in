...
const COLLECTION_META: Record<SeedCollectionName, CollectionMeta> = {
  ...
  shipments: {
    ...
    icon: "📦",
    group: "transactional",
    description: "Admin-only import/procurement tracking. Header + overall customs/shipping/labor totals; per-lot and per-item detail live in the sibling shipmentLots/shipmentItems collections. `totals` is a persisted rollup recomputed by a Firestore-triggered Function cascade, never inline.",
    slugPattern: "shipment-{supplier}-{yyyymmdd}-{rand6}",
    seededItems: ["3-4 sample shipments across planning/in_transit/received/completed statuses"],
    pendingItems: [],
    uiPath: "/admin/shipments",
    fields: [
      { name: "shipmentNumber",        type: "string",    searchable: true, filterable: true, indexed: true, note: "admin-entered, unique" },
      { name: "supplierName",          type: "string",    searchable: true },
      { name: "status",                type: "enum",      filterable: true, sortable: true, indexed: true },
      { name: "etaDate",               type: "timestamp", sortable: true, indexed: true },
      { name: "customsTotalPaise",     type: "number" },
      { name: "shippingTotalPaise",    type: "number" },
      { name: "laborHoursSpent",       type: "number" },
      { name: "totals",                type: "map",       note: "persisted rollup — see ShipmentTotals" },
      { name: "totalsComputedAt",      type: "timestamp" },
      { name: "createdBy",             type: "ref" },
      { name: "createdAt",             type: "timestamp", sortable: true, filterable: true, indexed: true },
      { name: "updatedAt",             type: "timestamp", sortable: true },
    ],
  },
  shipmentLots: {
    ...
    icon: "🧺",
    group: "transactional",
    description: "One doc per lot within a shipment (FK shipmentId). Holds weight/cost (drives customs-by-value + shipping-by-weight allocation), per-lot images, and the 'remainder' lump estimate for items not worth individually itemizing.",
    slugPattern: "auto-id (Firestore-generated)",
    mediaFields: ["images"],
    seededItems: ["2-3 lots per seeded shipment"],
    pendingItems: [],
    uiPath: "/admin/shipments/[id]/edit",
    fields: [
      { name: "shipmentId",                    type: "ref",       filterable: true, indexed: true },
      { name: "shipmentStatus",                type: "enum",      filterable: true, indexed: true, note: "denormalized from parent shipment" },
      { name: "lotName",                       type: "string",    searchable: true },
      { name: "weightGrams",                   type: "number",    note: "drives shipping allocation" },
      { name: "purchaseCostPaise",             type: "number",    note: "drives customs allocation" },
      { name: "images",                        type: "array" },
      { name: "remainderItemCount",            type: "number" },
      { name: "remainderEstimatedValuePaise",  type: "number" },
      { name: "itemCount",                     type: "number",    note: "recomputed by onShipmentItemWrite" },
      { name: "mainItemsProjectedRevenuePaise",type: "number",    note: "recomputed by onShipmentItemWrite" },
      { name: "customsAllocatedPaise",         type: "number",    note: "recomputed by the allocation cascade" },
      { name: "shippingAllocatedPaise",        type: "number",    note: "recomputed by the allocation cascade" },
      { name: "totalLandedCostPaise",          type: "number" },
      { name: "projectedRevenuePaise",         type: "number",    sortable: true },
      { name: "projectedProfitPaise",          type: "number",    sortable: true, indexed: true, note: "Projections list sort key" },
      { name: "createdAt",                     type: "timestamp", sortable: true, indexed: true },
  ...
  shipmentItems: {
    ...
    icon: "🏷️",
    group: "transactional",
    description: "One doc per individually-tracked 'main' item within a lot (FK lotId/shipmentId). Capped at 500/lot (Firestore's native batch-write limit) — bulk paste-import writes a whole lot's items in one WriteBatch. Reuses ProductDraftFields (title/price/condition/categorySlugs/brandSlug) so linking to a real product is a near-literal field copy.",
    slugPattern: "auto-id (Firestore-generated)",
    seededItems: ["a handful of main items per seeded lot, one self-use, one pre-linked to a seeded product"],
    pendingItems: [],
    uiPath: "/admin/shipments/[id]/lots/[lotId]/items",
    fields: [
      { name: "shipmentId",              type: "ref",    filterable: true },
      { name: "lotId",                   type: "ref",    filterable: true, indexed: true },
      { name: "title",                   type: "string", searchable: true },
      { name: "quantity",                type: "number" },
      { name: "isForSelfUse",            type: "boolean",filterable: true, note: "zeroes this item's contribution to lot revenue" },
      { name: "price",                   type: "number", note: "paise — projected sale price" },
      { name: "condition",               type: "enum",   filterable: true },
      { name: "categorySlugs",           type: "array" },
      { name: "brandSlug",               type: "ref" },
      { name: "linkedProductId",         type: "ref",    filterable: true, indexed: true, note: "set by 'Create pre-order link'" },
      { name: "linkedProductSlug",       type: "string" },
      { name: "linkedProductListingType",type: "enum" },
      { name: "createdAt",               type: "timestamp", sortable: true, indexed: true },
      { name: "updatedAt",               type: "timestamp" },
    ],
  },
...
function StatusDot({
  ...
  if (state === "running")
    return (
      <Span
        className="inline-block w-3.5 h-3.5 border-2 border-amber-500 border-t-transparent animate-spin shrink-0" rounded="full"
        aria-label="Running"
      />
    );
  if (state === "queued")
  ...
  if (state === "error")
    return <Span className="shrink-0" color="error" aria-label="Error">✗</Span>;
  return null;
}
    ...
    <>
      <Row justify="between" gap="sm" wrap className="mb-2">
        <Text className="text-indigo-600 dark:text-indigo-400 tracking-wider m-0" size="xs" weight="bold" transform="uppercase">
          📐 Schema Fields <Span className="text-zinc-400 normal-case tracking-normal" weight="normal">({fields.length} fields)</Span>
        </Text>
        <Row gap="sm">
          <Input
            bare
      ...
      <Stack gap="md">
        ...
        <Div>
          <Row justify="between" className="mb-2">
            <Span color="muted" className="font-mono" size="xs">DB: {existingCount} / {seedCount} seed docs</Span>
          </Row>
  ...
  return (
    <Div className="grid grid-cols-3 gap-[var(--appkit-space-3)] sm:gap-[var(--appkit-space-4)]">
      ...
      <Stack className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/40 text-center" gap="xs" rounded="xl" padding="md">
        ...
        <Span color="muted" size="xs">docs in seed files</Span>
      </Stack>
      <Stack className="dark:border-white/10 text-center" surface="muted" gap="xs" rounded="xl" padding="md" border="default">
        <Span color="muted" className="font-extrabold font-mono leading-none" size="2xl">{collectionCount}</Span>
        <Span color="muted" size="xs">collections</Span>
      </Stack>
    </Div>
  );
}
