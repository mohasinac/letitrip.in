 ...
 * Style via `className` using THEME_CONSTANTS tokens.
 *
 * @example
 * ```tsx
 * import { Section, Article, Main, Aside, Nav, Ul, Ol, Li } from '@/components';
 *
 ...
 *     <Li><TextLink href="/products">Products</TextLink></Li>
 *   </Ul>
 * </Nav>
 * ```
 */
 ...
 * (that is handled by `TitleBar` / `MainNavbar` from `@/components`).
 *
 * @example
 * ```tsx
 * <Article>
 ...
 * (that is handled by `Footer` from `@/components`).
 *
 * @example
 * ```tsx
 * <Article>
 *   <Text>{post.body}</Text>
 *   <BlockFooter className="mt-4 flex gap-4">
 *     <Caption>{formatDate(post.updatedAt)}</Caption>
