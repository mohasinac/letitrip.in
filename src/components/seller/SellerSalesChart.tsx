...
export default function SellerSalesChart() {
  ...
  useEffect(() => {
    const fetchSalesData = async () => {
      try {
        const response = await fetch(`/api/seller/analytics/sales?period=${activeTab}`);
        if (response.ok) {
  ...
  const trend = currentData[currentData.length - 1] - currentData[currentData.length - 2];
  const trendPercentage = ((trend / currentData[currentData.length - 2]) * 100).toFixed(1);
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="text-center">
            <div className="text-2xl font-bold text-gray-900">
              {activeMetric === 'revenue' ? `₹${total.toLocaleString()}` : total.toLocaleString()}
            </div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-gray-900">
              {activeMetric === 'revenue' ? `₹${average.toLocaleString()}` : average.toLocaleString()}
            </div>
