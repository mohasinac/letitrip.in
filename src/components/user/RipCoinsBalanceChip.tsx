...
import { useAuth, useRipCoinBalance } from "@/hooks";
import { ROUTES, THEME_CONSTANTS } from "@/constants";
import { Span, Text, TextLink } from "@/components";

const { themed } = THEME_CONSTANTS;

interface RipCoinsBalanceChipProps {
...
export function RipCoinsBalanceChip({
  minimumRequired,
  className = "",
  variant = "chip",
}: RipCoinsBalanceChipProps) {
  const { user } = useAuth();
  const t = useTranslations("auctions");
  const { data, isLoading } = useRipCoinBalance();

  if (!user || isLoading) return null;

  const balance = data?.ripcoinBalance ?? 0;
  ...
  if (variant === "chip") {
    return (
      ...
      >
        ...
        <Span className="hidden sm:inline opacity-70">{t("ripcoinsUnit")}</Span>
        <TextLink
    ...
    >
      ...
      {isInsufficient && minimumRequired !== undefined && (
        <Text size="xs" className="mt-2 text-amber-600 dark:text-amber-400">
          {t("ripcoinsNeeded", { count: minimumRequired - balance })}
        </Text>
      )}
    </div>
  );
