...
function StatPill({ label, value }: { label: string; value: string | number }) {
  return (
    <Div className="border border-[var(--appkit-color-border)] bg-[var(--appkit-color-surface)] px-4 py-3" rounded="lg">
          ...
          {!bidsData || bidsData.bids.length === 0 ? (
            ...
            <Stack gap="sm">
              {bidsData.bids.slice(0, 5).map((b) => (
                <Div key={b.id} className="border border-[var(--appkit-color-border)] px-3 py-2" rounded="md">
          ...
          {!reviewsData || reviewsData.reviews.length === 0 ? (
            ...
            <Stack gap="sm">
              {reviewsData.reviews.slice(0, 5).map((r) => (
                <Div key={r.id} className="border border-[var(--appkit-color-border)] px-3 py-2" rounded="md">
