...
vi.mock("@mohasinac/appkit", async () => {
  ...
  return {
    ...
    orderRepository: { findById: vi.fn(), update: vi.fn() },
    productRepository: { findById: vi.fn() },
    OrderStatusValues: { SHIPPED: "SHIPPED", DELIVERED: "DELIVERED", CONFIRMED: "CONFIRMED" },
    ShippingMethodValues: { SHIPROCKET: "shiprocket" },
    shiprocketAuthenticate: vi.fn(),
    shiprocketAddPickupLocation: vi.fn(),
    shiprocketVerifyPickupOTP: vi.fn(),
    shiprocketCreateOrder: vi.fn(),
    shiprocketGenerateAWB: vi.fn(),
    shiprocketGeneratePickup: vi.fn(),
    isShiprocketTokenExpired: vi.fn().mockReturnValue(false),
    SHIPROCKET_TOKEN_TTL_MS: 86400000,
    buildShiprocketTrackingUrl: vi.fn().mockReturnValue("https://track.shiprocket.in/abc"),
    SHIPROCKET_STATUS_PICKUP_SCHEDULED: "Pickup Scheduled",
