...
export function initializeSocketServer(httpServer: HTTPServer) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join-auction', (auctionId: string) => {
      ...
      console.log(`[Socket.io] Client ${socket.id} joined auction ${auctionId}`);
    ...
    socket.on('bid-placed', async (data: { auctionId: string; userId: string; amount: number }) => {
      ...
      console.log(`[Socket.io] Bid placed on auction ${data.auctionId}: ₹${data.amount}`);
    });

    // Setup auto-bid
    socket.on('setup-autobid', (data: { auctionId: string; userId: string; maxBid: number }) => {
      socket.join(`autobid-${data.auctionId}-${data.userId}`);
      console.log(`[Socket.io] Auto-bid setup for user ${data.userId} on auction ${data.auctionId}: max ₹${data.maxBid}`);
    });

    // Cancel auto-bid
    socket.on('cancel-autobid', (data: { auctionId: string; userId: string }) => {
      socket.leave(`autobid-${data.auctionId}-${data.userId}`);
      console.log(`[Socket.io] Auto-bid cancelled for user ${data.userId} on auction ${data.auctionId}`);
    });
...
export async function broadcastNewBid(auctionId: string, bid: any) {
  ...
  console.log(`[Socket.io] Broadcast new bid: ₹${bid.amount} on auction ${auctionId}`);
 ...
 */
async function processAutoBids(auctionId: string, currentBid: number, excludeUserId: string) {
  ...
  autoBidsSnapshot.forEach((doc) => {
    ...
    if (autoBid.user_id !== excludeUserId && autoBid.max_bid > highestAutoBidAmount) {
  ...
  if (highestAutoBid) {
    ...
    const autoBidAmount = Math.min(currentBid + minIncrement, highestAutoBid.max_bid);

    // Place auto-bid via API
    console.log(`[Socket.io] Auto-bid triggered: User ${highestAutoBid.user_id} bids ₹${autoBidAmount}`);
    ...
    io?.to(`autobid-${auctionId}-${highestAutoBid.user_id}`).emit('autobid-executed', {
      auctionId,
      amount: autoBidAmount,
 ...
 */
export function broadcastAuctionStatus(auctionId: string, status: string, data?: any) {
  if (!io) return;
...
async function sendCountdownSync(auctionId: string) {
  ...
  const endTime = auction.end_time instanceof Timestamp 
 ...
 */
export function broadcastEndingSoon(auctionId: string, minutesRemaining: number) {
  if (!io) return;
  ...
  console.log(`[Socket.io] Broadcast ending soon: ${auctionId} (${minutesRemaining}m remaining)`);
}
