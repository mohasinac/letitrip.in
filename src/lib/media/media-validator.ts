...
export function validateFileType(
  ...
  const isValid = allowedTypes.some(type => {
    const formatKey = type === 'image' ? 'IMAGES' : type === 'video' ? 'VIDEOS' : 'DOCUMENTS';
    ...
    img.onload = () => {
      ...
      if (constraints.aspectRatio !== null && constraints.aspectRatio !== undefined) {
        ...
        if (Math.abs(actualRatio - expectedRatio) > tolerance) {
          const recommendedRatioStr = constraints.recommendedRatio || expectedRatio.toFixed(2);
...
): Promise<{ isValid: boolean; error?: string; metadata?: { duration: number; width: number; height: number } }> {
...
export async function validateMedia(
  ...
  if (mediaType === 'image' && constraintKey && constraintKey in IMAGE_CONSTRAINTS) {
  ...
  if (mediaType === 'video' && constraintKey && constraintKey in VIDEO_CONSTRAINTS) {
...
export function getMediaType(file: File): MediaType | null {
  ...
  if ((SUPPORTED_FORMATS.IMAGES.mimeTypes as readonly string[]).includes(mimeType)) {
    return 'image';
  }
  
  if ((SUPPORTED_FORMATS.VIDEOS.mimeTypes as readonly string[]).includes(mimeType)) {
    return 'video';
  }
  
  if ((SUPPORTED_FORMATS.DOCUMENTS.mimeTypes as readonly string[]).includes(mimeType)) {
    return 'document';
  }
