...
export const categoryQuerySchema = z.object({
  ...
  // Sorting
  sortBy: z.enum(['name', 'createdAt', 'sortOrder', 'productCount']).default('sortOrder').optional(),
...
export type ValidateLeafCategoryInput = z.infer<typeof validateLeafCategorySchema>;
...
export type GetSimilarCategoriesInput = z.infer<typeof getSimilarCategoriesSchema>;

/**
 * Utility: Check if category is leaf (has no children)
 * Note: This would typically be checked against the database
 */
export function isLeafCategory(category: { hasChildren?: boolean; childCount?: number }): boolean {
  return !category.hasChildren && (category.childCount === undefined || category.childCount === 0);
}
 ...
 */
export function buildCategoryPath(parentPath: string | null, categoryId: string): string {
