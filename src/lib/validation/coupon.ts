 ...
 * Coupon Status enum
 */
export const CouponStatus = z.enum(['active', 'inactive', 'expired', 'used-up']);
...
export const createCouponSchema = z.object({
  ...
  tiers: z.array(tieredDiscountSchema)
  ...
  usageLimit: z.number()
    .int()
    .positive()
    .optional(), // Total usage limit (null = unlimited)
  ...
  if (data.applicability === 'category') {
    return data.applicableCategories !== undefined && data.applicableCategories.length > 0;
  }
  if (data.applicability === 'product') {
    return data.applicableProducts !== undefined && data.applicableProducts.length > 0;
  }
  return true;
}, {
  message: 'Applicable categories/products are required based on applicability type',
  path: ['applicability'],
});
...
export const couponQuerySchema = z.object({
  ...
  // Sorting
  sortBy: z.enum(['code', 'name', 'createdAt', 'startDate', 'endDate', 'usageCount']).default('createdAt').optional(),
...
export type BulkUpdateCouponStatusInput = z.infer<typeof bulkUpdateCouponStatusSchema>;
