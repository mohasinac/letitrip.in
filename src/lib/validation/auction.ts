 ...
 * Auction Status enum
 */
export const AuctionStatus = z.enum(['draft', 'scheduled', 'live', 'ended', 'cancelled']);
...
export const createAuctionSchema = z.object({
  ...
  minimumBidderRating: z.number()
    .min(0)
    .max(5)
    .optional(), // Minimum rating to bid
  
  // Auto-bid Settings
  allowAutoBid: z.boolean().default(true).optional(),
  
  maxAutoBidders: z.number()
    .int()
    .positive()
    .optional(), // Limit auto-bidders
  
  // Payment Terms
  paymentDueHours: z.number()
    .int()
    .positive()
    .default(48)
    .optional(), // Hours after auction ends
  
  acceptedPaymentMethods: z.array(z.enum(['razorpay', 'paypal', 'cod']))
  ...
  returnWindowDays: z.number()
    ...
    .min(0)
    .max(14)
    .optional(), // Shorter window for auctions
  
  returnConditions: z.string()
    .max(500)
    .optional(),
  ...
  featuredPriority: z.number()
    .int()
    .min(0)
    .max(100)
    .optional(), // Higher = more prominent
...
export const placeBidSchema = z.object({
  ...
  maxAutoBid: z.number()
    .positive()
    .optional(), // Maximum amount for auto-bidding
 ...
 */
export function canBid(startTime: Date, endTime: Date): {
  canBid: boolean;
  reason?: string;
} {
  const now = new Date();
