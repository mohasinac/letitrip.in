...
export class BeybladeStatsService {
   ...
   */
  async getBeybladesByType(type: 'attack' | 'defense' | 'stamina' | 'balanced'): Promise<BeybladeStats[]> {
    try {
  ...
  async saveBeybladeStats(stats: BeybladeStats, userId: string): Promise<void> {
    try {
      ...
      const beybladeData = {
        ...
        createdBy: doc.exists ? (doc.data() as BeybladeStats).createdBy : userId,
      };

      await docRef.set(beybladeData);
    } catch (error) {
      console.error('Error saving Beyblade stats:', error);
      throw new Error(`Failed to save Beyblade stats: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  async batchUpdateBeyblades(updates: BeybladeStats[], userId: string): Promise<void> {
    try {
      const batch = this.db.batch();
      const now = new Date().toISOString();
      ...
      throw new Error(`Failed to batch update Beyblades: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async searchBeybladesByName(query: string): Promise<BeybladeStats[]> {
    try {
      const snapshot = await this.db.collection(this.collection).get();
      const allBeyblades = snapshot.docs.map(doc => doc.data() as BeybladeStats);
  ...
  async getTopBeybladesByCategory(
    ...
    try {
      ...
      return allBeyblades
        .sort((a, b) => b.typeDistribution[category] - a.typeDistribution[category])
        .slice(0, limit);
    } catch (error) {
