...
export async function processEndedAuctions() {
  try {
    ...
    console.log(`[Auction Scheduler] Found ${snapshot.size} auctions to process`);
    ...
    console.error('[Auction Scheduler] Error processing ended auctions:', error);
  }
}
...
async function closeAuction(auctionId: string) {
  try {
    ...
    if (bidsSnapshot.empty) {
      ...
      console.log(`[Auction Scheduler] Auction ${auctionId} ended with no bids`);
    ...
    if (auction.reserve_price && finalBid < auction.reserve_price) {
      ...
      console.log(`[Auction Scheduler] Auction ${auctionId} ended - reserve price not met`);
    ...
    console.log(`[Auction Scheduler] Auction ${auctionId} won by user ${winnerId} for ₹${finalBid}`);
    ...
    console.error(`[Auction Scheduler] Error closing auction ${auctionId}:`, error);
  }
}
 ...
 */
async function createWinnerOrder(auction: any, winnerId: string, finalBid: number) {
  try {
    // Get winner details
    ...
    if (!addressesSnapshot.empty) {
      shippingAddress = { id: addressesSnapshot.docs[0].id, ...addressesSnapshot.docs[0].data() };
    }
    ...
    console.log(`[Auction Scheduler] Created order ${orderId} for winner ${winnerId}`);
...
async function updateInventory(productId: string) {
  try {
    ...
    if (productDoc.exists) {
      ...
      console.log(`[Auction Scheduler] Updated inventory for product ${productId}: ${newStock} remaining`);
    }
  } catch (error) {
...
async function notifyWinner(auction: any, winnerId: string, finalBid: number) {
  console.log(`[Notification] Winner ${winnerId}: You won auction "${auction.name}" for ₹${finalBid}`);
...
async function notifySeller(auction: any, winnerId: string, finalBid: number) {
  console.log(`[Notification] Seller: Auction "${auction.name}" sold to user ${winnerId} for ₹${finalBid}`);
...
async function notifySellerNoWinner(auction: any) {
  console.log(`[Notification] Seller: Auction "${auction.name}" ended with no bids`);
  // TODO: Send email to seller
  // - No bids received
  // - Option to relist
}

async function notifyReserveNotMet(auction: any, highestBidderId: string, highestBid: number) {
  console.log(`[Notification] Auction "${auction.name}" ended - reserve price not met (highest: ₹${highestBid})`);
  // TODO: Send emails
