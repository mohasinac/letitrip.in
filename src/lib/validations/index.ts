...
export const createDynamicSchema = {
   ...
   */
  conditionalRequired: <T>(schema: z.ZodSchema<T>, condition: (data: any) => boolean) =>
   ...
   */
  arrayLength: <T>(itemSchema: z.ZodSchema<T>, min: number, max: number, itemName: string) =>
    z.array(itemSchema)
      .min(min, `At least ${min} ${itemName}${min === 1 ? ' is' : 's are'} required`)
...
export const BusinessValidators = {
   ...
   * Validate IFSC code
   */
  ifscCode: z.string().regex(
    ValidationPatterns.ifsc,
  ...
  minimumAge: (minimumAge: number = 18) =>
    z.string().datetime().refine((dateString) => {
...
export const CommonFormSchemas = {
  ...
  fileUpload: z.object({
    files: z.array(
      createDynamicSchema.fileUpload(['image/jpeg', 'image/png', 'image/webp'], 5 * 1024 * 1024)
