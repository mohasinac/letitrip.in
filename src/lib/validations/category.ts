...
export const categoryFormSchema = z.object({
  ...
  slug: z
    ...
    .regex(
      /^buy-[a-z0-9]+(?:-[a-z0-9]+)*$/,
      'Slug must start with "buy-" followed by lowercase letters, numbers, and hyphens',
    )
    .trim(),
  ...
  sortOrder: z
    ...
    .pipe(
      z.coerce.number().int().nonnegative("Sort order must be non-negative"),
    )
    .default(0),
...
export function validateCategoryDepth(
  ...
  if (newDepth >= maxDepth) {
    return {
      valid: false,
      error: `Category hierarchy cannot exceed ${maxDepth} levels`,
    };
  }
