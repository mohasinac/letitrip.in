...
const phoneSchema = z.string().regex(/^[+]?[\d\s\-\(\)]{10,15}$/, 'Please enter a valid phone number');
const pincodeSchema = z.string().regex(/^\d{6}$/, 'Please enter a valid 6-digit pincode');
const urlSchema = z.string().url('Please enter a valid URL');
const slugSchema = z.string().regex(/^[a-z0-9-]+$/, 'Slug can only contain lowercase letters, numbers, and hyphens');
...
export const addressSchema = z.object({
  ...
  phone: phoneSchema,
  addressLine1: z.string().min(5, 'Street address is required').max(200, 'Address is too long'),
  ...
  instructions: z.string().max(500, 'Delivery instructions are too long').optional(),
});
...
export const productSEOSchema = z.object({
  title: z.string().min(10, 'SEO title is too short').max(60, 'SEO title should be under 60 characters'),
  description: z.string().min(50, 'SEO description is too short').max(160, 'SEO description should be under 160 characters'),
...
export const createProductSchema = z.object({
  // Basic Information
  name: z.string().min(2, 'Product name is required').max(200, 'Product name is too long'),
  slug: slugSchema,
  ...
  // Pricing
  price: z.number().positive('Price must be positive').max(10000000, 'Price is too high'),
  ...
  quantity: z.number().int().nonnegative('Quantity must be non-negative').max(100000, 'Quantity is too high'),
  lowStockThreshold: z.number().int().nonnegative('Low stock threshold must be non-negative').max(1000, 'Threshold is too high').default(10),
  trackQuantity: z.boolean().default(true),
  allowBackorders: z.boolean().default(false),
  
  // Physical Properties
  weight: z.number().positive('Weight is required for shipping calculations').max(1000, 'Weight is too high'),
  ...
  videos: z.array(videoFileSchema).max(5, 'Maximum 5 videos allowed').optional(),
...
export const updateProductSchema = z.object({
  // Basic Information
  name: z.string().min(2, 'Product name is required').max(200, 'Product name is too long').optional(),
  slug: slugSchema.optional(),
  description: z.string().min(10, 'Description must be at least 10 characters').max(5000, 'Description is too long').optional(),
  shortDescription: z.string().max(500, 'Short description is too long').optional(),
  
  // Pricing
  price: z.number().positive('Price must be positive').max(10000000, 'Price is too high').optional(),
  compareAtPrice: z.number().positive('Compare at price must be positive').optional(),
  cost: z.number().positive('Cost must be positive').optional(),
  
  // Inventory
  sku: z.string().min(1, 'SKU is required').max(100, 'SKU is too long').optional(),
  barcode: z.string().max(50, 'Barcode is too long').optional(),
  quantity: z.number().int().nonnegative('Quantity must be non-negative').max(100000, 'Quantity is too high').optional(),
  lowStockThreshold: z.number().int().nonnegative('Low stock threshold must be non-negative').max(1000, 'Threshold is too high').optional(),
  trackQuantity: z.boolean().optional(),
  allowBackorders: z.boolean().optional(),
  
  // Physical Properties
  weight: z.number().positive('Weight is required for shipping calculations').max(1000, 'Weight is too high').optional(),
  weightUnit: z.enum(['kg', 'g', 'lb', 'oz']).optional(),
  dimensions: productDimensionsSchema.optional(),
  
  // Media
  images: z.array(imageFileSchema).min(1, 'At least one image is required').max(10, 'Maximum 10 images allowed').optional(),
  videos: z.array(videoFileSchema).max(5, 'Maximum 5 videos allowed').optional(),
...
export const createCategorySchema = z.object({
  ...
  sortOrder: z.number().int().nonnegative('Sort order must be non-negative').default(0),
  seo: categorySEOSchema.optional(),
  commission: z.number().min(0).max(100, 'Commission cannot exceed 100%').optional(),
...
export const orderReturnSchema = z.object({
  ...
  reason: z.enum([
    'damaged', 'wrong_item', 'not_as_described', 'quality_issues', 
    'size_issue', 'color_issue', 'changed_mind', 'other'
  ]),
...
export const updateCartItemSchema = z.object({
  quantity: z.number().int().nonnegative('Quantity must be non-negative').max(10, 'Maximum quantity is 10'),
});
...
export const couponRestrictionsSchema = z.object({
  ...
  applicablePaymentMethods: z.array(z.enum(['razorpay', 'cod', 'wallet'])).optional(),
...
export const createCouponSchema = z.object({
  code: z.string().min(3, 'Coupon code must be at least 3 characters').max(20, 'Coupon code is too long')
    .regex(/^[A-Z0-9_-]+$/, 'Coupon code can only contain uppercase letters, numbers, hyphens, and underscores'),
  name: z.string().min(2, 'Coupon name is required').max(100, 'Coupon name is too long'),
  description: z.string().max(500, 'Description is too long').optional(),
  type: z.enum(['percentage', 'fixed', 'free_shipping', 'bogo', 'tier']),
  value: z.number().positive('Value must be positive'),
  minimumAmount: z.number().nonnegative('Minimum amount must be non-negative').default(0),
  maximumAmount: z.number().positive('Maximum amount must be positive').optional(),
  maxUses: z.number().int().positive('Max uses must be positive').optional(),
  maxUsesPerUser: z.number().int().positive('Max uses per user must be positive').default(1),
...
export const updateCouponSchema = z.object({
  ...
  code: z.string().min(3, 'Coupon code must be at least 3 characters').max(20, 'Coupon code is too long')
    .regex(/^[A-Z0-9_-]+$/, 'Coupon code can only contain uppercase letters, numbers, hyphens, and underscores').optional(),
  name: z.string().min(2, 'Coupon name is required').max(100, 'Coupon name is too long').optional(),
  description: z.string().max(500, 'Description is too long').optional(),
  type: z.enum(['percentage', 'fixed', 'free_shipping', 'bogo', 'tier']).optional(),
  value: z.number().positive('Value must be positive').optional(),
  minimumAmount: z.number().nonnegative('Minimum amount must be non-negative').optional(),
  maximumAmount: z.number().positive('Maximum amount must be positive').optional(),
  maxUses: z.number().int().positive('Max uses must be positive').optional(),
  maxUsesPerUser: z.number().int().positive('Max uses per user must be positive').optional(),
...
export const createReviewSchema = z.object({
  ...
  title: z.string().min(2, 'Review title is required').max(100, 'Review title is too long'),
  comment: z.string().min(10, 'Review comment must be at least 10 characters').max(2000, 'Review comment is too long'),
  images: z.array(urlSchema).max(5, 'Maximum 5 images allowed').optional(),
...
export const createAuctionSchema = z.object({
  productId: z.string().min(1, 'Product ID is required'),
  title: z.string().min(2, 'Auction title is required').max(200, 'Auction title is too long'),
  description: z.string().max(1000, 'Auction description is too long').optional(),
  ...
  extensionTime: z.number().int().positive('Extension time must be positive').default(10), // minutes
  bidIncrement: z.number().positive('Bid increment must be positive').default(10),
...
export const sellerRegistrationSchema = z.object({
  businessName: z.string().min(2, 'Business name is required').max(200, 'Business name is too long'),
  storeName: z.string().min(2, 'Store name is required').max(100, 'Store name is too long'),
  storeDescription: z.string().max(1000, 'Store description is too long').optional(),
...
export const updateSellerProfileSchema = z.object({
  storeName: z.string().min(2, 'Store name is required').max(100, 'Store name is too long').optional(),
  storeDescription: z.string().max(1000, 'Store description is too long').optional(),
  ...
  policies: z.object({
    ...
    privacyPolicy: z.string().max(2000, 'Privacy policy is too long').optional(),
  }).optional(),
});
...
export const refundRequestSchema = z.object({
  ...
  reason: z.string().min(10, 'Refund reason is required').max(500, 'Reason is too long'),
...
export const adminUserManagementSchema = z.object({
  ...
  action: z.enum(['activate', 'deactivate', 'verify', 'unverify', 'change_role', 'delete']),
  newRole: z.enum(['admin', 'seller', 'user']).optional(),
  reason: z.string().max(500, 'Reason is too long').optional(),
});

export const adminSettingsSchema = z.object({
  siteName: z.string().min(1, 'Site name is required').max(100, 'Site name is too long').optional(),
  siteDescription: z.string().max(500, 'Site description is too long').optional(),
...
export const productFilterSchema = paginationSchema.extend({
  ...
  sort: z.enum([
    ...
    'newest', 'oldest', 'popular', 'rating', 'reviews'
  ]).optional(),
});
...
export const orderFilterSchema = paginationSchema.extend({
  ...
  paymentMethod: z.enum(['razorpay', 'cod', 'wallet', 'bank_transfer']).optional(),
  startDate: dateStringSchema.optional(),
...
export const analyticsFilterSchema = z.object({
  ...
  metrics: z.array(z.enum([
    'revenue', 'orders', 'customers', 'products', 'conversion_rate',
    'average_order_value', 'customer_lifetime_value', 'retention_rate'
  ])).optional(),
});
...
export const createNotificationSchema = z.object({
  type: z.enum(['order', 'product', 'review', 'payment', 'system', 'promotion']),
...
export const createSupportTicketSchema = z.object({
  ...
  category: z.enum([
    'order_issue', 'payment_issue', 'product_inquiry', 'technical_support',
    'account_issue', 'shipping_issue', 'return_refund', 'other'
  ]),
...
export const contactFormSchema = z.object({
  ...
  category: z.enum([
    'general_inquiry', 'business_inquiry', 'partnership', 'media_inquiry',
    'technical_support', 'feedback', 'complaint', 'other'
