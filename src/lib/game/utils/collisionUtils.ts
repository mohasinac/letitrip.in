...
export const resolveCollisionWithAcceleration = (
  bey1: GameBeyblade,
  bey2: GameBeyblade,
): void => {
  const distance = vectorDistance(bey1.position, bey2.position);
  ...
  const chargeKnockbackMultiplier =
    bey1ChargeDash || bey2ChargeDash ? 1.25 : 1.0; // 1.25x for charge dash, 1x for normal

  // Check for special attack multipliers
  const bey1DamageMultiplier = bey1.ultimateAttackActive
    ? 2.0
    : bey1.heavyAttackActive
      ? 1.25
      : 1.0;
  const bey2DamageMultiplier = bey2.ultimateAttackActive
    ? 2.0
  ...
  if (isOppositeSpins) {
    ...
    const avgAcceleration = Math.floor(
      (bey1.acceleration + bey2.acceleration) / 2,
    );
    damage1 =
      (avgAcceleration + bey2.acceleration * bey2DamageMultiplier) * 0.9; // 0.6 * 1.5 = 0.9
    damage2 =
      (avgAcceleration + bey1.acceleration * bey1DamageMultiplier) * 0.9; // 0.6 * 1.5 = 0.9
    ...
    damage1 =
      (accelerationDiff + bey2.acceleration * bey2DamageMultiplier) * 0.9; // 0.6 * 1.5 = 0.9
    damage2 =
      (accelerationDiff + bey1.acceleration * bey1DamageMultiplier) * 0.9; // 0.6 * 1.5 = 0.9
  ...
  const knockback1 = vectorMultiply(
    normal,
    -damage1 * chargeKnockbackMultiplier,
  );
  const knockback2 = vectorMultiply(
    normal,
    damage2 * chargeKnockbackMultiplier,
  );
  ...
  if (knockback1Length <= maxKnockbackDistance) {
    ...
    const limitedKnockback = vectorMultiply(
      knockbackDirection,
...
export const calculateSpinExchange = (
  bey1: GameBeyblade,
  bey2: GameBeyblade,
  force: number,
): number => {
  ...
  const oppositeFactor =
    bey1.config.direction !== bey2.config.direction ? 1.5 : 1.0;

  return Math.min(baseExchange * oppositeFactor, spinDiff * 0.3);
};

export const applySpinExchange = (
  bey1: GameBeyblade,
  bey2: GameBeyblade,
  exchange: number,
): void => {
