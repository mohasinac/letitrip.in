...
export const resolveCollisionWithAcceleration =
  resolveCollisionWithAccelerationUtil;
...
export const resolveCollision = (
  ...
  if (speed > 0)
    return { beyblade1: bey1, beyblade2: bey2, force: 0, angle: 0 };
  ...
  const impulse =
    (-(1 + restitution) * speed) / (1 / bey1.mass + 1 / bey2.mass);

  // Apply impulse to velocities
  const impulseVector = vectorMultiply(normal, impulse);
  bey1.velocity = vectorAdd(
    bey1.velocity,
    vectorMultiply(impulseVector, 1 / bey1.mass),
  );
  bey2.velocity = vectorSubtract(
    bey2.velocity,
    vectorMultiply(impulseVector, 1 / bey2.mass),
  );
