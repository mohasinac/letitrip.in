...
export function drawContactPoints(
  ...
  if (
    !showAlways &&
    !beyblade.heavyAttackActive &&
    !beyblade.ultimateAttackActive
  ) {
    return;
  }
  ...
  for (const contact of stats.pointsOfContact) {
    ...
    const intensity = Math.min(
      255,
      Math.floor((contact.damageMultiplier - 1) * 255),
    );
    ...
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.arc(0, 0, radius, angleRad - widthRad / 2, angleRad + widthRad / 2);
    ctx.stroke();
  }
...
export function drawHitQualityIndicator(
  ...
  const text =
    quality === "perfect" ? "PERFECT!" : quality === "good" ? "Good!" : "Hit";
  const color =
    quality === "perfect"
      ? "#FFD700"
      : quality === "good"
