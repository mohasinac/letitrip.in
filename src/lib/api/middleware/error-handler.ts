...
export class ResponseHelper {
  ...
  static validationError(message = 'Validation failed', errors?: any): NextResponse {
  ...
  return async (request: NextRequest, ...args: T): Promise<NextResponse> => {
    try {
      ...
      if (error.name === 'FirebaseError' || error instanceof FirebaseError) {
        ...
        const mapped = firebaseErrorMap[error.code] || { status: 500, message: 'Database error' };
        return ResponseHelper.error(mapped.message, mapped.status, null, error.code);
      }
