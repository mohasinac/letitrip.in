...
export const calculateWeightedRating = (reviews: any[]): number => {
  ...
  reviews.forEach(review => {
    ...
    if (review.comment && review.comment.length >= RATING_CONSTANTS.THRESHOLDS.DETAILED_REVIEW_MIN_CHARS) {
  ...
  const weightedAverage = totalWeight > 0 ? weightedSum / totalWeight : RATING_CONSTANTS.DEFAULTS.FALLBACK_RATING;
...
export const calculateSellerPerformance = (stats: {
  ...
  const reviewResponseRate = completedOrders > 0 ? (totalReviews / completedOrders) : 0;
  ...
  } else if (avgShippingDays > SELLER_CONSTANTS.FULFILLMENT.STANDARD_SHIPPING_DAYS) {
    shippingPerformance = 'average';
  } else if (avgShippingDays > SELLER_CONSTANTS.FULFILLMENT.FAST_SHIPPING_DAYS) {
