 ...
 * RC Repository
 *
 * Data access layer for RC transaction documents in Firestore.
...
import type {
  ...
  RCTransactionCreateInput,
  RCTransactionType,
} from "@/db/schema";
import { RC_COLLECTION, RC_SIEVE_FIELDS } from "@/db/schema";

class RCRepository extends BaseRepository<RCTransactionDocument> {
  constructor() {
    super(RC_COLLECTION);
  }
   ...
   */
  async create(
    input: RCTransactionCreateInput,
  ): Promise<RCTransactionDocument> {
    const id = this.db.collection(this.collection).doc().id;

    const doc: Omit<RCTransactionDocument, "id"> = {
      ...input,
      createdAt: new Date(),
    };
  ...
  async listForUser(
    userId: string,
    model: SieveModel,
  ): Promise<FirebaseSieveResult<RCTransactionDocument>> {
    return this.sieveQuery<RCTransactionDocument>(model, RC_SIEVE_FIELDS, {
   ...
   */
  async findByBid(bidId: string): Promise<RCTransactionDocument[]> {
    try {
      ...
      return snap.docs.map(
        (d) => ({ id: d.id, ...d.data() }) as RCTransactionDocument,
      );
    } catch (error) {
  ...
  ): Promise<RCTransactionDocument | null> {
    try {
      const snap = await this.getCollection()
      ...
      const d = snap.docs[0];
      return { id: d.id, ...d.data() } as RCTransactionDocument;
    } catch (error) {
      throw new DatabaseError(
  ...
  async findEngageTransactionForBid(
    ...
    try {
      const snap = await this.getCollection()
        .where("bidId", "==", bidId)
        .where("type", "==", "engage" as RCTransactionType)
        .orderBy("createdAt", "desc")
        .limit(1)
        .get();
   ...
   */
  async findById(id: string): Promise<RCTransactionDocument | null> {
    try {
      const snap = await this.getCollection().doc(id).get();
      if (!snap.exists) return null;
      return this.mapDoc<RCTransactionDocument>(snap);
    } catch (error) {
      throw new DatabaseError(`Failed to find RC transaction: ${id}`, error);
    }
  }
...
export const rcRepository = new RCRepository();
