...
describe("StoreProductsView", () => {
  ...
  it("shows spinner while loading", () => {
    mockUseStoreProducts.mockReturnValue({ data: undefined, isLoading: true, error: null });
  ...
  it("renders TablePagination when items are present", () => {
    mockUseStoreProducts.mockReturnValue({
      data: { items: [baseItem], total: 1, page: 1, pageSize: 24, totalPages: 1, hasMore: false },
      isLoading: false,
      error: null,
    });
    render(<StoreProductsView storeSlug="test-store" />);
