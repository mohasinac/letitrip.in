 ...
 * RCWallet
 *
 * Displays the user's RC balance (available, engaged, total),
...
export function RCWallet() {
  const t = useTranslations("rcWallet");
  ...
  } = useRCHistory(historyParams);

  const { mutate: refundPurchase } = useRefundRCPurchase();

  const available = balanceData?.rcBalance ?? 0;
  const engaged = balanceData?.engagedRC ?? 0;
  ...
  return (
    <div className={spacing.stack}>
      {/* ── Balance stat cards ─────────────────────────────────────── */}
      <div className="grid grid-cols-1 sm:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-3 gap-4">
      ...
      <Card className={`p-4 ${flex.between} flex-wrap gap-3`}>
        ...
        <Button
          variant="primary"
          size="md"
          onClick={() => router.push(ROUTES.USER.RC_PURCHASE as never)}
        >
          {t("buyButton")}
        </Button>
      </Card>
