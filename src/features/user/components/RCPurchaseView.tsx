 ...
 * RCPurchaseView
 *
 * Full-page view for purchasing RC.
...
import { usePurchaseRC, useVerifyRCPurchase, useRazorpay } from "@/hooks";
...
export function RCPurchaseView() {
  const t = useTranslations("rcWallet.buy");
  const router = useRouter();
  const [selectedPackageId, setSelectedPackageId] = useState<string>(
    RC_PACKAGES[2].packageId, // default: 1000 RC
  );
  const [buying, setBuying] = useState(false);

  const { mutateAsync: purchaseCoins } = usePurchaseRC();
  const { mutateAsync: verifyPurchase } = useVerifyRCPurchase();
  const { openRazorpay, isLoading: isRazorpayLoading } = useRazorpay();

  const selectedPkg =
    RC_PACKAGES.find((p) => p.packageId === selectedPackageId) ??
    RC_PACKAGES[2];
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const payment = await openRazorpay({
        ...
        name: "LetItRip",
        description: `${order.totalCoins} RC`,
      ...
      router.push(ROUTES.USER.RC as never);
    } catch {
    ...
    <div className={`${spacing.stack} max-w-2xl mx-auto`}>
      ...
      <Card className="p-4">
        ...
        <Button
          ...
          onClick={() => router.push(ROUTES.USER.RC as never)}
          disabled={isProcessing}
        >
          {t("cancel")}
        </Button>
