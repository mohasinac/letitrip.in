...
import { ArenaConfig, LoopConfig, ExitConfig, ObstacleConfig } from '@/types/arenaConfig';
...
export function renderArenaFloor(
  ...
  switch (arena.shape) {
    ...
    case 'hexagon':
      renderPolygon(ctx, center.x, center.y, innerRadius, 6, arena.rotation || 0);
      ctx.fill();
      break;
      
    case 'octagon':
      renderPolygon(ctx, center.x, center.y, innerRadius, 8, arena.rotation || 0);
      ctx.fill();
      break;
      
    case 'pentagon':
      renderPolygon(ctx, center.x, center.y, innerRadius, 5, arena.rotation || 0);
      ctx.fill();
      break;
...
export function renderWallsAndExits(
  ...
  if (!wall.enabled) {
    ...
    if (wall.allExits) {
      ...
      ctx.beginPath();
      ctx.arc(center.x, center.y, (innerRadius + outerRadius) / 2, 0, Math.PI * 2);
      ctx.stroke();
    }
    return;
  }
  ...
  for (let i = 0; i < sortedExits.length; i++) {
    ...
    const wallStartAngle = ((currentExit.angle + currentExit.width / 2) * Math.PI) / 180;
    ...
    ctx.arc(center.x, center.y, innerRadius, wallEndAngle, wallStartAngle, true);
    ctx.closePath();
    ctx.fill();
    
    // Wall details (spikes or springs)
    if (wall.hasSpikes) {
      renderSpikes(ctx, center, innerRadius, outerRadius, wallStartAngle, wallEndAngle);
    }
  }
}
...
export function renderLoops(
  ...
  arena.loops.forEach(loop => {
    ...
    switch (loop.shape) {
      ...
      case 'hexagon':
        renderPolygon(ctx, center.x, center.y, loopRadius, 6, loop.rotation || 0, true);
        break;
        
      case 'octagon':
        renderPolygon(ctx, center.x, center.y, loopRadius, 8, loop.rotation || 0, true);
        break;
    }
...
export function renderObstacles(
  ...
  arena.obstacles.forEach(obstacle => {
    ...
    switch (obstacle.type) {
      case 'rock': color = '#8B7355'; break;
      case 'pillar': color = '#A0A0A0'; break;
      case 'barrier': color = '#FF6B6B'; break;
