...
function getCollisionAngle(
  ...
  const collisionAngle =
    Math.atan2(toCollision.y, toCollision.x) * (180 / Math.PI);
...
function calculateSpinSteal(
  ...
  const oppositeRotation =
    attacker.config.direction !== defender.config.direction;
  const spinStealMultiplier = oppositeRotation ? 1.5 : 0.5;

  // Base spin steal calculation
  const baseSteal =
    collisionForce * attackerStats.spinStealFactor * spinStealMultiplier * 0.1;

  // Check if attacker has active spin steal special move
  const hasSpinStealBoost =
    attacker.ultimateAttackActive &&
...
export function resolveEnhancedCollision(
  ...
  const bey1CollisionAngle = getCollisionAngle(
    bey1.position,
    bey1.rotation,
    collisionPoint,
  );
  const bey2CollisionAngle = getCollisionAngle(
    bey2.position,
    bey2.rotation,
    collisionPoint,
  );
  ...
  const bey1SpinSteal = calculateSpinSteal(
    bey1,
    bey1Stats,
    bey2,
    bey2Stats,
  ...
  const bey2SpinSteal = calculateSpinSteal(
    bey2,
    bey2Stats,
    bey1,
    bey1Stats,
  ...
  const overlap =
    bey1.radius +
    bey2.radius -
    vectorLength(vectorSubtract(bey1.position, bey2.position));
  if (overlap > 0) {
 ...
 */
export function getContactHitQuality(
  multiplier: number,
): "perfect" | "good" | "normal" {
