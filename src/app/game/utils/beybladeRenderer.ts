 ...
 */
export async function preloadBeybladeImages(stats: BeybladeStats[]): Promise<void> {
  const promises = stats
    .filter(stat => stat.imageUrl)
    .map(stat => loadBeybladeImage(stat.imageUrl!).catch(err => {
...
export function drawBeybladeWithGlow(
  ...
  const gradient = ctx.createRadialGradient(
    ...
    position.x, position.y, radius * (1.5 + glowIntensity * 0.5)
  );
  ...
  ctx.fillStyle = gradient;
  ctx.beginPath();
  ctx.arc(position.x, position.y, radius * (1.5 + glowIntensity * 0.5), 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();
...
export function drawSpinTrail(
  ...
  const speed = Math.sqrt(
    beyblade.velocity.x ** 2 + beyblade.velocity.y ** 2
  );
