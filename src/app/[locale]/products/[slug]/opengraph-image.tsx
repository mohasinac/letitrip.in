...
import { getProductForDetail, renderProductOg, safeRead } from "@mohasinac/appkit/server";
...
export default async function Image({ params }: Props) {
  ...
  const doc = await safeRead(() => getProductForDetail(slug), {
    route: "/products/[slug]",
    key: "products.getProductForDetail",
    fallback: null,
  });
  return new ImageResponse(
    renderProductOg(doc, { siteName: SEO_CONFIG.siteName ?? "LetItRip", baseUrl: SEO_CONFIG.siteUrl }),
    { ...size },
  );
}
