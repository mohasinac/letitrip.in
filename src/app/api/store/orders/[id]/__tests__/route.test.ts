...
describe("GET /api/store/orders/[id]", () => {
  it("unauthenticated → 401", async () => {
    _user = null;
    const res = await GET(makeGetReq("order-abc") as never, { params: Promise.resolve({ id: "order-abc" }) } as never);
    expect(res.status).toBe(401);
  });

  it("order not found → 404", async () => {
    mockOrderFindById.mockResolvedValue(null);
    const res = await GET(makeGetReq("order-missing") as never, { params: Promise.resolve({ id: "order-missing" }) } as never);
  ...
  it("seller tries to see other store's order → 404 (not 403)", async () => {
    ...
    const res = await GET(makeGetReq("order-xyz") as never, { params: Promise.resolve({ id: "order-xyz" }) } as never);
    expect(res.status).toBe(404);
  });
...
describe("PATCH /api/store/orders/[id]", () => {
  it("unauthenticated → 401", async () => {
    _user = null;
    const res = await PATCH(makePatchReq("order-abc", { status: "processing" }) as never, { params: Promise.resolve({ id: "order-abc" }) } as never);
    expect(res.status).toBe(401);
  });

  it("order not found → 404", async () => {
    mockOrderFindById.mockResolvedValue(null);
    const res = await PATCH(makePatchReq("order-missing", { status: "processing" }) as never, { params: Promise.resolve({ id: "order-missing" }) } as never);
    expect(res.status).toBe(404);
  });

  it("seller updates status=processing → calls updateStatus", async () => {
    await PATCH(makePatchReq("order-abc", { status: "processing" }) as never, { params: Promise.resolve({ id: "order-abc" }) } as never);
  ...
  it("seller updates status=shipped (manual tracking) → calls updateStatus", async () => {
    await PATCH(makePatchReq("order-abc", { status: "shipped", trackingNumber: "TRK123", shippingCarrier: "BlueDart" }) as never, { params: Promise.resolve({ id: "order-abc" }) } as never);
  ...
  it("seller tries to set status=delivered → 403", async () => {
    const res = await PATCH(makePatchReq("order-abc", { status: "delivered" }) as never, { params: Promise.resolve({ id: "order-abc" }) } as never);
  ...
  it("seller tries to cancel order via PATCH → 403 (not allowed for seller)", async () => {
    const res = await PATCH(makePatchReq("order-abc", { status: "cancelled" }) as never, { params: Promise.resolve({ id: "order-abc" }) } as never);
  ...
  it("admin can set status=delivered → calls updateStatus", async () => {
    _user = { uid: "admin-uid", role: "admin" };
    await PATCH(makePatchReq("order-abc", { status: "delivered" }) as never, { params: Promise.resolve({ id: "order-abc" }) } as never);
  ...
  it("admin cancels order → calls cancelOrder with reason", async () => {
    _user = { uid: "admin-uid", role: "admin" };
    await PATCH(makePatchReq("order-abc", { status: "cancelled", cancellationReason: "Out of stock" }) as never, { params: Promise.resolve({ id: "order-abc" }) } as never);
  ...
  it("admin cancels without reason → uses default reason", async () => {
    _user = { uid: "admin-uid", role: "admin" };
    await PATCH(makePatchReq("order-abc", { status: "cancelled" }) as never, { params: Promise.resolve({ id: "order-abc" }) } as never);
  ...
  it("markPicked → calls orderRepository.markPicked", async () => {
    await PATCH(makePatchReq("order-abc", { markPicked: true }) as never, { params: Promise.resolve({ id: "order-abc" }) } as never);
  ...
  it("markPacked → calls orderRepository.markPacked", async () => {
    await PATCH(makePatchReq("order-abc", { markPacked: true }) as never, { params: Promise.resolve({ id: "order-abc" }) } as never);
    expect(mockOrderMarkPacked).toHaveBeenCalledWith("order-abc");
  });

  it("assignedWorkerId → calls orderRepository.assignWorker", async () => {
    await PATCH(makePatchReq("order-abc", { assignedWorkerId: "worker-123" }) as never, { params: Promise.resolve({ id: "order-abc" }) } as never);
  ...
  it("seller's other store order → 404 via scope check", async () => {
    mockOrderFindById.mockResolvedValue(mockOrderOtherStore);
    const res = await PATCH(makePatchReq("order-xyz", { status: "processing" }) as never, { params: Promise.resolve({ id: "order-xyz" }) } as never);
    expect(res.status).toBe(404);
  });
});
