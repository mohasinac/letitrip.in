...
const LISTING_TYPE_CLAUSE = sieveFilter(PRODUCT_FIELDS.LISTING_TYPE, SIEVE_OP.EQ, "digital-code");

const SAFE_DIGITAL_CODE_FILTER_FIELDS = new Set([
...
function mergeListingTypeFilter(filters: string | null | undefined): string {
  const safe = filters ? validateSieveFilters(filters, SAFE_DIGITAL_CODE_FILTER_FIELDS) : "";
  const parts = safe
    .split(",")
    .map((c) => c.trim())
    .filter(Boolean)
...
export const GET = withProviders(async (request: Request) => {
  try {
    ...
    logError("digital-codes", "GET /api/digital-codes failed", error);
    return NextResponse.json(
      {
        success: false,
