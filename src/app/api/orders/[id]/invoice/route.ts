...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withRateLimit(req, async (r) => {
    try {
      ...
      const stream = doc.on('data', (chunk: Uint8Array) => chunks.push(chunk));
      ...
      doc.fontSize(12)
        ...
        .text(billing.line1 || '')
        .text(billing.line2 || '')
        .text(`${billing.city || ''} ${billing.state || ''} ${billing.pincode || ''}`)
      ...
      if (items.length === 0) {
        ...
        items.forEach((it: any, idx: number) => {
          ...
          doc.text(`${idx + 1}. ${it.name || it.product_name || 'Item'}  x${qty}  -  ₹${lineTotal.toFixed(2)}`);
        });
      }
      ...
      doc.moveDown();
      doc.fontSize(10).fillColor('gray').text('This is a system generated invoice and does not require a signature.');
