...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (order.status === 'shipped' || order.status === 'delivered') {
      return NextResponse.json({ success: false, error: 'Order cannot be canceled' }, { status: 400 });
    }

    const { reason } = await req.json();
    await ref.update({ status: 'canceled', cancel_reason: reason || '', canceled_at: new Date().toISOString(), updated_at: new Date().toISOString() });
