// Public listing endpoint for live animals & plants â€” mirrors /api/products?listingType=live
...
const LISTING_TYPE_CLAUSE = sieveFilter(PRODUCT_FIELDS.LISTING_TYPE, SIEVE_OP.EQ, "live");

const SAFE_LIVE_FILTER_FIELDS = new Set([
...
function mergeListingTypeFilter(filters: string | null | undefined): string {
  const safe = filters ? validateSieveFilters(filters, SAFE_LIVE_FILTER_FIELDS) : "";
  const parts = safe
    .split(",")
    .map((c) => c.trim())
    .filter(Boolean)
...
export const GET = withProviders(async (request: Request) => {
  try {
    ...
    logError("live", "GET /api/live failed", error);
    return NextResponse.json(
      {
        success: false,
