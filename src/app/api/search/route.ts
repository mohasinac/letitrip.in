...
async function resolveCategorySlug(categorySlug?: string | null): Promise<string | null> {
  if (!categorySlug) return null;
  const snap = await Collections.categories().where('slug', '==', categorySlug).limit(1).get();
...
export async function GET(req: NextRequest) {
  return withRedisRateLimit(req, async (request: NextRequest) => {
    try {
      ...
      const inStock = (searchParams.get('in_stock') || '').toLowerCase() === 'true';
      const sort = (searchParams.get('sort') || (type === 'auctions' ? 'endingSoon' : 'latest')).toLowerCase();

      // Resolve optional slugs
      let shopId: string | null = null;
      if (shopSlug) {
        const shopSnap = await Collections.shops().where('slug', '==', shopSlug).limit(1).get();
        if (!shopSnap.empty) shopId = shopSnap.docs[0].id; else return NextResponse.json({ success: false, error: 'Shop not found' }, { status: 404 });
      }
      ...
      // Common filters
      if (type === 'products') {
        query = query.where('is_deleted', '==', false).where('status', '==', 'published');
      ...
      if (!needsRelevance) {
        if (type === 'products') {
          ...
          if (sort === 'endingsoon' || sort === 'ending-soon') query = query.orderBy('end_time', 'asc');
          else if (sort === 'price-asc') query = query.orderBy('current_bid', 'asc');
          else if (sort === 'price-desc') query = query.orderBy('current_bid', 'desc');
      ...
      if (!needsRelevance) {
        if (type === 'products') {
          if (sort === 'latest') items.sort((a: any, b: any) => (itDate(b.created_at) || itDate(b.createdAt) || 0) - (itDate(a.created_at) || itDate(a.createdAt) || 0));
          else if (sort === 'price-asc') items.sort((a: any, b: any) => (a.price || 0) - (b.price || 0));
          else if (sort === 'price-desc') items.sort((a: any, b: any) => (b.price || 0) - (a.price || 0));
        } else {
          if (sort === 'endingsoon' || sort === 'ending-soon') items.sort((a: any, b: any) => (itDate(a.end_time) || 0) - (itDate(b.end_time) || 0));
          else if (sort === 'price-asc') items.sort((a: any, b: any) => (a.current_bid || 0) - (b.current_bid || 0));
          else if (sort === 'price-desc') items.sort((a: any, b: any) => (b.current_bid || 0) - (a.current_bid || 0));
        }
      }
      ...
      return NextResponse.json({ success: true, type, data: paged, pagination: { page, limit, total, hasMore: start + limit < total } });
    } catch (error) {
