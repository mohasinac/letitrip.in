 ...
 *
 * Initiates an RC purchase via Razorpay.
...
import { VALID_PACKAGE_IDS, getRCPackage } from "@/db/schema";
import { z } from "zod";

const purchaseSchema = z.object({
  packageId: z.string().refine((v) => VALID_PACKAGE_IDS.includes(v), {
    message: ERROR_MESSAGES.RC.INVALID_PACKAGE,
  }),
});
...
export const POST = createApiHandler<(typeof purchaseSchema)["_output"]>({
  ...
  handler: async ({ user, body }) => {
    const pkg = getRCPackage(body!.packageId);
    if (!pkg) {
    ...
    serverLogger.info("RC purchase initiated", {
      uid: user!.uid,
      packageId: pkg.packageId,
    ...
    return successResponse(
      ...
      SUCCESS_MESSAGES.RC.PURCHASE_INITIATED,
    );
  },
});
