...
export async function POST(request: NextRequest) {
  try {
    ...
    const heroSlideData = [
      { title: "Beyblade X Launch Sale", subtitle: "Up to 50% off on the latest X series", cta: "Shop Now", ctaLink: "/categories/beyblade-x" },
      { title: "Rare Vintage Collection", subtitle: "Original series Beyblades in stock", cta: "Explore", ctaLink: "/categories/vintage-beyblades" },
      { title: "Tournament Ready Gear", subtitle: "Pro launchers and stadiums", cta: "Shop Gear", ctaLink: "/categories/launchers-gear" },
      { title: "Metal Fight Masters", subtitle: "Galaxy Pegasus & L-Drago available", cta: "View Collection", ctaLink: "/categories/metal-fight" },
      { title: "Gold Edition Release", subtitle: "Limited gold plated Beyblades", cta: "Grab Now", ctaLink: "/categories/gold-series" },
      { title: "Flash Sale - 24 Hours", subtitle: "Attack types at lowest prices", cta: "Shop Sale", ctaLink: "/categories/attack-types" },
      { title: "Collector's Paradise", subtitle: "Sealed boxes and rare finds", cta: "Browse", ctaLink: "/categories/collector-items" },
      { title: "Free Shipping Week", subtitle: "On orders above ₹2000", cta: "Shop Now", ctaLink: "/products" },
      { title: "Live Auctions", subtitle: "Bid on rare collectible Beyblades", cta: "Join Auction", ctaLink: "/auctions" },
      { title: "Blader Membership", subtitle: "Exclusive deals for members", cta: "Join Now", ctaLink: "/membership" },
    ];
    ...
    for (let i = 0; i < sectionTypes.length; i++) {
      await db.collection(COLLECTIONS.FEATURED_SECTIONS).doc().set({
    ...
    if (shops) {
      for (let i = 0; i < Math.min(10, shops.length); i++) {
        await db.collection(COLLECTIONS.SHOPS).doc(shops[i].id).update({
          featured: true,
          featured_order: i + 1,
          featured_badge: ["Pro Blader", "Authentic Seller", "Top Rated", "Fast Shipping", "Verified Collector"][i % 5],
          featured_until: new Date(timestamp.getTime() + 30 * 24 * 60 * 60 * 1000),
        });
    ...
    const couponData = [
      { code: "LETITBEY10", discount: 10, type: "percentage", minOrder: 500, maxDiscount: 200 },
      { code: "BURST100", discount: 100, type: "fixed", minOrder: 1000, maxDiscount: 100 },
      { code: "FIRSTSPIN20", discount: 20, type: "percentage", minOrder: 800, maxDiscount: 500 },
      { code: "FREESHIP", discount: 0, type: "free_shipping", minOrder: 1500, maxDiscount: 100 },
      { code: "GOLDRUSH30", discount: 30, type: "percentage", minOrder: 2000, maxDiscount: 1000 },
    ];

    for (const coupon of couponData) {
      await db.collection(COLLECTIONS.COUPONS).doc().set({
    ...
    if (users) {
      const notificationTypes = [
        { type: "order_shipped", title: "Your order is on the way!", icon: "truck" },
        { type: "price_drop", title: "Price dropped on your wishlist item!", icon: "tag" },
        { type: "auction_ending", title: "Auction ending soon!", icon: "clock" },
        { type: "new_message", title: "You have a new message", icon: "message" },
      ...
      for (const user of users.slice(0, 50)) {
        const notif = notificationTypes[users.indexOf(user) % notificationTypes.length];
    ...
    if (buyers) {
      const ticketCategories = [
        { category: "order_issue", subject: "Beyblade order not received", priority: "high" },
        { category: "payment", subject: "Payment failed for launcher purchase", priority: "high" },
        { category: "shipping", subject: "Wrong tracking for stadium order", priority: "medium" },
        { category: "product_inquiry", subject: "Is this Beyblade authentic Takara Tomy?", priority: "low" },
        { category: "return_request", subject: "Want to return damaged Beyblade", priority: "medium" },
        { category: "refund", subject: "Refund not processed for cancelled order", priority: "high" },
        { category: "account", subject: "Cannot login to my blader account", priority: "medium" },
        { category: "technical", subject: "Product images not loading", priority: "low" },
      ];
      ...
      for (let t = 0; t < 40; t++) {
        ...
        await db.collection(COLLECTIONS.SUPPORT_TICKETS).doc().set({
          ...
          status: ["open", "in_progress", "waiting_on_customer", "resolved", "closed"][t % 5],
          attachments: hasAttachment ? [
            { type: "image", url: TICKET_ATTACHMENTS[t % TICKET_ATTACHMENTS.length], name: "screenshot.jpg" },
          ] : [],
          videos: hasVideo ? [
            { url: TICKET_VIDEOS[t % TICKET_VIDEOS.length], name: "screen_recording.mp4" },
          ] : [],
          assignedTo: t % 3 === 0 ? "support_agent_1" : null,
          tags: [ticketData.category, ticketData.priority],
          responseTime: t % 2 === 0 ? Math.floor(Math.random() * 24) + 1 : null, // hours
          satisfaction: t % 5 === 4 ? Math.floor(Math.random() * 2) + 4 : null, // 4-5 stars
          createdAt: new Date(timestamp.getTime() - Math.floor(Math.random() * 7) * 24 * 60 * 60 * 1000),
          updatedAt: timestamp,
        });
        counts.tickets++;
      }
    ...
    if (buyers && shops) {
      for (let c = 0; c < 20; c++) {
        ...
        const messages = [
          ...
          { sender: shop.ownerId, text: "Yes, it's available! Would you like to purchase?" },
          { sender: buyer.id, text: "What's the best price you can offer?" },
          { sender: shop.ownerId, text: "I can offer 10% discount on this item." },
          { sender: buyer.id, text: "Great, I'll place the order now!" },
          { sender: shop.ownerId, text: "Thanks for your query!" },
        ];

        for (let m = 0; m < messages.length; m++) {
          await db.collection(COLLECTIONS.MESSAGES).doc().set({
            conversationId,
            senderId: messages[m].sender,
            text: messages[m].text,
            type: "text",
            isRead: m < messages.length - 1,
            createdAt: new Date(timestamp.getTime() - (messages.length - m) * 60 * 1000),
          });
          counts.messages++;
        }
      }
