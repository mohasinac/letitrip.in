...
const STREETS = ["Marine Drive", "MG Road", "Park Street", "Anna Salai", "FC Road", "Brigade Road", "Linking Road"];
  ...
  try {
    ...
    for (const buyer of buyers) {
      ...
      for (let o = 0; o < numOrders; o++) {
        ...
        const status = ["pending", "confirmed", "processing", "shipped", "out_for_delivery", "delivered"][Math.floor(Math.random() * 6)];
        const paymentMethodData = RAZORPAY_METHODS[Math.floor(Math.random() * RAZORPAY_METHODS.length)];
        const city = INDIAN_CITIES[Math.floor(Math.random() * INDIAN_CITIES.length)];
        const courier = COURIER_COMPANIES[Math.floor(Math.random() * COURIER_COMPANIES.length)];
        ...
        await orderRef.set({
          ...
          paymentStatus: paymentMethodData.method === "cod" ? "pending" : "completed",
          shippingAddress: {
            name: buyer.name,
          ...
          notes: Math.random() > 0.7 ? "Please handle with care - fragile items" : "",
          giftWrap: Math.random() > 0.8,
          estimatedDelivery: new Date(timestamp.getTime() + (3 + Math.floor(Math.random() * 5)) * 24 * 60 * 60 * 1000),
          createdAt: timestamp,
          updatedAt: timestamp,
        });
        ...
        if (["shipped", "out_for_delivery", "delivered"].includes(status)) {
          ...
          const trackingEvents = [
            { status: "pickup_scheduled", location: city.city, timestamp: new Date(timestamp.getTime() - 4 * 24 * 60 * 60 * 1000) },
            { status: "picked_up", location: city.city, timestamp: new Date(timestamp.getTime() - 3 * 24 * 60 * 60 * 1000) },
            { status: "in_transit", location: "Sorting Hub", timestamp: new Date(timestamp.getTime() - 2 * 24 * 60 * 60 * 1000) },
          ];
          
          if (status === "out_for_delivery" || status === "delivered") {
            trackingEvents.push({ status: "out_for_delivery", location: city.city, timestamp: new Date(timestamp.getTime() - 1 * 24 * 60 * 60 * 1000) });
          }
          if (status === "delivered") {
            trackingEvents.push({ status: "delivered", location: city.city, timestamp });
          }
          ...
          await db.collection(COLLECTIONS.SHIPMENTS).doc().set({
            ...
            pickup_scheduled_date: new Date(timestamp.getTime() - 4 * 24 * 60 * 60 * 1000),
            pickup_date: new Date(timestamp.getTime() - 3 * 24 * 60 * 60 * 1000),
            delivered_date: status === "delivered" ? timestamp : null,
            pod_image: status === "delivered" ? `https://images.unsplash.com/photo-1566576912321-d58ddd7a6088?w=400&h=300&fit=crop` : null,
