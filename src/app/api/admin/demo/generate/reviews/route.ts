  ...
  try {
    ...
    if (!products || !Array.isArray(products) || products.length === 0) {
      return NextResponse.json(
        { success: false, error: "Products data required" },
        { status: 400 },
      );
    }
    ...
    for (const productId of products) {
      ...
      const numReviews = Math.max(
        1,
        Math.min(
          Math.floor((scale / 10) * (1 + Math.floor(Math.random() * 1))),
          2,
        ),
      );
      ...
      for (let r = 0; r < numReviews; r++) {
        ...
        const reviewImages = hasImages
          ? Array.from(
              { length: numImages },
              (_, idx) =>
                REVIEW_IMAGES[(totalReviews + idx) % REVIEW_IMAGES.length],
            )
          : [];
        const reviewVideo = hasVideo
          ? REVIEW_VIDEOS[totalReviews % REVIEW_VIDEOS.length]
          : null;
    ...
    const message =
      error instanceof Error ? error.message : "Failed to generate reviews";
    return NextResponse.json(
