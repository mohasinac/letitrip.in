...
export async function POST() {
  try {
    ...
    // 2. SEO Settings
    await db.collection("site_settings").doc("seo").set({
      defaultTitle: "LET IT RIP - Buy Beyblades, TCG Cards & Collectibles in India",
      defaultDescription: "India's most trusted platform for authentic Beyblades, Trading Card Games, and collectibles. Shop, auction, and trade with verified sellers.",
    ...
    await db.collection("site_settings").doc("maintenance").set({
      enabled: false,
      message: "We're upgrading our platform to serve you better! We'll be back soon.",
    ...
    await db.collection("payment_settings").doc("razorpay").set({
      enabled: true,
    ...
    await db.collection("payment_settings").doc("cod").set({
      enabled: true,
    ...
    const shippingZones = [
      ...
      },
      {
        ...
        pincodeRanges: ["560001-560100", "600001-600100", "700001-700100", "500001-500100"],
        baseRate: 80,
    ...
    for (const zone of shippingZones) {
      await db.collection("shipping_zones").doc().set({
        ...zone,
        isActive: true,
        weightSlabs: [
    ...
    const carriers = [
      ...
      },
      {
        name: "DTDC",
        code: "DTDC",
        trackingUrl: "https://www.dtdc.in/tracking.asp?trackingNumber={trackingNumber}",
        apiEnabled: true,
      },
      {
        name: "India Post",
        code: "INDIAPOST",
        trackingUrl: "https://www.indiapost.gov.in/_layouts/15/dop.portal.tracking/trackconsignment.aspx?cno={trackingNumber}",
        apiEnabled: false,
      },
    ];

    for (const carrier of carriers) {
      await db.collection("shipping_carriers").doc(carrier.code).set({
        ...carrier,
        isActive: true,
        createdAt: timestamp,
        updatedAt: timestamp,
    ...
    await db.collection("notification_settings").doc("push").set({
      enabled: true,
      provider: "firebase",
      firebaseConfig: {
    ...
    await db.collection("notification_settings").doc("in_app").set({
      enabled: true,
      retentionDays: 30,
      notificationTypes: {
    ...
    await db.collection("notification_settings").doc("sms").set({
      enabled: false,
      provider: "twilio",
      credentials: {
      ...
      templates: {
        order_confirmation: "Your order {{orderNumber}} has been confirmed. Track: {{link}}",
        order_shipped: "Your order {{orderNumber}} is shipped! Track: {{trackingNumber}}",
    ...
    await db.collection("homepage_settings").doc("current").set({
      ...
      sections: {
        ...
        categories: {
          ...
          subtitle: "Browse our extensive collection of Beyblades and collectibles",
        },
        products: {
          enabled: true,
          order: 3,
    ...
    const message = error instanceof Error ? error.message : "Failed to generate settings";
