...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const q1 = await Collections.auctions().where('category_id', '==', auction.category_id).limit(50).get();
    ...
    return NextResponse.json({ success: false, error: 'Failed to load similar auctions' }, { status: 500 });
  }
}
