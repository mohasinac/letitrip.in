...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (doc.exists) {
      ...
      await Collections.favorites().doc(key).set({ user_id: user.id, auction_id: id, created_at: new Date().toISOString(), type: 'auction_watch' });
      return NextResponse.json({ success: true, watching: true });
    }
  } catch (error) {
    console.error('Toggle watch error:', error);
    return NextResponse.json({ success: false, error: 'Failed to toggle watch' }, { status: 500 });
  }
}
