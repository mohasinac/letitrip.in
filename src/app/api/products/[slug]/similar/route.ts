...
export async function GET(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const q1 = await Collections.products().where('category_id', '==', prod.category_id).limit(50).get();
    ...
    if (results.length < limit && prod.category_parent_id) {
      const q2 = await Collections.products().where('category_parent_id', '==', prod.category_parent_id).limit(50).get();
