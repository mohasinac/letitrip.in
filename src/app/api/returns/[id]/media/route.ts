...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withRateLimit(req, async (r) => {
    try {
      ...
      return NextResponse.json({ success: false, error: 'Failed to create signed URLs' }, { status: 500 });
    }
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withRateLimit(req, async (r) => {
    try {
      ...
      if (!paths.length) return NextResponse.json({ success: false, error: 'No paths provided' }, { status: 400 });
      if (paths.length > MAX_FILES_PER_CONFIRM) {
        return NextResponse.json({ success: false, error: 'Too many files in single confirm' }, { status: 400 });
      }
      ...
      if (!valid.length) return NextResponse.json({ success: false, error: 'No valid file paths' }, { status: 400 });

      // Convert to public URLs (assuming GCS bucket public or served via CDN)
      const storage = getStorageAdmin();
      const bucket = storage.bucket();
      const urls = valid.map(v => `https://storage.googleapis.com/${bucket.name}/${v}`);

      await retRef.update({ media: [...(ret.media || []), ...urls], updated_at: new Date().toISOString() });
      return NextResponse.json({ success: true, data: { urls } });
    } catch (error) {
      console.error('Return media confirm error:', error);
      return NextResponse.json({ success: false, error: 'Failed to confirm media' }, { status: 500 });
    }
