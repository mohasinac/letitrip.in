...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, data: { id: doc.id, ...doc.data() } }, { status: 201 });
  } catch (error) {
    console.error('Return create error:', error);
    return NextResponse.json({ success: false, error: 'Failed to initiate return' }, { status: 500 });
  }
}
