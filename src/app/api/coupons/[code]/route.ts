...
export async function GET(request: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  try {
    ...
    const snapshot = await Collections.coupons().where('code', '==', code).limit(1).get();
    if (snapshot.empty) {
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  try {
    ...
    delete update.id; delete update.shop_id; delete update.code; delete update.created_at;
