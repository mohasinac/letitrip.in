...
export function activateBarrageOfAttacks(
  ...
  const state: CinematicMoveState = {
    ...
    attackerLosesControl:
      attackerStats.specialMove.flags.userLosesControl || true,
    defenderLosesControl:
      attackerStats.specialMove.flags.opponentLosesControl || true,

    startTime: currentTime,
    endTime:
      currentTime + (attackerStats.specialMove.flags.duration || 4) * 1000,
  };
...
export function activateTimeSkip(
  ...
  const pushDistance =
    timeSkipConfig?.repositionOpponent?.distance || defender.radius * 4;
  ...
  const state: CinematicMoveState = {
    ...
    attackerLosesControl:
      attackerStats.specialMove.flags.userLosesControl || false,
    defenderLosesControl:
      attackerStats.specialMove.flags.freezeOpponent || true,
...
export function updateCinematicMoves(
  ...
  for (const [beybladeId, state] of activeCinematicMoves.entries()) {
    ...
    const defender = state.targetId
      ? beyblades.find((b) => b.id === state.targetId)
      : null;
...
function updateBarrageOfAttacks(
  ...
  if (!defender || !state.orbitCenter || state.orbitRadius === undefined)
    return;
  ...
  if (
    state.nextAttackAngle !== undefined &&
  ...
  attacker.position.x =
    state.orbitCenter.x + Math.cos(angleRad) * state.orbitRadius;
  attacker.position.y =
    state.orbitCenter.y + Math.sin(angleRad) * state.orbitRadius;
  ...
  if (distFromCenter > stadium.innerRadius) {
    ...
    attacker.position.x =
      stadium.center.x + Math.cos(angle) * stadium.innerRadius;
    attacker.position.y =
      stadium.center.y + Math.sin(angle) * stadium.innerRadius;
  }
}
...
function updateTimeSkip(
  ...
  attacker.position.x =
    stadium.center.x + Math.cos(angleRad) * state.targetLoopRadius;
  attacker.position.y =
    stadium.center.y + Math.sin(angleRad) * state.targetLoopRadius;
 ...
 */
export function shouldLoseControl(beybladeId: string): {
  loses: boolean;
  reason?: string;
} {
  ...
  if (state.phase === "execution") {
    if (state.beybladeId === beybladeId) {
      return {
        ...
        reason: state.attackerLosesControl
    ...
    if (state.targetId === beybladeId) {
      return {
        ...
        reason: state.defenderLosesControl
          ? "Under special move effect..."
          : undefined,
      };
    }
  }
 ...
 */
export function getActiveCinematicMove(
  beybladeId: string,
): CinematicMoveState | null {
