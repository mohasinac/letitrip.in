...
export const updateBeyblade = (
  ...
  const enhancedVelocity = vectorMultiply(
    beyblade.velocity,
  ...
  beyblade.rotation +=
    (beyblade.config.direction === "right" ? rotationSpeed : -rotationSpeed) *
    deltaTime;
...
export const launchPowerAttack = (
  beyblade: GameBeyblade,
  direction: Vector2D,
): void => {
...
export const checkStadiumBounds = (
  beyblade: GameBeyblade,
  stadium: Stadium,
): void => {
  const distanceFromCenter = vectorLength(
    vectorSubtract(beyblade.position, stadium.center),
  );
