...
export default function CheckoutPage() {
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      // Use apiClient - automatically sends session cookies
      ...
      setCouponDiscount(response.discount.amount);
      toast.success(
        `Coupon applied! You saved ${formatPrice(response.discount.amount)}`
      );
    } catch (error: any) {
  ...
  const handleRazorpayPayment = async () => {
    try {
      ...
      // Create Razorpay order - apiClient sends session cookies automatically
      ...
      const options = {
        ...
        handler: async (response: any) => {
          try {
            ...
            // Create order - apiClient sends session cookies automatically
            const orderCreateData = await apiClient.post("/api/orders/create", {
            ...
            // Then verify payment - apiClient sends session cookies automatically
  ...
  const handleCODOrder = async () => {
    try {
      ...
      // Create order - apiClient sends session cookies automatically
      const orderData = await apiClient.post("/api/orders/create", {
        items: items.map((item) => ({
          id: item.id,
          productId: item.productId,
          name: item.name,
