...
export function useCamera(): UseCameraReturn {
  ...
  const takePhoto = useCallback((): Blob | null => {
    ...
    // canvas.toBlob is async but we return synchronously via toDataURL fallback
    // Production use: callers should prefer the async version; this sync version
    // works for the immediate capture-preview-upload pattern.
    canvas.toBlob(
      (blob) => {
        result = blob;
      },
    ...
    // toBlob fires synchronously in all modern browsers when the canvas is
    // already populated — the callback runs before the next microtask.
    return result;
  }, [isActive]);
  ...
  const stopRecording = useCallback((): Promise<Blob> => {
    return new Promise((resolve) => {
      ...
      if (
        ...
        // Nothing was recording — resolve immediately with empty blob
