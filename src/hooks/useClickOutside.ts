...
export function useClickOutside<T extends HTMLElement = HTMLElement>(
  ...
  const { enabled = true, eventType = 'mousedown', additionalRefs = [] } = options;
    ...
    const handleClickOutside = (event: MouseEvent | TouchEvent) => {
      ...
      if (ref.current && !ref.current.contains(event.target as Node)) {
        ...
        const isOutsideAll = additionalRefs.every(
          (additionalRef) =>
            !additionalRef.current || !additionalRef.current.contains(event.target as Node)
        );
  ...
  }, [ref, callback, enabled, eventType, additionalRefs]);
}
