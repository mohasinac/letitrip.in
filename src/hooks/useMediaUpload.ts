...
export function useMediaUpload(options: MediaUploadOptions = {}) {
  ...
  const { addUpload, updateUpload, removeUpload, retryUpload } = useUploadContext();
  ...
  const retry = useCallback(async (): Promise<string | null> => {
    ...
    const uploadFile = document.querySelector<HTMLInputElement>('input[type="file"]')?.files?.[0];
    if (!uploadFile) {
