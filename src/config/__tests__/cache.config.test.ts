import { DEFAULT_CACHE_CONFIG, CacheConfigEntry } from '../cache.config';
...
describe('Cache Config', () => {
  ...
  describe('Products cache config', () => {
    ...
    it('should have stale time longer than TTL', () => {
      expect(productsCache.staleWhileRevalidate!).toBeGreaterThan(productsCache.ttl);
    });
  });
  ...
  describe('Auctions cache config', () => {
    ...
    it('should have shorter TTL than products', () => {
      expect(auctionsCache.ttl).toBeLessThan(DEFAULT_CACHE_CONFIG['/products'].ttl);
    });
  ...
  describe('Categories cache config', () => {
    ...
    it('should have longer TTL than most endpoints', () => {
      expect(categoriesCache.ttl).toBeGreaterThan(DEFAULT_CACHE_CONFIG['/products'].ttl);
      expect(categoriesCache.ttl).toBeGreaterThan(DEFAULT_CACHE_CONFIG['/auctions'].ttl);
    });
  ...
  describe('User-specific endpoints', () => {
    it('should have short TTL for orders', () => {
      ...
      expect(ordersCache.ttl).toBeLessThan(DEFAULT_CACHE_CONFIG['/products'].ttl);
    });
  ...
  describe('Edge cases', () => {
    it('should handle search with dynamic queries', () => {
      ...
      expect(searchCache.ttl).toBeLessThan(DEFAULT_CACHE_CONFIG['/categories'].ttl);
    });
    ...
    it('should handle analytics with longer cache', () => {
      ...
      expect(analyticsCache.ttl).toBeGreaterThan(DEFAULT_CACHE_CONFIG['/cart'].ttl);
    });
  });
