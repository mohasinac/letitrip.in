...
class ProgressBar {
  ...
  update(current, message = "", status = "") {
    ...
    const progressText = colorize(
      `[${bar}] ${percentage}% (${current}/${this.total})`,
      "cyan",
    );
    const timeText = colorize(`${elapsed}s${eta}`, "yellow");
    const statusText = message
      ? colorize(`${statusEmoji} ${message}`, "blue")
      : "";
...
class Spinner {
  ...
  start() {
    this.interval = setInterval(() => {
      ...
      process.stdout.write(
        colorize(`${this.frames[this.current]} ${this.message}`, "cyan"),
      );
...
function getExistingEnvVars() {
  ...
  try {
    const result = executeCommand("vercel env ls production --json", {
      silent: true,
    });
    if (result) {
      ...
      spinner.stop(
        colorize(
          `✅ Found ${envVars.length} existing environment variables`,
          "green",
        ),
      );
    ...
    spinner.stop(
      colorize("⚠️  Could not fetch existing environment variables", "yellow"),
    );
...
function displayResults(results) {
  ...
  if (results.failed > 0) {
    log(
      `\n⚠️  ${results.failed} variables failed to sync. Please check the errors above.`,
      "yellow",
    );
  }
}
...
function main() {
  try {
    ...
    if (!config.env || Object.keys(config.env).length === 0) {
      spinner.stop(
        colorize("⚠️  No environment variables found in vercel.json", "yellow"),
      );
      process.exit(0);
    }
    ...
    spinner.stop(
      colorize(
        `✅ Found ${Object.keys(config.env).length} environment variables in vercel.json`,
        "green",
      ),
    );
