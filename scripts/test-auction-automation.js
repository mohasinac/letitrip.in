...
async function triggerProcessing() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(
        "⚠️  API returned error (this is expected if auth is required)",
      );
...
async function verifyResults(auctionId) {
  ...
  console.log(
    "  Final Bid:",
    auction.final_bid
      ? `₹${auction.final_bid.toLocaleString("en-IN")}`
      : "None",
  );
  ...
  if (auction.status === "ended" && auction.winner_id) {
    ...
    const wonAuctionsSnapshot = await db
      .collection("won_auctions")
