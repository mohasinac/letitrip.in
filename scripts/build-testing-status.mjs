...
L.push("");
L.push("## 6. Case-catalogue audit (Phase 1)");
L.push("");
L.push("Changes made to the catalogue itself, so nothing disappears silently.");
L.push("");
L.push("### Removed");
L.push("");
L.push("- **`selling/listing-a-product` → `media-upload-images-capped-at-5`** — asserted");
L.push("  the gallery caps at \"5 images (not 10)\". The cap was raised to 10");
L.push("  (`PRODUCT_MAX_IMAGES`), and `selling/media-limits` asserts the opposite in its");
L.push("  own label: *\"The 10th gallery image uploads — the 5th used to fail with a 400\"*.");
L.push("  The case documented the OLD behaviour as the expectation, so it would fail");
L.push("  against correct code and anyone \"fixing\" the product to satisfy it would");
L.push("  re-break ten-image support. Its **id encoded the wrong number**, so correcting");
L.push("  the label alone would still have misled. The invariant it really tested —");
L.push("  stated limit equals enforced limit — survives in");
L.push("  `media-eleventh-image-refused-client-side`, which requires the refusal to name");
L.push("  the limit. Overlay entry removed with it; no orphan left behind.");
L.push("");
L.push("### Numeric sweep — every case asserting a count, checked against its constant");
L.push("");
L.push("26 cases assert a specific number. All were cross-checked; **one inversion found**");
L.push("(above), the rest agree with the code:");
L.push("");
L.push("| case claims | constant | verdict |");
L.push("|---|---|---|");
L.push("| wishlist 20-item cap | `WISHLIST_MAX = 20` | agrees |");
L.push("| history evicts past 50 | `HISTORY_MAX = 50` | agrees |");
L.push("| gallery 10 images + 1 video | `PRODUCT_MAX_IMAGES = 10`, `PRODUCT_MAX_VIDEOS = 1` | agrees |");
L.push("| offer-cancel reason ≥ 10 chars | `.min(10)` on the route schema | agrees |");
L.push("| store reviews exactly 10 per page | `REVIEWS_DETAIL_PAGE_SIZE = 10` | agrees |");
L.push("| up to 4 related carousels | the 4-signal design (category/brand/tags/store) | agrees — `RELATED_DISPLAY_LIMIT = 8` is items *per* carousel, a different number |");
L.push("");
L.push("## 7. What the next run should look like");
L.push("");
