...
async function testProductsPage() {
  ...
  try {
    ...
    const paginationWorks = page2.status === 200 && page2.data.pagination.page === 2;
    logTest('Pagination', paginationWorks);
    recordTest(paginationWorks);
    
    // Test search
    const searchResults = await axios.get(`${BASE_URL}/api/products?search=test`);
...
async function testProductDetail() {
  ...
  try {
    ...
    // Test product detail
    const detailRes = await axios.get(`${BASE_URL}/api/products/${product.slug}`);
    ...
    // Test variants
    const variantsRes = await axios.get(`${BASE_URL}/api/products/${product.slug}/variants`);
    const variantsWork = variantsRes.status === 200;
    logTest('Product variants API', variantsWork);
    recordTest(variantsWork);
    
    // Test similar products
    const similarRes = await axios.get(`${BASE_URL}/api/products/${product.slug}/similar`);
    const similarWorks = similarRes.status === 200;
    logTest('Similar products API', similarWorks);
    recordTest(similarWorks);
    
    // Test seller items
    const sellerRes = await axios.get(`${BASE_URL}/api/products/${product.slug}/seller-items`);
...
async function testShopDetail() {
  ...
  try {
    ...
    // Test shop products
    const productsRes = await axios.get(`${BASE_URL}/api/shops/${shop.slug}/products`);
    const productsWork = productsRes.status === 200;
    logTest('Shop products API', productsWork);
    recordTest(productsWork);
    
    // Test shop reviews
    const reviewsRes = await axios.get(`${BASE_URL}/api/shops/${shop.slug}/reviews`);
...
async function testCategoryDetail() {
  ...
  try {
    ...
    const productsRes = await axios.get(`${BASE_URL}/api/categories/${category.slug}/products`);
    const productsWork = productsRes.status === 200;
    logTest('Category products API', productsWork);
    recordTest(productsWork);
    
    // Test subcategories
    const subsRes = await axios.get(`${BASE_URL}/api/categories/${category.slug}/subcategories`);
    const subsWork = subsRes.status === 200;
    logTest('Subcategories API', subsWork);
    recordTest(subsWork);
    
    // Test similar categories
    const similarRes = await axios.get(`${BASE_URL}/api/categories/${category.slug}/similar`);
    const similarWorks = similarRes.status === 200;
    logTest('Similar categories API', similarWorks);
    recordTest(similarWorks);
    
    // Test category hierarchy
    const hierarchyRes = await axios.get(`${BASE_URL}/api/categories/${category.slug}/hierarchy`);
...
async function testGuestCart() {
  ...
  try {
    ...
    const mergeRes = await axios.post(`${BASE_URL}/api/cart/merge`, {
      guestCart: [
        { productId: 'test-id', quantity: 1 }
      ]
    }).catch(err => err.response);
    
    // Should return 401 without auth, which means endpoint exists
    const endpointExists = mergeRes && (mergeRes.status === 401 || mergeRes.status === 200);
