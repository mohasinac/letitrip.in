...
function migrateRouteFile(filePath, handlerType) {
  try {
    ...
    if (content.includes('createAdminHandler') || 
    ...
    if (!content.includes('createUserHandler') && !content.includes('createAdminHandler') && !content.includes('createSellerHandler')) {
    ...
    for (const method of httpMethods) {
      ...
      if (match) {
        ...
        for (let i = 0; i < content.length; i++) {
          if (!inFunction && content.substring(i).startsWith(`export const ${method} = ${handlerType}(async (request: NextRequest, user) => {`)) {
            inFunction = true;
            braceCount = 1;
            i += `export const ${method} = ${handlerType}(async (request: NextRequest, user) => {`.length - 1;
            continue;
          }
        ...
        if (functionEnd !== -1) {
          content = content.substring(0, functionEnd) + '});' + content.substring(functionEnd + 1);
        }
