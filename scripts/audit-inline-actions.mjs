...
// Parse ACTION_ID string values from action-defs.ts
const actionDefsContent = readFileSync(ACTION_DEFS, "utf8");
const ACTION_ID_VALUES = new Set(
  [...actionDefsContent.matchAll(/:\s+"([a-z][a-z0-9-]+)",/g)].map((m) => m[1]),
);
...
for (const dir of SCAN_DIRS) {
  for (const file of walk(dir)) {
    ...
    if (content.includes("BulkActionBar") || content.includes("actions={[")) {
      for (const lbl of ACTION_META_LABELS) {
        if (content.includes(`label: "${lbl}"`) && !importsRegistry) {
    ...
    if (/\{\s*id:\s*["'][^"']+["'][^}]*requiresAuth:\s*true/.test(content) && !importsRegistry) {
      VIOLATIONS.push(`${file}: inline requiresAuth: true — use useAuthGate(ACTION_ID.*) from registry`);
    }

    // Rule 4: <Button onClick={handle*}> with mutation but no auth gate
    if (
