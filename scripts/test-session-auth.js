...
async function runTests() {
  ...
  try {
    ...
    if (logoutResponse.status === 200) {
      ...
      console.log('   Cookie cleared:', !cookieJar || cookieJar.includes('session=;'));
    } else {
    ...
    if (protectedAfterLogoutResponse.status === 401) {
      ...
      console.log('❌ Protected route should be blocked:', protectedAfterLogoutResponse.data);
    }
    console.log('');
    ...
    if (protectedAfterLoginResponse.status === 200) {
      ...
      console.log('❌ Protected route failed:', protectedAfterLoginResponse.data);
    }
    console.log('');
