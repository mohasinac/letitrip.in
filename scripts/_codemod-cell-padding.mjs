#!/usr/bin/env node
/**
 * Migrate `px-N py-N` patterns on Th/Td to `padding="..."` variant + color
 * tokens to color="...". Compound patterns like `px-4 py-3` map to padding="md".
 */
...
// Compound padding pairs → padding variant
const PADDING_PAIRS = [
  [["px-4", "py-3"], "md"],
  [["px-3", "py-2"], "sm"],
  [["px-2", "py-1"], "xs"],
...
// Single-shade color tokens (no dark sibling required)
const COLOR_MAP = new Map([
  ["text-zinc-900", "primary"], ["text-zinc-800", "primary"], ["text-zinc-700", "primary"],
  ["text-zinc-600", "muted"], ["text-zinc-500", "muted"],
  ["text-zinc-400", "faint"], ["text-zinc-300", "faint"],
]);

// Compound color pairs
const COLOR_PAIRS = [
...
function transform(text, tag) {
  ...
  while ((m = openRe.exec(text)) !== null) {
    ...
    for (let pi = 0; pi < parts.length; pi++) {
      ...
      // Try compound padding pairs first
      ...
      // Color pairs
      for (const [pair, value] of COLOR_PAIRS) {
        const i1 = wordIdx.find(i => toks[i] === pair[0]);
        const i2 = wordIdx.find(i => toks[i] === pair[1]);
        if (i1 !== undefined && i2 !== undefined) {
          if (addProp("color", value)) {
            removeAt(i1); removeAt(i2);
            touchedAny = true;
          }
          break;
      ...
      // Single-shade color (no dark sibling)
      if (!toks.some((t) => t.startsWith("dark:text-"))) {
        const ci = wordIdx.find(i => COLOR_MAP.has(toks[i]));
        if (ci !== undefined && addProp("color", COLOR_MAP.get(toks[ci]))) {
          removeAt(ci);
          touchedAny = true;
        }
      }
...
console.log(`Th/Td padding+color codemod modified ${filesChanged} files.`);
