...
Last updated: 2026-04-19 (B54: Fix 4 stores/[storeSlug]/ copy-paste route bugs; appkit tsc: 0 errors, letitrip tsc: 0 errors)
...
| B54 | Post-Phase Wave 7.3 | Fix 4 stores/[storeSlug]/ copy-paste route bugs (had site-settings content) | B52 | ✅ | ✅ | ✅ | ✅ | ✅ | **Letitrip:** All 4 `stores/[storeSlug]/` routes had site-settings content. Replaced with thin wrappers delegating to appkit-exported handlers: `stores/[storeSlug]/route.ts` → `storeSlugGET`, `stores/[storeSlug]/auctions/route.ts` → `storeAuctionsGET`, `stores/[storeSlug]/products/route.ts` → `storeProductsGET`, `stores/[storeSlug]/reviews/route.ts` → `storeReviewsGET`. All exported from `@mohasinac/appkit/features/stores/server`. appkit tsc: 0 errors, letitrip tsc: 0 errors. |
