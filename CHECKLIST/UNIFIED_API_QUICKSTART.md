...
export async function verifyShopOwnership(req: NextRequest, shopId: string) {
  const user = req.user;
...
export async function GET(req: NextRequest) {
  try {
    ...
    result.shops = result.shops.map((shop) => filterShopData(shop, user));
...
function buildShopsQuery(user: any, searchParams: URLSearchParams) {
  ...
  if (!user || user.role === "user") {
    ...
    query.$or = [{ ownerId: user.id }, { verified: true, banned: false }];
...
export function ShopsList() {
  ...
  return (
    <div>
      <h2>{user?.role === "admin" ? "All Shops" : "Featured Shops"}</h2>
      {shops.map((shop) => (
...
describe("GET /api/shops", () => {
  ...
  it("should filter by verified status (admin only)", async () => {
    ...
    expect(data.success).toBe(true);
    expect(data.data.shops).toEqual(
      expect.arrayContaining([expect.objectContaining({ verified: false })])
    );
  });
});
...
function buildQuery(user: User | null, filters: any) {
  ...
  switch (user?.role) {
    ...
    case "seller":
      ...
      query.$or = [{ ownerId: user.id }, { public: true }];
      break;
    default:
      // Public only
      query.public = true;
