...
| Realtime Database | 1 GiB stored, 10 GiB downloaded/month (Spark-only free allowance; Blaze is pay-as-you-go beyond a much smaller included amount) | **Six path families**, not one — `chat/*` (admin chat), `chats/*` (conversation pings), `auction-bids/*`, and the three short-lived signal channels `auth_events`/`payment_events`/`bulk_events`. Only the last three are pruned (`cleanupRtdbEvents`, 3/15/15 min). `bulk_events` payloads are **not** size-capped — `newsletterExport` writes a full CSV there. | **Client writes are forbidden by the rules** — every node is `.write: false` and all writes go through the Admin SDK; a client-side write is a bug, not a config gap. Don't store long-lived data here: `chat/*/messages` and `chats/**/lastUpdate` already accumulate with no TTL. This row claimed "used only for the `bulk_events` ping channel — tiny payloads, short-lived" until 2026-08-29; it was never accurate. |
