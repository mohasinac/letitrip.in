...
> `next.config.js` (Root Cause #95 — 120 → 4 concurrent page renders) and
> `turbopackSourceMaps: false`. **The goal is NOT met**: across 7 valid cold
> builds every run exceeded 8,192 MB on private commit (medians 8,701 / 9,163),
> and the measurements were taken on a contended machine with a ~1.9 GB noise
> floor (Root Cause #96). The remaining lever is structural, not a flag — compile
> memory scales with the module graph, and 715 of 1,099 source files import a
> 459 KB barrel re-exporting ~5,000 symbols. `optimizePackageImports` cannot fix
> that from outside and measured worse; the real fix is the deep-import migration
> already mandated by Root Cause #18 (Phase 11, W5-1/W5-2).
>
> Measure with **`node scripts/probe-build-memory.mjs --variant <name> --cold`**
> (per-phase peak RSS + wall time; needs `scripts/lib/proc-sampler.ps1`), collate
> with **`scripts/summarize-build-memory.mjs`** (groups replicates, prints the
> noise floor, excludes failed runs), and prove no routes were silently dropped
> with **`scripts/diff-build-routes.mjs <baseline> <candidate>`**. Results land in
> gitignored `.build-memory/`. **Never quote a single run as a result.**
...
> `node_modules` — an appkit version bump above all — can land on Standard, exceed it, and
> die with `SIGKILL` and a "Build system report / Out of Memory" block. Measured cold
> compile is **6.5–8.4 GB** (`next.config.js`), against an 8 GB container.
>
> **Raising `NODE_OPTIONS=--max-old-space-size` in `vercel.json` does NOT fix this.**
> Turbopack runs in a worker thread with `isolatedMemory: false` and its Rust arena is
> invisible to V8's heap cap — the flag bounds the JS heap, not the allocation that is
> actually blowing the container.
>
> **The response is to retry**: elastic has already escalated by the time you read the
> failure, so the next build gets the 16 GB machine. Pin `buildMachineType` only if the
> reactive escalation becomes a recurring cost in wasted deploys — it bills more per build.
> Verify, never assume: `GET /v9/projects/{projectId}?teamId={orgId}` → `resourceConfig`.

Turbopack's build-time peak RSS for this app (~5.7–6.2 GB, verified locally) didn't reliably fit in Hobby's real available headroom, causing repeated OOM/SIGKILL build failures even though the code itself was correct (see the 2026-08-20 server-only-leak saga below and Root Cause #24-adjacent findings). **This is a build-time-only exception.** Every *runtime* ceiling below (function memory, timeouts, payload, quotas) is still deliberately treated as Hobby-tier and must keep being respected when writing new code — the Pro upgrade was not an invitation to write less defensive code or lean on higher runtime limits Pro would otherwise unlock. The goal is to avoid inflating Vercel usage/credit consumption beyond what the build-machine fix required.
...
### 🛑 Caching semantics are NOT what `grep` suggests — read the build manifest

The account was paused on Hobby limits 2026-09-14 and the first diagnosis was wrong
three times because it was inferred from source. Ground truth lives in
**`.next/prerender-manifest.json`**:

| Where a route appears | What it is |
|---|---|
| `routes` | prerendered / static |
| `dynamicRoutes` | on-demand ISR |
| **neither** | **fully dynamic SSR** — every request is a render |

Measured on this app: only **four** routes carry a short ISR window
(`/en`, `/en/lottery`, `/en/promotions`, `/en/sellers`), 43 are cached
indefinitely, and **no `[slug]`/`[id]` content detail route is in the manifest at
all**. So the CPU and invocation spend is dynamic SSR, not ISR.

Four Next 16 facts that make `grep "export const revalidate"` misleading:

1. **The default is `false` ≡ `Infinity` — cached indefinitely.** A route with *no*
   `revalidate` is cheaper than one with a long window, not more expensive. Adding
   a number to such a route is a **regression**.
2. **`await searchParams` or `getServerSessionUser()` makes the route dynamic**, and
   the `revalidate` declaration then does nothing. 14 of this app's 33 short-window
   declarations are inert for exactly this reason, including `/products`.
3. **"The lowest `revalidate` across each layout and page of a single route
   determines the frequency of the entire route."** So `events/[id]/layout.tsx` (60)
   and `lottery/[id]/layout.tsx` (30) cascade to their whole subtrees — raising a
   page value without its layout achieves nothing.
4. **`revalidate` / `dynamic` / `fetchCache` are REMOVED when `cacheComponents` is
   enabled.** It is not enabled here (verified in `next.config.js`). Turning it on
   would silently kill all 80 declarations at once.

**Client timers are a recurring server bill**, and `scripts/audit-client-poll-cost.mjs`
(strict-zero) now blocks the two shapes that caused the overage: a sub-60s
`refetchInterval` literal, and a `new EventSource` outside the module owning the
ref-counted channel. An SSE connection here holds a function open for its full 45s
TTL, so it is billed compute, not a cheap socket. Marker: `// audit-client-poll-ok: <reason>`.

**`experimental.staleTimes.dynamic` defaults to 0**, i.e. a dynamic page segment is
not reused at all and every soft navigation back to it refetches the whole RSC
payload. On a mostly-dynamic app that is a standing Edge-Request + invocation cost;
it is set to 180 in `next.config.js`.
...
| Root `firestore.rules`, `firestore.indexes.json`, `storage.rules`, `database.rules.json` | Generated by `npm run firebase -- generate` | **Yes — all four are tracked** |

> 🛑 This row said **"No (gitignored)"** until 2026-09-14, and it was wrong: `git ls-files` returns all four. Believing it means editing `appkit/firebase/base/` and *not committing the regenerated root*, so `base/` and root ship out of sync — and root is what `firebase.json` deploys. `audit-firebase-rules-generated` exists precisely to catch that gap and was failing on a 190-line drift when this was corrected. **Regenerate and commit the root files in the same change as any `base/` edit.**
...
> 🛑 **Corrected 2026-09-13: BOTH are Turbopack now.** This paragraph said "local dev uses webpack" and that has been false since Next 16 made Turbopack the default. `scripts/dev-next.mjs` spawns `next dev` with **no bundler flag**, and `next/dist/lib/bundler.js` `parseBundlerArgs()` returns Turbopack when none is given. The `webpack:` function in `defineNextConfig` — its `externals`, its `IgnorePlugin`, its 31 client `alias: false` stubs — is therefore **dead code in dev AND in `next build`**. It runs only under an explicit `--webpack`. Do not delete it (`audit-firebase-alias` pins it, and it is the escape hatch if Turbopack is ever abandoned), but do not reason from it either. The historical explanation below is kept because it is why `serverExternalPackages` exists.

Local dev used to use **webpack**, which respects the `externals` function in `next.config.js` — firebase-admin is silently externalized even if it leaks into the main index. **Turbopack ignores webpack `externals`.** Turbopack strictly follows the full import chain from every module included in `dist/index.js`. If any re-exported symbol's module chain reaches `firebase-admin` (which has a **static top-level** `import from "firebase-admin/app"`), Turbopack will include `child_process`/`fs` in the **client** bundle → build failure.
...
### 🛑 …but a dynamic API in a DASHBOARD layout is the correct lever

The mirror of the rule above, and it is not a contradiction. `admin/layout.tsx` and
`store/layout.tsx` `await getServerSessionUser()` because those subtrees genuinely
are per-request — and that single call is what makes **all 238 routes beneath them
dynamic, with zero `force-dynamic` exports anywhere in the tree**. That is the
sanctioned way to declare "this subtree is per-request". A route-segment config is
not (Root Cause #89, `audit-no-force-dynamic`).

Three consequences worth knowing before touching any of it:

1. **`cookies()` must stay OUTSIDE the `try`** in
   [auth-server.ts](src/lib/firebase/auth-server.ts). During prerender it throws
   `DynamicServerError`, which is a bailout signal, not a failed session read. The
   bailout itself survives being caught — `throwToInterruptStaticGeneration` sets
   `prerenderStore.revalidate = 0` *before* throwing — but swallowing it hands the
   layout `user = null`, so it runs its own `redirect()` **at build time**. That is
   what made a page-level `<Suspense>` look useless for two weeks.
2. **Next still ATTEMPTS to prerender pages under a dynamic layout.** The HTML is
   discarded (`app-page.js` early-returns on `revalidate === 0`), so no stale admin
   page is ever served — but an unguarded throw during the attempt is **fatal to the
   build**. Server-side reads on dashboard pages belong in `safeRead`.
3. **`user/layout.tsx` reads nothing server-side** (`RoleGuard` is a client
   component), so `/user/*` prerenders to a signed-out chrome-plus-spinner shell and
   hydrates the session client-side. Verified 2026-09-02: no uid, email, displayName
   or storeId appears in `/en/user/profile.html` or its RSC payload. If those routes
   ever need to be dynamic, give that layout a real session read — do not reach for
   a segment config.
...
3. `LISTING_TYPE_REGISTRY` (`_registry.ts`) — compile-checked, same reason. **This is also where the type's browse chrome lives** — see the next section. Since 2026-09-14 the plugin also carries the required **`inGeneralCatalogue: boolean`**, so a tenth type cannot compile without answering "does this belong on `/products`, or does it own a nav-linked page?";
...
**`/products` is the GENERAL CATALOGUE, not an index of every listing type** (2026-09-14). It spans exactly the types whose plugin sets **`inGeneralCatalogue: true`** — today `standard`, `classified`, `digital-code`, `live`. `GENERIC_PRODUCT_LISTING_TYPES` is `ALL_LISTING_TYPES.filter(t => pluginFor(t).inGeneralCatalogue)`; never hand-write the array (`audit-listing-type-tab-coverage`'s STAYS_DERIVED blocks it).

**The rule for the flag is nav reachability, not taste.** A type whose dedicated page is in `MAIN_NAV_ITEMS` (auction, pre-order, prize-draw, art, stickers) lives only there, so one item is never reachable from two places with two different chromes — an auction card in the general grid renders Buy-Now chrome instead of bid affordances. A type with a dedicated page but **no nav entry** (classified, digital-code, live) stays in the catalogue, because `/products` is its only discovery path; dropping those is how Root Cause #61 happened. 🛑 Setting the flag `false` for a type obliges you to add its nav entry in the same commit.

Its TYPE filter is a **multi-select checkbox group inside the filter drawer** (`FilterFacetSection`, not the old always-visible `FilterChipGroup` row): nothing ticked = every type the catalogue spans, one or more ticked = a pipe-joined OR-group. `PRODUCT_TYPE_FILTER_TABS` derives from `GENERIC_PRODUCT_LISTING_TYPES`, **not** `ALL_LISTING_TYPES` — a chip for an out-of-span type intersects to empty and `listPublicProducts` then falls back to the FULL span, so ticking it would silently *widen* the results. `ADMIN_PRODUCT_LISTING_TYPE_TABS` is separate and correctly stays all-nine.

A `?listingType=` naming only out-of-span types **redirects** to the page that owns them (`ProductsIndexPageView`), because the same empty-intersection fallback would otherwise render the general catalogue as though the filter had been honoured. The "Full X filters →" link is suppressed for `standard`, whose `browseRoute` is `/products` itself.

🛑 **`browseRoute` is NOT the discriminator.** Every one of the nine plugins sets it non-null, including `standard` → `/products` and `art`/`stickers` → `/art`. Its doc comment claimed `null` meant "browses on /products" and no config ever matched that.
...
### Guest price gating (2026-09-14)

**Prices, current bids, deposits and per-entry amounts are hidden from signed-out
visitors**, so the catalogue cannot be read at scale without an account. A guest still
sees the listing — SEO and discovery are unaffected.

| Need | Use |
|---|---|
| A listing's primary price | `<GatedPrice>` — keeps the row and renders "Sign in to see price" linking to `/auth/login` |
| A SECONDARY money detail beside an already-gated amount (deposit note, handling fee, min increment, "you save ₹X", a discount %) | `<PricesOnly>` — renders nothing for a guest. **Never** for a primary price: a card whose price row vanishes reads as a rendering bug and breaks row heights |
| A plain STRING prop (a bottom-bar `infoLabel`, an OG caption, a lightbox `sub`) | `useCanSeePrices()` — a string cannot be wrapped in a slot |

**Three states, not two.** `SessionProvider` mounts `initialUser={null}` (the root layout
must stay static — Root Cause #82), so `loading` starts `true` on every hard load *for
signed-in users too*. `isAuthResolving` renders a neutral placeholder; only
`!canSeePrices && !isAuthResolving` is a confirmed guest. A two-state gate flashes
"Sign in to see price" at signed-in buyers on every page load.

`useCanSeePrices` calls **`useOptionalSession()`**, not `useSession()` — library code that
adjusts for the viewer must not require a provider (Root Cause #20). No provider fails
CLOSED.

**`PriceDisplay` is gated unconditionally and takes no `ungated` opt-out.** Its only two
consumers are `ProductGrid` and `ProductDetailPageView`, both public; admin/seller/cart/
checkout/order surfaces call `formatCurrency` directly and are signed-in by definition.

**The price-range facet and every money SORT are hidden from guests**
(`withoutPriceSorts`, `MONEY_SORT_FIELDS` = price / currentBid / startingBid /
buyNowPrice — `bidCount` is deliberately absent, a count is not an amount). Bracketing a
hidden price with a range filter, or ranking it with a sort, is the only casually
reachable way around the gate. This removes the CONTROL, not the capability: a
hand-written `?sort=price:asc` or `?f=price>N` still works, and that residue is accepted.

**Accepted residue, deliberately not closed**: `price` stays in the RSC payload and in
`GET /api/products`. Withholding it would mean fetching prices separately on every
listing page. `sanitizeProductForPublic` remains a spread-and-delete deny-list that strips
only seller identity.

#### The gate must be DECLARED to crawlers, never hidden from them

`offers.price` is **kept** in the JSON-LD on purpose, so search engines still see it. But
Googlebot crawls signed out — it renders "Sign in to see price" while the markup declares
a real amount, and Google requires marked-up content to be visible to the user. So every
page emitting `offers.price` must also emit **`gatedPriceWebPageJsonLd(path)`**
(`isAccessibleForFree: false` + `hasPart.cssSelector`). Serving prices to crawler
user-agents but not to people is cloaking and is not done.

🛑 **`GATED_PRICE_CLASS` (`GatedPrice.tsx`) and `GATED_PRICE_SELECTOR` (`seo/json-ld.ts`)
are two literals that MUST agree.** They cannot import each other — `seo/` is reached from
server-only metadata paths and must not pull in a `"use client"` module (Root Cause #76).
A rename on either side points the crawler at a selector matching nothing, which reads
exactly like no declaration at all. `audit-guest-price-leak`'s R4 cross-checks them.

**OG images carry no money at all.** An OG image is a public image URL with the amount
burned into the pixels — no gate reaches it and no markup describes it. All eight
money-bearing `og.tsx` renderers were stripped; auction and pre-order already used a
non-money accent and are the pattern.

Enforced by [`scripts/audit-guest-price-leak.mjs`](scripts/audit-guest-price-leak.mjs)
(strict-zero, R1–R4). Suppression `// audit-guest-price-ok: <reason>`, for genuinely
...
| 17 | **`useSearchParams()` requires `<Suspense>` — and it fails in TWO modes, only one of which this row used to describe** | Every appkit listing view calls `useUrlTable()` → `useSearchParams()`. **(a) Runtime**: in `next start`, no boundary → error boundary → "Something went wrong". **(b) Build time**: during static prerender the same call throws `BailoutToCSRError` → `useSearchParams() should be wrapped in a suspense boundary at page "/x"` → **the build fails**. Fix for both: the admin/store/user dashboard layouts AND the root `[locale]/layout.tsx` wrap `{children}` in `<Suspense>`. Never remove these boundaries. New dashboard sub-layouts should also include `<Suspense>` around `{children}`. **A boundary at ANY ancestor satisfies it — the layout ones count**, verified 2026-09-02 by removing all 222 `force-dynamic` exports and building clean with zero Suspense failures. **The fix is never a route-segment config** (Root Cause #89 — that mistake cost 225 files). Note this row described only mode (a) until 2026-09-02, while ~246 code comments citing "#17" were all about mode (b); that gap is why `force-dynamic` looked like a novel fix rather than a documented one. |
...
| 28 | **On this Windows setup, `npm install` does not reliably resync a `file:./appkit` dependency's non-`dist` files (e.g. `appkit/scripts/*.mjs`) — even a full `rm -rf node_modules/@mohasinac/appkit && npm install`** | `node_modules/@mohasinac/appkit` is a real copy, not a symlink/junction, on this machine (`Get-Item ... | Select LinkType` returns empty). `npm run build` inside `appkit/` only refreshes `appkit/dist/` at the source, which the linked copy also does not pick up automatically. A source fix to `appkit/scripts/seed-cli.mjs` (or any non-compiled file the `bin` entries point at) can silently keep failing with the pre-fix behavior indefinitely even after multiple `npm install` runs report "up to date" or "added N packages." **Verification, not assumption**: after any fix to a file under `appkit/scripts/` or a rebuild of `appkit/dist/`, diff the **whole tree** — `diff -rq appkit/dist node_modules/@mohasinac/appkit/dist` — before trusting the CLI reflects the fix. If they differ, manually resync: `rm -rf node_modules/@mohasinac/appkit/scripts node_modules/@mohasinac/appkit/dist && cp -r appkit/scripts node_modules/@mohasinac/appkit/scripts && cp -r appkit/dist node_modules/@mohasinac/appkit/dist`. 🛑 **A single-file diff is not the check** — this row said `diff <file>` until 2026-09-05, and `diff appkit/dist/index.js node_modules/.../dist/index.js` reported "in sync" while nine files under `dist/features/tester/` differed: the barrel is byte-identical whenever no export was added, so the one file most likely to be checked is the one least likely to have changed. `npx appkit-seed load` then wrote 972 checklist docs from the stale copy, 22 cases short and with none of the new fields, reporting `created 972, errors 0`. Use `-rq`. |
...
| 89 | **`force-dynamic` reached for as a build-error silencer rather than as a rendering declaration — 225 of them in 48 hours, 197 provably no-ops** | Root-caused 2026-09-02. `817b602df` correctly removed `await headers()` from the root locale layout and added `generateStaticParams` (Root Cause #82). Side effect: the site prerendered for the **first time**, so every latent unguarded `useSearchParams()` surfaced at once (Root Cause #17b). The response was `export const dynamic = "force-dynamic"` — first on the three dashboard layouts (`fccc9a539`, reverted the same day by `b2d205d55`), then per-page via a 259-file stash. **Three things were wrong with it.** (a) **197 of the 222 were no-ops**: `admin/layout.tsx` and `store/layout.tsx` both `await getServerSessionUser()`, and `throwToInterruptStaticGeneration` sets `prerenderStore.revalidate = 0` **before** it throws, so `export/routes/app-page.js` early-returns writing **no HTML** — those subtrees can never be static, with or without the config. Verified by removing all 222 and rebuilding: 238 admin+store routes, **zero** non-dynamic. (b) **The claim that justified it was a real observation with the wrong diagnosis.** The commit recorded *"a page-level `<Suspense>` does NOT satisfy Next 16's static-export check — admin/moderation still failed with one in place"*. It failed because `getServerSessionUser` wrapped `await cookies()` in a blanket `try/catch`: the prerender bailout was swallowed, `user` became `null`, and the layout ran its own `redirect()` **at build time** — thrown in the LAYOUT, above `{children}`, where no boundary below could ever catch it. The layout `<Suspense>` had worked the whole time. (c) **It hid real bugs.** Removing it immediately surfaced two: `serverErrors` composite indexes declared `occurredAt ASCENDING` against a query ordering `desc`, so all four `/admin/maintenance/*` error pages were `9 FAILED_PRECONDITION` **in production**; and `/admin/maintenance/cloud-logs` called a Google Cloud API with no guard. Both had been invisible because the pages were never prerendered. **The rule: a route-segment config declares a rendering contract, never a workaround.** The fix is a `<Suspense>` boundary at any ancestor, or a genuine dynamic API in the server layout when the subtree really is per-request. **One caveat worth knowing**: Next still *attempts* the prerender of a page under a dynamic layout — the output is discarded, but an unguarded throw during the attempt is fatal to the build, which is why server-side reads on dashboard pages belong in `safeRead`. Enforced by `scripts/audit-no-force-dynamic.mjs` (strict-zero, which also blocks the `revalidate = 0` / `fetchCache` / `unstable_noStore` respellings inside the auth-gated subtrees, and the indirection dodge). |
| 90 | **A collection wired for PII but never registered for LOADING — six collections held ZERO documents in every run ever, and the half that WAS wired made the gap read as support** | Root-caused 2026-09-11 mid-run, from a tester batch returning `0 pass / 0 fail / 2 blocked` with *"the fixture did not seed any offers for this seller."* `appkit-seed`'s `COLLECTION_MAP` had 29 keys and **`offers` was not one of them**, so the collection never appeared in `npx appkit-seed status` and Firestore held **0 offers in every run to date** (measured: `offers 10 seed / 0 in db`). **What made it invisible for so long is that the PII half WAS wired**: `OFFER_PII_FIELDS` is imported and mapped in the same file, and `manifest.ts` already combined `offersSeedData` + `offersTesterSeedData` — so every grep for "offers" in the seeding code found real hits, and the loader's omission read as support rather than absence. Only `COLLECTION_MAP` and the DATA map actually drive loading. Same shape as Root Cause #57 (a feature complete end-to-end except one adapter) and the symptom is the same kind: a polite empty state — *"No offers received"* — rather than an error. **Sweeping for the same defect rather than waiting to trip over each one found five more**, all at 0 documents live: `supportTickets` (8 batches need it), `catalogueItems` (4), `procurementShipments`/`shipmentLots`/`shipmentItems` (3). 47 documents that had never existed. **Two of my own measurements were wrong in opposite directions and both are worth recording**: a bare grep for `catalogue` matched 36 batches, but most hits are the English word in prose ("the main catalogue") — scoping to the ROUTE gives 4; and I counted a collection named `shipments` when `SHIPMENT_COLLECTION` is actually `"procurementShipments"`, so the first live count was of a collection that does not exist and would have read `0` either way (Root Cause #84, both directions). **Registering a collection is not the end of it** — `audit-tester-plugin-wiring`'s R1 immediately failed all five, correctly: a collection the seeder writes must also declare what happens to it on a tester wipe, or it is PRESERVED by default and accumulates stale rows. Classify by what a row REFERENCES, never by what it is called: `supportTickets`/`catalogueItems` hold **real user rows** so they cascade on their reference into seed-owned products and are restored (the orders pattern), while procurement references nothing seed-owned (`shipmentId`/`lotId` are internal FKs, `createdBy` is a PRESERVE-tier user) so a cascade would match nothing and leave every row standing — it is `SEED_OWNED`. Note `SEED_TRANSACTIONAL` is deliberately **not** part of R1's classified set: it is a restore list, orthogonal to the delete tiers. **When a feature's cases all report "nothing here", check the seeder's own registry before believing the feature is broken — and grep the map that DRIVES loading, not every mention of the collection's name.** |
| 91 | **An ENVIRONMENT failure handled with a BATCH failure's retry policy — 55 sessions burned across ~25 batches in minutes, and the fix was a documented TODO nobody built** | Root-caused 2026-09-11 from the user having to kill 50+ sessions by hand. The subscription hit its usage limit, so every `claude` spawn returned in ~1s having done nothing. The tester pool treated each as a *batch* failure — retry once, requeue at the tail, **take the next batch** — and marched through the queue at two sessions apiece. **The signature is why nothing caught it**: `{"type":"result","subtype":"success","is_error":true,"num_turns":1,"duration_ms":1210,"result":"You've hit your limit · resets 4:50am (Asia/Calcutta)"}` — `subtype` is `"success"` while `is_error` is true, so an exit-code check sees an ordinary non-zero and the circuit breaker (which only counts "did not load") sees nothing. **This was predicted in writing and not built**: the plan's own D4 section said *"a rate-limited session exits non-zero and is counted as a real batch failure, burning the retry budget — and the circuit breaker only counts 'did not load', so this failure mode is NOT currently caught. Add it."* A predicted failure with no mechanism enforcing the prediction is an unfixed failure — the same lesson as Root Cause #74, one layer up. **The distinction that matters**: a batch failure is about ONE batch, so retry-then-move-on is right and "requeue at the tail so a poisoned batch cannot starve the queue" is good design. An environment failure is about the WHOLE RUN, so moving on is the worst available response — every batch is equally poisoned, and advancing converts one stalled batch into an exhausted queue. **The fix**: classify from the transcript's LAST `result` row (not a grep anywhere, or a tester legitimately testing a rate-limit UI pauses the pool over page text); on an environment failure REFUND the attempt (the batch did no work and is not at fault — counting it retires a good batch after three limit hits and reports it failed having never run), requeue at the **head** so nothing is skipped, and pause **every** worker because the limit is account-wide. The wait comes from the reset time the CLI itself reports. **Two more bugs surfaced in the same investigation, both worse than the storm**: (a) `run.mjs`'s internal retry DOUBLED every failure into two sessions, so the pool's own retry budget and the runner's were multiplying — one owner only; (b) the worker consolidated **unconditionally**, writing a `0 pass / 0 fail / 0 blocked` row for a batch that produced nothing. That row satisfies the already-done check, so the batch is skipped forever while the report counts it as complete-with-no-findings — **indistinguishable from a clean pass**. 52 such rows existed; left alone they would have silently removed a quarter of the catalogue from the run. **When a long-running harness fails, ask first whether the fault is about the unit of work or about the world it runs in — and never let a retry policy for one be applied to the other.** |
| 92 | **A trigger that writes back to the collection it watches, whose no-op guard used `JSON.stringify` — 12M invocations against a 2M/month free quota, and the only symptom was a billing page** | Root-caused 2026-09-12 from a Firebase console screenshot, not from any error. `onShipmentHeaderWrite` watches `procurementShipments/{shipmentId}` on **documentWritten** and writes back to that same document (`totals`, `totalsComputedAt`, `updatedAt`). The one thing stopping infinite recursion was `JSON.stringify(shipment.totals ?? {}) === JSON.stringify(totals)` — and **`JSON.stringify` is key-ORDER sensitive**. `allocateShipmentCosts()` builds `totals` in construction order (`lotsCost, customsTotal, shippingTotal, laborCost, …`) while **Firestore returns map fields sorted alphabetically** (`customsTotal, estimatedProcessingDays, laborCost, lotCount, …`). The two strings could never match, so `totalsUnchanged` was **always false**, every invocation wrote, and every write re-triggered the function. Measured: `onShipmentHeaderWrite` **1,017,548** invocations in 24h, `onShipmentDeleted` **1,017,372**, every other function in the project **0–95**. **`onShipmentDeleted` was pure collateral** — it is correctly guarded (`if (event.after) return`) but is bound to the same path, so it woke a million times a day just to return; a correct guard does not save you from a neighbour's loop. **Nothing in the codebase could see it**: the trigger is an ordinary definition, the handler an ordinary function, and the broken guard reads like a perfectly normal deep comparison whose own docstring correctly described the intent it failed to implement. `npm run check`, `tsc` and every audit passed throughout. **The fix is a key-order-independent comparison WITH A FLOAT TOLERANCE** — `projectedMarginPercent` and `projectedRoiPercent` are unrounded divisions, and a Firestore round-trip can return a last-bit difference, so exact `!==` on those is the same bug by another mechanism. **Two audits, and the lesson is in the second one**: `verify-shipment-allocation-guard` reproduces the key-order failure against the real 14-field shape, with negative controls so a comparison that answers "equal" to everything — silently freezing allocation instead of looping — fails too. `audit-trigger-self-write` generalises it, and **demands an explicit `// trigger-self-write-ok: <why>` rather than trying to RECOGNISE a guard**: its first version scanned for `unchanged`/`Equal(`/`if (event.after) return`, and removing the real guard changed nothing, because a variable merely NAMED `totalsUnchanged` while assigned `false` still matched. **A check a variable name satisfies is decoration.** There is no reliable static test for "this comparison terminates" — the original bug HAD a guard, in the right place, and it was broken — so the audit asks a human to state why, which is checkable with certainty. **The blast radius went well past the bill**: the same overage blocked `firebase deploy` with a 403 billing denial, and the sibling Firestore read quota (47K/50K) degraded PRODUCTION — `/api/site-settings` 500ing and every search returning `total:0` — which a tester run would have recorded as ~1,187 evidenced, confident, wrong product defects. **When a free-tier ceiling is blown, check the per-function breakdown before anything else: one runaway hides among forty healthy functions, and the aggregate number never names it.** |
| 93 | **A client-side visibility gate does not reach server-rendered metadata — and where the two disagree, the disagreement has to be DECLARED rather than left implicit** | Built 2026-09-14 to hide prices from signed-out visitors, the stated risk being a customs officer inferring a valuation mismatch from publicly visible resale prices. **The instructive part is what a client gate does not cover.** Public listing pages are static/ISR by design (the root locale layout must stay static, Root Cause #82), so there is no per-request session and the gate is necessarily client-side — which reaches the rendered DOM and nothing else. Three surfaces sat outside it: **JSON-LD `offers.price`** (`seo/json-ld.ts`), the **OG images** (eight `og.tsx` renderers printing a ₹ amount into the pixels of a public image URL), and a pre-baked **`priceLabel`** string on the listing adapter that any component could render directly, bypassing the gate — and which had **zero readers**, so it was pure exposure. **Googlebot crawls signed out**, so once the page gates the price the crawler renders "Sign in to see price" while the markup declares a real amount; Google requires marked-up content to be visible to the user, so an undeclared gap risks the rich result. The resolution keeps `offers.price` (search value was the point) and declares the gate with paywalled-content markup — `isAccessibleForFree: false` + `hasPart.cssSelector` on a `WebPage` node, because `isAccessibleForFree` is a `CreativeWork` property and `Product` is not one. Serving prices to crawler user-agents but not to people would be cloaking, a harder violation, and is not done. **Three second-order traps, each of which bit during the work.** (a) **The `loading`-starts-true flash** — `SessionProvider` mounts `initialUser={null}`, so a two-state `!user → prompt` gate tells an already-signed-in buyer they are signed out for several hundred ms on every hard load; the gate has three states and the middle one renders a neutral placeholder. (b) **The selector is a cross-file contract with no type behind it** — `GATED_PRICE_CLASS` and the JSON-LD's `GATED_PRICE_SELECTOR` must agree and cannot import each other (`seo/` must not pull in a `"use client"` module, Root Cause #76), so a rename silently points the crawler at nothing, which reads exactly like no declaration at all. (c) **Hiding the number is not hiding the value** — a price-range facet brackets a hidden price in a handful of clicks and a price sort ranks every hidden amount on the page, so both are withheld from guests; `?f=price>N` and `?sort=price:asc` still work by hand and that residue is documented as accepted rather than quietly ignored. **The audit's own history is the lesson worth keeping.** `audit-guest-price-leak` found **17** sites where a hand-sweep had found 13 — Root Cause #84 again. Then refining its regex to stop flagging a bare `₹` used as a filter prefix silently introduced a FALSE NEGATIVE: `PrizeRevealModal` renders `Estimated value: ₹` with the value on the NEXT line, and a line-scoped rule sails past it. And R3 (offer-without-markup) passed its own negative control because a leftover `import { gatedPriceWebPageJsonLd }` satisfied a bare-name check with the markup deleted — **a check a name satisfies is decoration** (Root Cause #92), so it matches a CALL now. All four rules were verified by breaking them and watching them fail. **When you gate something in the UI, enumerate every surface that renders the same value WITHOUT going through the UI — metadata, images, adapters, captions, string props — and either strip it there or declare why it stays.** |
| 94 | **A browser timer is a recurring server bill — and an ISR diagnosis inferred from source rather than from the build manifest was wrong three times over** | Root-caused 2026-09-14 after the Vercel account was **paused mid-cycle** on Hobby limits (ISR Writes 1.3M/200K, Fluid Active CPU 23h36m/4h, Fast Origin Transfer 33.72GB/10GB, Function Invocations 1.1M/1M, Edge Requests 1M/1M) and had to be force-upgraded to Pro. **Two lessons, and the second is the more expensive one.** **(a) Timers bill.** `useNotifications` polled every 30s from `TitleBar` — i.e. on EVERY page — for an unread COUNT: 120 invocations/hour, per open tab, per signed-in user, wall-clock rather than activity-gated, so a tab left open overnight kept billing until closed. Worse, `/api/realtime/bids/[id]` holds an invocation open for its full 45s TTL, so an SSE connection is a **continuously-billed function, not a cheap socket** — and `useRealtimeBids` called `new EventSource` per hook, while `AuctionDetailPageView` mounts five components that each call it, so ONE auction page view pinned **five** concurrent held-open functions pushing byte-identical data. Neither reads as a cost in review: the diff is one number or one constructor. Fixed with a ref-counted URL-keyed channel map (one connection per URL, 1s close grace so a remount does not tear down and rebuild), a 5-minute interval plus `refetchOnWindowFocus`, and a `document.visibilityState === "hidden"` skip on the session ping with a `visibilitychange` refresh — which makes freshness *"fresh whenever you are actually looking at it"*, strictly better than a fixed tick that fires whether anyone is there or not. Enforced by `scripts/audit-client-poll-cost.mjs` (strict-zero): blocks a sub-60s `refetchInterval` LITERAL and any `new EventSource` outside the one module owning the channel. It judges only literals — `refetchInterval: someVar` is left alone, because an audit reports what it can prove and never what it guesses. **(b) The diagnosis was inferred, and inference lost to ground truth three separate times.** The plan asserted a combinatorial faceted URL space was regenerating constantly at `revalidate = 300`. Reality, from `.next/prerender-manifest.json`: `categories/[slug]/[tab]/sort/…` has **no `revalidate` at all** → cached **indefinitely**, so the "fix" of `86400` would have REGRESSED it; `stores/[storeSlug]/[tab]/sort/…` `await`s `searchParams` → **fully dynamic, no ISR entry**; the other two families are ~10-line `redirect()` stubs; and **nothing in the UI links into that space** (zero `href` containing `sort/` — the live UI uses `?sort=`), so a planned `rel="nofollow"` had no target. Then the headline fix — raise 34 short `revalidate` declarations — turned out **mostly inert**: only **FOUR** routes site-wide carry a short window (`/en`, `/en/lottery`, `/en/promotions`, `/en/sellers`), because 14 of those 34 files `await searchParams` or call `getServerSessionUser()`, which makes the route dynamic and the declaration meaningless, and **no `[slug]`/`[id]` content detail route appears in the manifest at all**. That last fact is the real finding: the CPU and invocation spend is **dynamic SSR**, not ISR. The 1.3M ISR-write figure remains **unattributed** — the routes that exist account for ~238K/month — and was deliberately left unexplained rather than guessed at; the per-route breakdown in Vercel Observability is the only way to close it. **🛑 Read `.next/prerender-manifest.json` before theorising about ISR.** `routes` = static/SSG, `dynamicRoutes` = on-demand ISR, and **absence from both means fully dynamic**. A `grep` for `export const revalidate` tells you what a developer *wrote*, not what Next *does* — and the two diverge the moment anything in the route touches a Request-time API. Same family as #84 (trust the rule's output over the list you typed), on the framework-semantics axis. |
| 95 | **Next sizes its build worker pool from `os.cpus()`, which reports the HOST core count and not the container's CPU quota — and then deletes `--max-old-space-size` from every one of those workers** | Root-caused 2026-09-13 while trying to fit the build into Vercel Hobby's 2 vCPU / 8 GB container. `getNumberOfWorkers()` (`next/dist/build/index.js`) picks the static-generation pool size from `experimental.cpus`, and `next/dist/server/config-shared.js` defaults that to `Math.max(1, os.cpus().length - 1)`. **`os.cpus()` does not read the cgroup quota**, so a 2-vCPU build container on a 32-core host asks for **31 worker processes**. Measured on this repo by evaluating the real function against the real loaded config: **15 workers**, and since `export/worker.js` defaults `staticGenerationMaxConcurrency` to **8**, that is **120 pages rendering simultaneously**. **And you cannot bound them by heap**: the pool is created with `isolatedMemory: true`, and `next/dist/lib/worker.js` then does `delete nodeOptions['max-old-space-size']` — the source comment says *"remove --max-old-space-size flag as it can cause memory issues"* — so each worker ignores NODE_OPTIONS and falls back to V8's default heap **sized off total system RAM**. `vercel.json`'s `--max-old-space-size=4096` therefore caps only the main process, which is not the process that multiplies. **Worker COUNT is the only control that exists.** Fixed with `experimental.cpus` + `staticGenerationMaxConcurrency` in `next.config.js` (120 → 4 concurrent renders), both env-overridable so a measurement can isolate one variable. **Two lessons beyond the fix.** (a) *The knob you are setting may not be the knob that matters* — a heap cap that is silently stripped reads exactly like a heap cap that is working. (b) **This was necessary but NOT sufficient, and only measurement showed that**: per-phase sampling put the compile phase at 6.5–8.4 GB against prerender's 1.8–3.0 GB, so the pool was never the binding constraint once capped. The compile cost lives in the MAIN process because Turbopack runs in a worker thread with `isolatedMemory: false`, sharing that address space — its Rust arena is invisible to both `--max-old-space-size` and `v8.getHeapStatistics()`. Anyone chasing build memory should measure the two phases separately (`scripts/probe-build-memory.mjs`) before touching a flag. |
| 96 | **n=1 is not a measurement — a per-run noise floor wider than the effect, and a harness that counted FAILED runs as data** | Same 2026-09-13 session, and it invalidated most of that session's own conclusions. Four Turbopack flags were measured one at a time against a cold baseline and three were reported as quantified regressions and one as a 1,744 MB win. Then a reproducibility run of a **byte-identical** config measured **6,563 / 8,398 / 8,426 MB** — a **1.9 GB spread, wider than every delta that had been attributed to a flag**. All four numbers were noise. **The tell was available and skipped**: no config had been run twice. **Two harness defects made it worse, and both flattered the results.** (a) *Failed runs were pooled into the statistics.* Two builds died mid-compile when a concurrent `npm install` re-extracted `node_modules/@mohasinac/appkit` underneath them (`Cannot find module .../dist/configs/index.js`) — and a build that dies during compile **never reaches prerender, so its peak is systematically LOWER**. A crash reads as a win. `summarize-build-memory.mjs` now excludes non-zero exits and lists them. (b) *No replicate grouping*, so a single run could be quoted as a result; it now prints the within-config spread as an explicit **noise floor** and warns when a between-config delta is smaller than it. **Three rules worth carrying.** (i) **Prefer the less noisy metric.** Wall time survived where peak RSS did not — Windows working set is trimmable and moves with system pressure, so it is a poor comparison metric even though it is the obvious one. (ii) **Measure on a quiet machine, or say you didn't.** Every run here competed with a parallel session doing npm installs and appkit rebuilds; that contention plausibly *is* the noise floor. (iii) **A structural proof beats a benchmark.** The one finding that survived — 15 workers, 120 renders — came from evaluating Next's own `getNumberOfWorkers()` against the loaded config, cost seconds, and has no error bars. Reach for that shape first. Same family as #83 (a green test asserting a fiction) and #87 (never trust an audit you have not seen fail), on the benchmark axis. |
...
| `export const dynamic = "force-dynamic"` on a page or layout | A `<Suspense>` boundary around the client subtree — **a boundary in the layout counts**, and every dashboard layout plus the root locale layout already has one. If the segment genuinely is per-request, declare that with a real dynamic API in its **server layout** (`await getServerSessionUser()`), which is how `admin/` and `store/` stay dynamic with zero segment configs between them. A segment config is a rendering contract, never a way to make a build error go away — 225 of these accumulated in 48 hours and 197 were no-ops. Enforced by `audit-no-force-dynamic.mjs`. Root Cause #89. |
| `export const revalidate = 0` / `fetchCache = "force-no-store"` / `unstable_noStore()` / `await connection()` under `admin/`, `store/`, `user/`, `wishlist/`, `auth/` | Nothing — delete it. These are `force-dynamic` by another spelling, and the same audit blocks them **inside those subtrees only**. Those routes are already per-request because their layout reads the session; opting out of caching again declares nothing. On a **public** page `revalidate = 0` is a legitimate "this data is live" declaration and is not blocked. |
...
| `<Text>{formatCurrency(price, currency)}</Text>` on any PUBLIC listing or detail surface | `<Text><GatedPrice>{formatCurrency(price, currency)}</GatedPrice></Text>`. Prices are hidden from signed-out visitors. Use `<PricesOnly>` for a SECONDARY amount beside an already-gated one (deposit note, handling fee, min increment, discount %), and `useCanSeePrices()` when the money lands in a plain STRING prop (a bottom-bar `infoLabel`, an OG caption, a lightbox `sub`) — a string cannot be wrapped in a slot. `formatCurrency` itself is deliberately NOT gated; admin/seller/cart/checkout/order surfaces use it legitimately. Enforced by `audit-guest-price-leak.mjs`. Root Cause #93. |
| `if (!user) return <SignInPrompt/>` for a price, read off `useAuth()` | `const { canSeePrices, isAuthResolving } = useCanSeePrices()` — THREE states. `SessionProvider` mounts `initialUser={null}` so `loading` starts `true` on every hard load for signed-in users too; a two-state gate flashes "Sign in to see price" at a signed-in buyer on every page load. Only `!canSeePrices && !isAuthResolving` is a confirmed guest. Root Cause #93. |
| A `₹` amount inside any `_internal/server/**/og.tsx` | Nothing — use a non-money accent. An OG image is a public image URL with the amount burned into the pixels: no sign-in gate reaches it and no paywall markup describes it. The auction and pre-order cards already use an end date and a release date. Enforced by `audit-guest-price-leak.mjs` R1. |
| Emitting `productJsonLd(...)` / `auctionJsonLd(...)` without `gatedPriceWebPageJsonLd(path)` beside it | Emit both. The markup keeps `offers.price` on purpose so search engines still see it, but Googlebot crawls signed out and renders the gated page — so the gap must be DECLARED (`isAccessibleForFree: false` + `cssSelector`) rather than left as an undeclared structured-data mismatch. Enforced by `audit-guest-price-leak.mjs` R3, which matches a CALL, not a leftover import. Root Cause #93. |
| Renaming `GATED_PRICE_CLASS` or `GATED_PRICE_SELECTOR` on one side only | Neither. They are two literals in two files that must agree, and cannot import each other (`seo/` must not pull in a `"use client"` module — Root Cause #76). A mismatch points the crawler at a selector matching nothing, which is indistinguishable from making no declaration at all. Enforced by `audit-guest-price-leak.mjs` R4. |
...
**🛑 Every PUBLIC read must call `hidePublicTestData(rows)`** (`_internal/server/features/tester/visibility.ts`) — it is `filterTestDataForViewer(rows, undefined)` under a name that says what it is for, because the two-argument form invites a call site to pass a viewer it does not have. Enforced by [`audit-public-test-data-leak`](scripts/audit-public-test-data-leak.mjs) (strict-zero), which scans `components/`, `actions/` and `_internal/server/features/` and ignores dashboards, where staff are supposed to see the sandbox. Measured live on 2026-09-01 right after a reseed: the homepage carried **44** sandbox mentions, /categories 21, /events 20, /bundles 6, /stores a rendered card for `store-tester-sandbox`. Only the products path had ever been filtered — every one of those pages had an API counterpart that filtered correctly, so the SSR first paint published the sandbox and the client refetch removed it, which is why nobody saw it. `ReviewDocument` and `CouponDocument` carry no `isTestData` field at all; both sites are marked with that reason rather than cast past.
...
### 🛑 `startPage` is where the tester LANDS — and it overrides `href`

The merge rule in `tester-checklist-seed-data.ts` is:

```ts
startPage: c.startPage ?? authored?.startPage ?? c.href ?? page.href
```

So an **overlay `startPage` beats the case's own `href`**. Two consequences that
have each already cost real coverage:

1. **Validate both fields, in both places.** `audit-tester-checklist-hrefs`
   validated `href` in the seed file only and reported `clean ✓ (981 hrefs
   checked)` while two cases carried `startPage: "/admin/support"` — a route with
   no `page.tsx`. Every automated run sent them to a 404 and recorded "could not
   test". It now scans `href` **and** `startPage` across the seed **and** its
   `authored/*.ts` overlays: 981 → **2,151** values, i.e. it had been checking
   under half of what it should (Root Cause #84 — a measurement narrower than the
   rule it feeds).
2. **A blanket overlay `startPage` silently erases per-case routing.**
   `buying__user-dashboard-extras.ts` set `startPage: "/user"` on six cases whose
   own `href`s pointed at `/user/catalogue`, `/user/prize-draws`,
   `/user/digital-codes`, `/user/returns` and `/user/reviews`. The overlay won, so
   all six landed on the dashboard index — and those five pages then read as
   **zero-coverage** in a gap sweep despite having cases written for them. Repointed
   2026-09-14; the fix cost nothing and recovered six pages.

**When adding an overlay, set `startPage` only when it genuinely differs from the
case's `href`.** Copying one value across a file is how six cases end up testing
the same page.

### Email cases — the RECORDER first, a real inbox only for real delivery

21 checklist cases are about email. Two instruments, and reaching for the wrong one
is how six of them become permanently unprovable.

**1. `emailEvents` — the send recorder. Use this for ~19 of the 21.**
`appkit/src/_internal/server/notifications/send-recorder.ts`, called from
`sendEmail()` on all three outcomes: `sent`, `suppressed` (carrying the rule that
stopped it) and `failed`. Firestore holds the record; `email_events` in RTDB is a
one-line ping so a poller need not spin on Firestore.

Why it lives in `sendEmail()` and not in `guardSend`: `SendGuardContext` carries
channel/feature/audience and deliberately **no recipient** — the guard's own
docstring explains it must not become the transport. `sendEmail()` is the only place
holding the recipient, the rendered body *and* the verdict.

🛑 **It is gated to the harness mailbox.** It writes only when a recipient equals
`TESTER_EMAIL_ID`, or when `EMAIL_RECORDER_ALL=true` is set for local debugging.
Recording real users' recipients and bodies would be a new PII store with no
retention story. Verified by direct test of `shouldRecordSend`: a real address
returns false, and an unset mailbox records nothing at all.

**Its decisive advantage is the absence cases.** Six cases assert an email should
*not* arrive (kill switch spares staff, opt-out honoured, no employee blast, losing
bidders get no mail). Over IMAP "nothing arrived" is unprovable — you wait N seconds
and hope, and a slow inbox is indistinguishable from a working suppression. Here it
is one query, and the row even names which rule fired.

Pruned by `cleanupRtdbEvents` at **1 hour**, deliberately longer than the 3/15/15
of its siblings: a batch can take ten minutes between triggering a send and
asserting on it, and a ping deleted mid-batch reads as "no email was sent" — the
exact false negative the record exists to prevent.

**2. `tester/scripts/lib/inbox.mjs` — real IMAP. Only for real delivery.**
Nothing else can prove Resend accepted and Gmail received. `findMessage` requires a
`sinceMs` captured *before* the action: the mailbox is shared across batches, and
"newest matching" will cheerfully return a message an earlier case left behind,
which reads as a pass.

#### 🛑 Credentials live in `tester/.env`, not `.env.local`

The app only ever *sends* mail (`resend`) and never reads it, so a live mailbox
credential has no business in the env the Vercel build sees.

| Var | Note |
|---|---|
| `TESTER_EMAIL_ID` | the mailbox |
| `TESTER_EMAIL_APP_PWD` | a Google **App Password** |

Two gotchas, both of which cost time when missed:

- **It is stored the way Google displays it** — four groups of four, 19 characters.
  The credential is the **16 without spaces**; always `.replace(/\s/g, "")`. Without
  that it fails `AUTHENTICATIONFAILED` and looks like a wrong password.
- **The account password will never work.** Gmail has rejected those for IMAP since
  2022. Enumerate the keys before picking one — `TEST_EMAIL_PWD` (13 chars, the
  account password) and `TEST_EMAIL_APP_PWD` once sat side by side, and reading the
  wrong one produced a confident, wrong "this is blocked" conclusion.

`tester/.gitignore` ignores `/.env` — **the rule was added before the file existed**,
because `tester/` is a submodule that ships via pointer bumps and an un-ignored
