...
**Payment**: `ManualPaymentProvider` (`appkit/src/providers/payment-manual/`) is the default — buyer pays via bank transfer/UPI outside the app and uploads a reference (UTR) + proof, which the seller/admin manually confirms (the existing `paymentProofUrl`/`paymentTransactionId` fields on `OrderDocument`). `RazorpayProvider` (`appkit/src/providers/payment-razorpay/`) remains fully implemented and registrable, gated behind `siteSettings.payment.razorpayEnabled` (default `false`). `MockRazorpayProvider` + `resolvePaymentProvider` still exist for dev/test — `siteSettings.featureFlags.useMockPayment`, same production-throw guard as before.
...
**Webhook simulation** in dev runs through the admin-only `POST /api/admin/dev/emit-payment-webhook` endpoint, which invokes the mock Razorpay provider's `emitWebhook` method. There is no shipping equivalent (nothing to simulate). New routes under `src/app/api/dev/**` are blocked by `audit-orphan-dev-routes`.
