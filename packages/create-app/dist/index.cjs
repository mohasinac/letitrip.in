...
function writeFeaturePageStubs(enabled, targetDir) {
  ...
  for (const key of enabled) {
    ...
    for (const seg of segments) {
      const filePath = (0, import_path.join)(targetDir, "app", seg, "page.tsx");
      const result = writeIfMissing(filePath, pageStub(key));
      log.push({ path: (0, import_path.join)("app", seg, "page.tsx"), result });
    }
  }
  return log;
}
...
// src/prompts.ts
var rl = (0, import_readline.createInterface)({ input: process.stdin, output: process.stdout });
...
async function selectOne(question, options, defaultValue) {
  ...
  return (_b = (_a = options[n - 1]) == null ? void 0 : _a.value) != null ? _b : defaultValue;
}
async function selectMany(question, options, defaults) {
  const defaultNums = defaults.map((d) => options.findIndex((o) => o.value === d) + 1).filter((n) => n > 0);
  console.log(`
  ${question}`);
  options.forEach((o, i) => {
  ...
  const picked = raw.split(",").map((s) => parseInt(s.trim(), 10)).filter((n) => n >= 1 && n <= options.length).map((n) => options[n - 1].value);
...
function collectEnvKeys(selections) {
  ...
  if (selections.search !== "none") add((_e = ENV_KEYS[`search.${selections.search}`]) != null ? _e : []);
  if (selections.payment !== "none") add((_f = ENV_KEYS[`payment.${selections.payment}`]) != null ? _f : []);
  if (selections.shipping !== "none") add((_g = ENV_KEYS[`shipping.${selections.shipping}`]) != null ? _g : []);
  return result;
}
...
function write(absTarget, relPath, content) {
  const full = (0, import_path2.join)(absTarget, relPath);
  (0, import_fs2.mkdirSync)((0, import_path2.dirname)(full), { recursive: true });
  (0, import_fs2.writeFileSync)(full, content, "utf-8");
}
function isEmptyDir(dir) {
  try {
    return (0, import_fs2.readdirSync)(dir).length === 0;
  } catch (e) {
    return true;
  }
}
function confirm(question) {
  return new Promise((resolve2) => {
    const rl2 = (0, import_readline2.createInterface)({ input: process.stdin, output: process.stdout });
...
async function main() {
  ...
  const absTarget = (0, import_path2.resolve)(process.cwd(), targetDir);
  if ((0, import_fs2.existsSync)(absTarget) && !isEmptyDir(absTarget)) {
  ...
  (0, import_fs2.mkdirSync)(absTarget, { recursive: true });
  console.log("\n  Scaffolding files\u2026\n");
  const created = [];
