...
/**
 * MediaPreviewCard Component (Library Version)
 *
 * Framework-agnostic media preview card for uploads.
 * Displays images/videos with upload status, progress, and actions.
 *
 * Features:
 * - Image and video preview
 * - Upload progress indicator
 * - Success/error status overlay
 * - Edit, remove, retry actions
 * - File metadata display
 * - Component injection for Image
 *
 * @example
 * ```tsx
 * <MediaPreviewCard
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-video bg-gray-100">
        {isImage && media.preview && ImageComponent && (
          <ImageComponent
            src={media.preview}
            alt={media.file.name}
            fill={true}
        ...
        {/* Status Overlay */}
        {isError && icons?.alertCircle && (
          <div className="absolute inset-0 bg-red-500/10 flex items-center justify-center">
            <div className="w-8 h-8 text-red-500">{icons.alertCircle}</div>
          </div>
        )}
        {isSuccess && !isUploading && icons?.checkCircle && (
          <div className="absolute top-2 right-2 bg-green-500 rounded-full p-1">
            <div className="w-4 h-4 text-white">{icons.checkCircle}</div>
          </div>
        )}
        ...
        {showActions && !isUploading && (
          <div className="absolute top-2 left-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            {onEdit && !isError && icons?.edit && (
              <button
                onClick={onEdit}
                className="p-1.5 bg-white rounded-md shadow-sm hover:bg-gray-50 transition-colors"
                title="Edit"
              >
                <div className="w-4 h-4 text-gray-700">{icons.edit}</div>
              </button>
            )}
            {onRetry && isError && icons?.rotateCw && (
              <button
                onClick={onRetry}
                className="p-1.5 bg-white rounded-md shadow-sm hover:bg-gray-50 transition-colors"
                title="Retry Upload"
              >
                <div className="w-4 h-4 text-gray-700">{icons.rotateCw}</div>
              </button>
            )}
          </div>
        )}

        {showActions && onRemove && !isUploading && icons?.x && (
          <button
            onClick={onRemove}
            ...
            title="Remove"
          >
            <div className="w-4 h-4 text-red-600">{icons.x}</div>
          </button>
        )}
      </div>
