...
/**
 * BulkActionBar component for batch operations on selected items
 * Shows desktop top bar and mobile bottom sticky bar
 * Supports confirmation dialogs and different action variants
 */
export function BulkActionBar({
  ...
  const handleActionClick = (action: BulkAction) => {
    if (action.confirm && ConfirmDialog) {
  ...
  const executeAction = async (action: BulkAction, input?: any) => {
    try {
      ...
      if (onError) {
        onError(error as Error, action.id);
      }
    } finally {
  ...
  // Default icons
  const XIconComponent = XIcon || (() => <span>✕</span>);
  const LoaderIconComponent = LoaderIcon || (() => <span>⟳</span>);
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t-2 border-blue-200 dark:border-blue-800 shadow-lg z-40">
        <div className="px-4 py-3 pb-safe">
          <div className="flex items-center justify-between mb-3">
            ...
            >
              <XIconComponent className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {actions.map((action) => {
              ...
              return (
                ...
                >
                  {actionLoading ? (
                    <LoaderIconComponent className="w-4 h-4 animate-spin" />
                  ) : (
                    Icon && <Icon className="w-4 h-4" />
                  )}
