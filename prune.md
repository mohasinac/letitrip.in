...
- **DONE (session 19 batch 5)**: type-check parser blocker fixed in `src/hooks/useMediaUpload.ts` (broken JSDoc around `useMediaCrop`) to restore `npx tsc --noEmit` as usable validation signal.
- **DONE (session 19 batch 6)**: products phase-1 wrapper migration applied in `ProductReviews.tsx`, `PreOrderDetailView.tsx`, `PromoBannerStrip.tsx`, `BidHistory.tsx`, and `PlaceBidForm.tsx`; repeated `flex items-center gap-*` / centered-stack bundles replaced with `Row`/`Stack` wrappers.
