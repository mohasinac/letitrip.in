...
| **32** | ✅ Done       | 2026-02-26 | 2026-02-26 | Products/checkout/search/auctions i18n wiring — `ProductsView` filter-chip labels (`filterCategory`, `filterMinPrice`, `filterMaxPrice`, `filterPriceValue`) converted from hardcoded strings to `t()`; `AuctionCard` fully wired to `useTranslations("auctions")` — `UI_LABELS.AUCTIONS_PAGE.*` eliminated, `formatCountdown` updated to accept `endedLabel` param, `totalBids` ICU plural used; `auctions/page` `filterBidRange` label translated; `checkout/success` raw `fetch()` replaced with `orderService.getById()`; `auctions.filterBidRange`, `auctions.endingSoon`, `products.filterCategory/filterMinPrice/filterMaxPrice/filterPriceValue` keys added to `en.json` + `hi.json`; 0 TS errors.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |                                |
| **33** | ✅ Done       | 2026-02-26 | 2026-02-26 | Static/content pages i18n wiring — `promotions/page` converted from `UI_LABELS.PROMOTIONS_PAGE` + hardcoded `"🎉 Exclusive Offers"` to `useTranslations("promotions")`; `promotions` top-level namespace added (`title`, `subtitle`, `exclusiveOffersBadge`, `dealsTitle/Subtitle`, `featuredTitle/Subtitle`, `couponsTitle/Subtitle`, `emptyDeals`, `checkBack`, `emptyCoupons`) to `en.json` + `hi.json`; `about`, `terms`, `privacy`, `sellers`, `help`, `contact`, `blog`, `blog/[slug]` already wired in prior sessions; 0 TS errors.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |                                |
| **34** | ✅ Done       | 2026-02-26 | 2026-02-26 | Seller portal pages i18n wiring — `seller/products/[id]/edit` hardcoded validation strings (`"Title is required"`, `"Description is required"`, `"Category is required"`) replaced with `t("titleRequired")` / `t("descriptionRequired")` / `t("categoryRequired")`; keys added to `sellerProducts` namespace in `en.json` + `hi.json`; all other seller pages (`page`, `analytics`, `orders`, `payouts`, `products`) already wired in prior sessions; 0 TS errors.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |                                |
...
| **37** | ✅ Done       | 2026-02-28 | 2026-02-26 | Service Layer Migration — all 37.1–37.14 complete. New services: `bidService`, `contactService`, `mediaService` (added to `@/services` barrel). apiClient violations eliminated from: `Sidebar.tsx` (→ `authService.logout`), `FAQPageContent.tsx` (→ `faqService.list`), `FAQHelpfulButtons.tsx` (→ `faqService.vote` + `useTranslations("faq")`), `PromoCodeInput.tsx` (→ `couponService.validate` + `useTranslations("cart")`), `PlaceBidForm.tsx` (→ `bidService.create` + `useTranslations("auctions"/"actions"/"loading")`), `AuctionDetailView.tsx` (→ `productService.getById` + `bidService.listByProduct`), `ContactForm.tsx` (→ `contactService.send` + `useTranslations("contact")`), `CheckoutView.tsx` (→ `addressService`/`cartService`/`checkoutService`), `ImageUpload.tsx` (→ `mediaService.upload`), `user/notifications/page.tsx` (→ `notificationService`), `user/settings/page.tsx` (→ `profileService.update`). All page.tsx files ≤ 150 lines. i18n keys added: `auctions.minimumBidError`, `auctions.bidFailed`, `auctions.auctionEndedInfo`, `auctions.loginToBid`, `auctions.yourBidLabel`, `cart.promoInvalid`, `cart.promoCode`, `cart.promoApply`, `cart.promoRemove`, `cart.promoPlaceholder`, `faq.wasThisHelpful`, `faq.thanksForFeedback`, `contact` form keys (12). `couponService.validate` param fixed (`total`→`orderTotal`). `faqService.vote` signature fixed to `{ vote: "helpful" | "not-helpful" }`. 0 TS errors. |
| **38** | ✅ Done       | 2026-02-28 | 2026-02-28 | Page Extraction batch 1 — `blogService.list/getBySlug()` added to `@/services`. `sellerService.listProducts(uid)` added. `AuctionsView.tsx` extracted from `auctions/page.tsx` (192→13 lines) using `productService.listAuctions()`; raw `fetch()` replaced. `UserOrdersView.tsx` extracted from `user/orders/page.tsx` (206→13 lines) using `orderService.list()`. `blog/page.tsx` queryFn migrated to `blogService.list()`. 0 TS errors.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **39** | ✅ Done       | 2026-02-28 | 2026-02-28 | Page Extraction batch 2 — `SellerOrdersView.tsx` extracted from `seller/orders/page.tsx` (215→5 lines) using `sellerService.listOrders()`. `SellerProductsView.tsx` extracted from `seller/products/page.tsx` (326→5 lines) using `sellerService.listProducts()`; circular import fixed via relative paths. i18n keys: `orders.tabAll/Pending/Confirmed/Shipped/Delivered/Cancelled`. 0 TS errors.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **40** | ✅ Done       | 2026-02-28 | 2026-02-28 | apiClient elimination — seller pages: `seller/analytics` (→ `sellerService.getAnalytics()`), `seller/payouts` (→ `sellerService.listPayouts()`, `sellerService.requestPayout()`), `seller/page` (→ `sellerService.listProducts(uid)`). Removed `API_ENDPOINTS` import from `seller/page`. 0 TS errors.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **41** | ✅ Done       | 2026-02-28 | 2026-02-28 | apiClient elimination — admin pages: `admin/media` (→ `mediaService.crop/trim()`), `admin/site` (→ `siteSettingsService.get/update()`), `admin/orders` (→ `adminService.listOrders/updateOrder()`). `adminService.listOrders()` and `updateOrder()` added to `admin.service.ts`. 0 TS errors.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **42** | ✅ Done       | 2026-02-28 | 2026-02-28 | apiClient elimination — user address + track pages: `user/addresses/add` (→ `addressService.create()`), `user/addresses/edit/[id]` (→ `addressService.getById/update/delete()`), `user/orders/[id]/track` (→ `orderService.getById()`). 0 TS errors.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **43** | ✅ Done       | 2026-02-28 | 2026-02-28 | apiClient elimination — `user/profile` (→ `orderService.list()` + `addressService.list()`), `user/wishlist` (→ `wishlistService.list()`), `promotions` (→ `promotionsService.list()`). New `promotionsService` created and exported from `@/services`. 0 TS errors.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **44** | ✅ Done       | 2026-02-28 | 2026-02-28 | apiClient elimination — `blog/[slug]` (→ `blogService.getBySlug()`), `products/[slug]` (→ `productService.getById()`, removed unused `API_ENDPOINTS`), `seller/products/[id]/edit` (→ `productService.getById/update()`). 0 TS errors.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **45** | ✅ Done       | 2026-02-28 | 2026-02-28 | apiClient elimination — `cart` (→ `cartService.get/updateItem/removeItem()`), `events` (→ `eventService.list()`), `events/[id]` (→ `eventService.getById()`), `events/[id]/participate` (→ `eventService.getById/enter()`). **Rule 20 FULLY COMPLIANT — zero `apiClient` direct calls in any `page.tsx` under `src/app/[locale]/`.** 0 TS errors.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
