...
Static RSC guide for the make-an-offer flow. 6-step `FlowDiagram`: search product → make offer → negotiate (counter/accept) → checkout → cancel/refund.
...
Static explainer for the three-step checkout: address selection + Consent OTP, order review, payment (Razorpay / COD).
...
Static explainer: verified-purchase gate, moderation flow, and helpful voting.
