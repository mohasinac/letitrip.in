...
**Actions:** `admin.actions.ts` (mutations), `admin-read.actions.ts` (reads), `admin-coupon.actions.ts`, plus domain-specific actions
...
_Removed — RC virtual currency system was fully deleted._
...
`BidFilters` · `BlogFilters` · `CarouselFilters` · `CategoryFilters` · `CouponFilters` · `EventEntryFilters` · `FaqFilters` · `HomepageSectionFilters` · `NewsletterFilters` · `NotificationFilters` · `PayoutFilters` · `SessionFilters` · `StoreFilters` · `UserFilters`

---

## Admin Hooks (`features/admin/hooks/`)

### `createAdminListQuery` Factory

All paginated admin list hooks are generated by a shared factory that eliminates the repeated URLSearchParams parsing + `useQuery` boilerplate.

```ts
import { createAdminListQuery, extractBasicMeta } from "@/features/admin/hooks";

export const useAdminOrders = createAdminListQuery<
  OrderDocument,
...
Exports: `createAdminListQuery<TItem, TResult>()`, `extractBasicMeta()`, `AdminListMeta` type.

### Hook Reference

| Hook                            | Factory? | Description                 |
| ------------------------------- | -------- | --------------------------- |
...
| `useAdminFaqs(sieveParams)`     | ✅       | Paginated FAQs              |
