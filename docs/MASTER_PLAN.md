...
**~~One improvement worth making:~~** ✅ Fixed 2026-03-10 — `createApiHandler` now attaches `RateLimit-Limit`, `RateLimit-Remaining`, `RateLimit-Reset` headers to all responses from rate-limited routes (429 and successful), so clients can track their quota.
...
> **G1 ✅** committed 2026-03-10 — all mutations converted to Server Actions; Rule 20 violations fixed; pure-passthrough service methods deleted (H3 ✅). Final actions: `createReviewAction` (self-resolves product/user), `adminUpdateReviewAction`, `adminDeleteReviewAction`. G1 complete.
...
All previously pending files have been deleted. See "Already deleted / moved ✅" above.
