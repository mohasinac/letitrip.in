...
function isServerFull() {
  ...
  console.log(
    `Server status: ${totalPlayers}/${MAX_PLAYERS} players, ${totalRooms}/${MAX_ROOMS} rooms`,
  );
...
function calculateServerCollisionDamage(bey1Data, bey2Data) {
  ...
  const L1 = Math.abs(
    calculateAngularMomentum(bey1Data.mass, bey1Data.radius, omega1),
  );
  const L2 = Math.abs(
    calculateAngularMomentum(bey2Data.mass, bey2Data.radius, omega2),
  );
  ...
  // Attack multipliers
  const bey1AttackMultiplier = bey1Data.ultimateAttackActive
    ? 2.0
    : bey1Data.heavyAttackActive
      ? 1.5
      : 1.0;
  const bey2AttackMultiplier = bey2Data.ultimateAttackActive
    ? 2.0
    : bey2Data.heavyAttackActive
      ? 1.5
      : 1.0;
  ...
  const chargeDashMultiplier =
    bey1Data.isChargeDashing || bey2Data.isChargeDashing ? 1.3 : 1.0;
  const knockbackMagnitude1 =
    (collisionForce / bey1Data.mass) * massRatio1 * chargeDashMultiplier;
  const knockbackMagnitude2 =
    (collisionForce / bey2Data.mass) * massRatio2 * chargeDashMultiplier;
  ...
  return {
    ...
    spinSteal2,
    knockback1: {
      x: -normalX * knockbackMagnitude1,
      y: -normalY * knockbackMagnitude1,
    },
    knockback2: {
      x: normalX * knockbackMagnitude2,
      y: normalY * knockbackMagnitude2,
    },
    collisionForce,
    timestamp: Date.now(),
  };
  ...
  socket.on("collision-detected", (collisionData) => {
    ...
    if (playerData) {
      ...
      if (room && room.players.length === 2) {
        ...
        const serverCollisionResult = calculateServerCollisionDamage(
          bey1Data,
          bey2Data,
        );
        ...
        console.log(
          `[SERVER] Collision: P1 dmg=${serverCollisionResult.damage1.toFixed(1)}, P2 dmg=${serverCollisionResult.damage2.toFixed(1)}, force=${serverCollisionResult.collisionForce.toFixed(1)}`,
        );
      }
    }
  });
