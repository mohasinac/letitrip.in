...
**NEW**: Strict TypeScript with Frontend/Backend type separation

### Core Principles

- **Zero `any` types** - Strict type checking enforced everywhere
- **FE/BE Separation** - Frontend types optimized for UI, Backend types match API
- **Service Layer Transformation** - Automatic conversion between FE and BE types
- **Validation** - Field-level validation with errors shown below inputs
- **Persistent Actions** - Save/Create buttons always visible in forms
...
/types
  ...
  /shared      - Common types (Status, Role, enums)
...
```typescript
// Service layer transforms automatically
...
const ProductCard: React.FC<{ product: ProductFE }> = ({ product }) => {
  return <div>{product.formattedPrice}</div>; // ₹1,999
};
```

### Documentation

- **[TYPE-MIGRATION-GUIDE.md](./TYPE-MIGRATION-GUIDE.md)** - Examples and patterns
- **[TYPE-REFACTOR-PLAN.md](./TYPE-REFACTOR-PLAN.md)** - Implementation plan
- **[TYPE-SYSTEM-STATUS.md](./TYPE-SYSTEM-STATUS.md)** - Current status
