...
---
### S-checkout-extras — checkout grows an Add-ons & fees step; four silent gates opened (2026-08-26)

**Trigger**: "why does the checkout still not show the add ons, and other stuff", then "i should be enable for all types of orders, even those who bypass cart, they shoud eid tthe addons in the chkout, as well as the ones in the cart edit is there, like in the checkout there should be a add on and other fees page".

**Four independent gates, none of which errors.** The add-ons feature shipped whole on 2026-08-21 and had been invisible ever since. (1) All three `*FeeEnabled` flags default to `false`, and `StoreAddonsPicker` hard-returns `null` when none is set — so the picker was absent on the cart *and* the checkout on any default install, which alone accounts for the entire report. The plan said the defaults lived in two places; they lived in **four** (`DEFAULT_SITE_SETTINGS_DATA`, the seed, **`CHECKOUT_DEFAULT_COMMISSIONS`**, and the admin editor's `?? false`). (2) `addonStores` was derived only from `pricingPreview.stores`, and the preview was `enabled: step === "payment"` — so the controls could not exist before the payment step and popped in ~300ms+RTT after. (3) `usePricingPreview` discarded both non-2xx (`if (!res.ok) return;`) and thrown errors, so a failed request made the whole block vanish with no placeholder — the cart degrades visibly here, checkout did not. (4) Shipment protection additionally requires `storeSubtotal > 0`, which only ever arrived with the preview.

**There is no cart-bypass checkout, and that was the good news.** Product Buy Now is add-to-cart-then-navigate (`ProductDetailActions.tsx:85`); auction Buy Now writes a real cart line with a real `storeId` (`bid-actions.ts:431`). Offer and auction lanes carry `storeId`, `previewCheckoutPricing` already computes add-on fees for **every** lane, and the add-ons block never had a lane guard. So "works for orders that skip the cart" needed no new plumbing — only the four gates opened.

**The step.** `address → extras → payment`, with `value-otp` and `processing` sharing payment's index because they are interstitials *within* it, not stages. Back is scoped to `extras → address` and `payment → extras` only — safe because add-on toggles persist to the cart document on every change, so stepping back loses nothing and costs no server call; deliberately unreachable from `processing` (an order is in flight) and `value-otp` (a code has been sent). The seller list comes from **cart items**, not the preview, which is what makes the step interactive at 0ms and what stops a failed request from erasing it.

**Three live bugs found on the exact lines being changed.** `bottomActions` ended in an unguarded `return [PAY_ONLINE]` reached by every step that wasn't address/value-otp/processing — and since `showRazorpay` defaults to **false**, the mobile bar was offering "Pay Online (Razorpay)", wired to the Razorpay handler, on a site where Razorpay is switched off. It is now an explicit per-step switch that picks a CTA matching an *enabled* method, and returns `[]` rather than a plausible-looking wrong one. `cartData.subtotal` comes from `cartRepository.getSubtotal`, which ignores `selectedItemIds`, `lockedPrice` and lanes alike, so a partially-selected cart quoted above what it charges. And two `as unknown as` casts meant the file had no type-level knowledge of the cart at all — a renamed field would have compiled and priced everything at zero.

**`null` was doing four jobs.** It meant not-started, in-flight, empty-lane and failed, and callers picked whichever reading suited them. The visible consequence was a hard **₹0.00 Total**: the server returns a fully-zeroed `EMPTY_PRICING_PREVIEW` for an empty lane, which is **truthy**, so `preview ? preview.total : fallback` rendered the zero confidently. `usePricingPreview` now returns a `status` (`idle`/`loading`/`ready`/`error`) and keeps the last good `preview` on a refetch failure — a stale real number plus a warning beats blanking to zero. Resolved client-side on purpose: `CheckoutPricingPreview` is mirrored in three files, so a wire-format discriminator would have been a three-file Root Cause #20 change for something `status` already answers.

**One pricing rule, not a fourth copy.** The checkout was about to hand-roll `(lockedPrice ?? price) * quantity` — the same shortcut that has already been got wrong three times (Root Cause #75). Instead `unitPriceFor`'s parameter was widened to a structural `PricedCartLine` (narrower than `CartItemDocument`, which a client cannot satisfy — `checkoutDeadline` is a `Date` on the document and a string after `JSON.parse`), and a `clientLineTotal()` delegates to `lineTotalFor(line, null)`. Five test cases pin the branches that matter: a bundle charges its own discounted price rather than the sum of its members, a group line sums members weighted by per-member quantity, and `lockedPrice` beats the listing price.

**`?lane=` is now read, and deliberately not trusted.** `AUCTION_CHECKOUT_URL` deep-links from a settlement email; if that win has since been paid or forfeited the buyer landed on an ordinary standard-lane checkout with no explanation at all. It is validated against `CART_LANE` and used solely to render a mismatch alert. It is **not** fed to the preview — `assertCheckoutLane()` refuses anything but the active lane at placement, so a URL-supplied lane would quote a payable-looking total for a lane the order will be refused on. The derived `checkoutLane` is passed explicitly instead, so client and server agree rather than defaulting independently.

**Not `SellerGroupSection`.** CLAUDE.md's "per-store detail belongs on the seller card via `SellerGroupSection`'s `renderFooter`" describes the *cart*; that component is private to `CartRouteClient` and carries selection checkboxes, quantity steppers, remove and move-to-wishlist — every one a cart mutation meaningless at checkout, each forcing a re-preview round trip. The extras step has a purpose-built read-only card. The aggregate `<CartPriceBreakdown>` in the Order Summary is unchanged.

**A false finding, caught before commit, worth more than the fix it wasn't.** This session reported twice that project tooling was missing — `check:lint`, `npm run audit:all`, `scripts/run-audits.mjs`, nine named audit scripts — and got as far as writing all of it into `crud-tracker.md`, `newchange.md`'s DEFERRED table and `prompt.md`'s NEXT UP list. **Every one of them exists.** The Bash tool's cwd persists between calls, an earlier `cd appkit` was still in effect, and the greps were reading `appkit/package.json` and `appkit/scripts/`. Two "independent" findings were one stale `cd`. The rows were corrected before the commit; the lesson is `pwd` before concluding absence, because a negative from a path-relative command is only as good as the directory it ran in.

**Files**: new `appkit/src/features/cart/utils/line-total.ts` (+ its test). Changed: `CheckoutRouteClient.tsx` (extras step, typed `CheckoutCartLine`, cart-derived `addonStores`, `bottomActions` switch, lane alert), `usePricingPreview.ts`, `StoreAddonsPicker.tsx` (`hasAnyStoreAddon` hoisted + exported), `CartPriceBreakdown.tsx` (`errorNote`), `CartRouteClient.tsx` (two call sites), `order-math.ts` (`PricedCartLine`), `action-registry.ts` + `action-defs.ts` (`continue-to-payment`, `back`), `constants/ui.ts` (3-step labels + 13 new strings), the three default sites, and `checkout/actions.ts` (`EMPTY_PRICING_PREVIEW` returned by value).

**Gates**: appkit `tsc` clean, audits pass, eslint **0 errors** in both repos, 54 cart tests + 20 money tests green. Two failures verified **pre-existing and not from this session** — `src/actions/review.actions.ts` arity (a concurrent session is mid-refactor of the helpful-vote to key on the voter) and a `BaseRepository` mock gap in `checkout/__tests__/actions.test.ts`, which fails identically with this session's one-line change stashed.

**Not done — needs a human**: the three flags must be toggled once at Admin → Site Settings → Commissions, because flipping a *default* does not touch an existing `siteSettings/global`. No visual pass (Rule #10).

---
### S-availability-scope — one definition of "available"; homepage stops advertising dead listings (2026-08-24)

**Trigger**: "in the homepage we see lots ended or sold listings even for the ones which say live or in stock", then "make a separate tab for sold items … this way it is easier for users to look in the sold items", then "expand scope to dashboards too that way its easier for users to sort the content, and for orders too".

**Four partial definitions of "available", none of them the definition.** The listing pages said `inStock` + a `dateFrom` cutoff; the homepage said nothing at all; related-items had a private `isValidRelatedItem()` in `_internal/server/features/products/data.ts` — the only per-type version, and the only place three bugs could hide: it read `codesAvailable` at the **top level** when the schema nests it under `digitalCode.codesAvailable` (so that branch had **never once fired**), it excluded `status` values — `"sold"`, `"out_of_stock"`, `"discontinued"` — that are **not members of `ProductStatus`**, and it had no branch at all for classified / live / art / stickers. There is now one: `isListingRowAvailable()` = shared `baseAvailable()` + a per-type `isAvailable`, both **required** fields on `ListingTypePlugin`, so a tenth listing type cannot compile without answering the question.

**The reported homepage bug, exactly.** `getFeaturedAuctions` was `listingType==auction,status==published` sorted `auctionEndDate ASC` **with no lower bound** — and ascending order front-loads the *oldest* end dates, so the "Live Auctions" strip led with the longest-dead lots in the catalogue. Nothing flagged it because an ended auction renders as a perfectly ordinary card. Two more layers had to move or the fix would have been undone: the three CSR hooks hand-wrote pre-encoded `filters=` strings with no liveness bound and **replace** the SSR paint after 30s; and `PrizeDrawsSection` ignored its own `revealStatus` config (seeded `"all"`), plus `maxItems`/`storeId`/`showCountdown`/`showEntriesRemaining`, with `limit` hardcoded to 8.

**The feature-flag gate existed and was unreachable.** `SECTION_FEATURE_GATE` lived inside `getHomepageInitial`, a function with **zero call sites** — so `auctions: false` hid auctions everywhere *except* the homepage, which kept advertising them. Moved to `_internal/shared/features/homepage/section-gate.ts` and actually applied. The rest of that module was deleted rather than wired: its only other unique calls, `findActiveAuctions()`/`findActivePreOrders()`, are unbounded `.get()` collection scans (Rule #6), and both were removed with it.

**Toggles → a three-tab scope.** `Available` / `Sold & Ended` / `All` replaced the `showSold`/`showEnded`/`showClosed` trio, which *widened* the list instead of scoping it — live and dead rows arrived mixed and the archive was unbrowsable. The middle label is derived from each plugin's `hideDefault`, so `/auctions` reads "Ended" and `/art` reads "Sold". Orders got the same affordance with their own vocabulary — `Active` / `Closed` / `All`, with `return_requested` deliberately **Active** because it is the seller's next action item.

**Availability is asymmetric, and that is the whole design.** "Unavailable" is *sparse* and expressible as an OR of **equalities** (sold out / ended / closed / depleted), so the query runs one bounded fetch per distinct clause in parallel and merges — five queries, 250 docs worst case across all nine types. "Available" is *dense* and **negation**-shaped, which no query expresses, so it stays a per-row predicate over one bounded window. Every clause must be an equality **or** name its own `sortField` (only auction's does), because Firestore appends an inequality's field to the `orderBy` implicitly — Root Cause #59. Ended auctions sorted by end date therefore get a real pushdown and unbounded-depth pagination through the archive.

**`truncated` exists because the old code lied.** Whenever the in-memory path ran, it reported the size of its own 50-row window as `total`. It is now a declared floor: render "50+", and `totalPages` is `page + 1` so the pager offers Next rather than claiming a false last page.

**Bugs found by sweeping rather than by the report**: `/products` leaked ended auctions into its default view (`dateFrom` only applied when exactly one type was selected); `PrizeDrawsIndexListing` filtered closed draws **after** pagination, silently shrinking pages below their size; the six store tabs sent no filter at all while their SSR half did (Root Cause #30, live); `SELLER_ORDER_STATUS_TABS` omitted `confirmed` — a status the seller's own Update-Status dropdown could **set** — and `return_requested`; `/api/user/orders` assigned a page-local count to `total`, so page 2 of the default "Normal" lane was unreachable; `SellerOrdersView`'s badge map was UPPERCASE-keyed against lowercase stored values and covered 6 of 9; the appkit package's `features/products/api/route.ts` was a second, drifted copy of the listing query whose safelist omitted `auctionEndDate`/`prizeRevealStatus`; and two seed fixtures contradicted themselves (`preorder-…-bx-11-sold-out` with `isSold: false`, `digitalcode-…-app-sold-out` with `stockQuantity: 15` against a drained pool).

**Seed**: every listing type now carries a sold/ended fixture, since the new scope is otherwise empty and untestable on most pages. Three are deliberately *non-canonical* because they are the only rows that prove the per-type branch does the work rather than the shared checks — `auction-…-lord-spryzen-ended-unsold` (ended, `isSold: false`), `prizedraw-…-vault-closed-pending-reveal` (closed, **stock remaining**), `digitalcode-…-launch-codes-depleted` (**nested** `codesAvailable: 0`, stock 5). All stay `published`: an archived row is hidden by the status filter before the availability predicate ever runs. The `products` count row in CLAUDE.md said **30**; the real number is **69** after this session.

**Two audit traps worth knowing.** (1) `audit-listing-indices` and `audit-registry-constants` both parse **comment text**: a backticked range clause in an explanatory comment reads as a real range filter, and a backticked `.where(...)` call reads as a raw field string. Both cost a debug cycle. Don't write clause-shaped or call-shaped prose in backticks. (2) This repo bans suppression markers outright (`audit-no-suppression-comments`), so three `audit-unknown-ok` markers had to become a real fix — `Record<string, unknown>` → the project's sanctioned `FirestoreDocument`/`FirestoreValue` aliases, which is better code anyway.

**Files**: new `_internal/shared/listing-types/availability.ts`, `features/products/components/AvailabilityTabs.tsx`, `features/products/hooks/useAvailabilityScope.tsx`, `features/orders/constants/order-scope.ts`, `features/orders/components/OrderScopeTabs.tsx`, `features/homepage/hooks/homepage-query.ts`, `_internal/shared/features/homepage/section-gate.ts`. Rewritten: `list-public.ts` (three execution paths), `product-actions.ts`'s three `getFeatured*`, all three homepage CSR hooks, `PrizeDrawsSection`. Migrated: 11 public listing components, 5 SSR views, 14 dashboard views, 3 order views, 6 API routes. Deleted: `_internal/server/features/homepage/data.ts` and the two unbounded repository scans. `buildListingTypeListingConfig` gained a `portal` option so the five hand-written seller per-type views stop being a sixth copy. +26 Firestore indexes.

**Gates**: both repos `tsc` clean, lint 0 errors, 21/21 `product.actions.test.ts`, 116 audits run. `audit-listing-filter-parity` gained a `CLIENT_OMITS_AVAILABILITY` check over all 11 client listing components plus banned tokens for the old client-side default — that omission is Root Cause #30 in its purest form and is the most fragile thing about this design.

**Not done — needs explicit approval**: appkit is pinned to `^4.13.1`, so none of this reaches the app without a publish (`node_modules/@mohasinac/appkit/dist` was manually resynced to verify types — `npm install` reverts it); the 26 new indexes must be deployed before the Sold & Ended tab works; and the homepage needs a human watching **~35s past first paint** so the CSR refetch lands and the strip is confirmed not to change contents.

**Blocked on a concurrent session**: `npm run check` does not exit 0. Four audits fail entirely in files this session never touched — `hex-tokens` + `tester-checklist-hrefs` both trace to that session moving category ids into `seed/_helpers/category-forest.ts`, a subdirectory the href audit's non-recursive scan cannot see; `variant-prop-coverage` (`BundleCollage.tsx`) and `unknown-leakage` (`_internal/shared/history/*`, `orders.repository.ts`) are likewise theirs. Left alone deliberately — whether that audit should recurse into `_helpers/` is their call, not a decision to make inside someone else's mid-refactor.

---
### S-bottom-edge — the bottom edge becomes three declared tiers (2026-08-24)

**Trigger**: a screenshot of a product page with the Back-to-top pill sitting on top of the "Buy Now" button — "can we make the back to top be bottom aware?" Then, during planning: "does this consider toolbars, bottom nav, checkouts and other CTA?", "this is on all pages", "some items only show up after scrolling", "can we do it like this — back to top / the cta row (or anything else which is not nav) / bottom nav", and "make sure all of footer is visible above this container to avoid unseeable contents".

**One cause, four symptoms.** `BottomNavLayout` published `--bottom-nav-height` and **nothing else on the bottom edge published anything**, so every surface above the CTA bar hardcoded a guess at its height, and each guess was wrong differently:

1. **BackToTop cleared only the nav** — the reported bug, and app-wide rather than product-specific: 25 call sites register into the single `BottomActions` instance, including `DataListingView` (~70 dashboard screens). On desktop the bar's reveal threshold (`DESKTOP_REVEAL_SCROLL_PX`) is **400** — the same as `BackToTop`'s own `threshold` — so both appear on the same scroll tick and the overlap was guaranteed there, not incidental.
2. **The mobile pagination bar was fully invisible** on any listing page with a page-level CTA. Its `--default` modifier resolved to the *identical* `bottom` as the CTA bar at `z-index: 39` vs its 40. Its `--bulk` variant only engaged on `selectedCount > 0` and hardcoded `3.5rem` — a copy of the bar's `h-14` main row, stale the moment an info or secondary row rendered.
3. **The footer's last rows sat under the chrome.** `<Main>` carried `mb-28`/`mb-16`/`lg:mb-24`/`lg:mb-0` from a boolean a code comment already flagged as needing manual sync with `isVisible` — **and it was on the wrong element**. `FooterLayout` is a sibling rendered *after* `<Main>`, so that margin only gapped main-to-footer; the copyright / bottom-links strip was never cleared.
4. **Dashboard mobile had two tab bars on identical pixels.** `DashboardBottomNav` hand-rolled its own `fixed bottom-0` at `z-bottom-nav`, and `BottomNavbar` was rendered ungated in `AppLayoutShell`. The dashboard one lost on DOM order and was invisible; worse, it published no height, so the entire stack above it was positioned against the *other* nav's.

**The fix — three tiers, three variables, three writers.** New `<BottomChrome>` (`appkit/src/features/layout/BottomChrome.tsx`) is a fixed flex column between the nav and BackToTop; its own `offsetHeight` *is* the sum of its children, published by one `ResizeObserver` as `--bottom-chrome-height`. Consumers read one variable, so a bar changing height, revealing on scroll, entering bulk mode, or being added years from now is covered with zero edits. **Deliberately not one var per bar** — that shape fails the moment someone adds bar number three without also editing every consumer's `calc()`.

**What made the container work**: `BottomActions` hid with `translate-y-full`, a transform that leaves layout height behind — inside a measuring container that would have reserved ~3.5rem of tier on every page with no CTA. It now collapses with `grid-template-rows: 1fr → 0fr` (no magic max-height clamp, honest height at every frame, so BackToTop glides down in step). Background/border/shadow had to move *inside* the clipper: on the collapsing box they still paint a hairline and a shadow smudge at zero height. And the container must never get `overflow: hidden` — `BulkPickerPanel`/`InfoPanel` are `absolute bottom-full`, deliberately outside the measured flow (transient tap-to-open overlays, not persistent bands) and clipping would erase them.

**Surfaces confirmed NOT affected**, so nothing was changed for them: checkout/cart/wishlist CTAs are the same `BottomActions` instance (no separate bar exists); every toolbar is top-anchored (`StickyToolbar` offsets are all `header*`, `BulkActionBar` is `position: relative`); drawers/modals/sheets are contained or full-viewport and only *consume* `--bottom-nav-height`; `Toast` is top-anchored on mobile. `FormActionBar` and `.appkit-formshell__footer` are `sticky` and consumerless — noted in CLAUDE.md that a sticky bar consumes the tier and never joins it, since no `ResizeObserver` can see a stick transition.

**Files**: new `BottomChrome.tsx` + `ui/components/bottom-chrome-slot.ts` (in `ui/` because `features/layout` imports `ui` — the reverse would close a cycle through the barrel). Changed `BottomActions.tsx`, `BackToTop.tsx`, `FooterLayout.tsx`, `AppLayoutShell.tsx` (+ `showBottomNav` prop), `ListingLayout.tsx`/`.style.css`, `DashboardLayoutClient.tsx`, `LayoutShellClient.tsx`. Removed: `Z_BOTTOM_ACTIONS`, `hasBottomActions`/`hasDesktopBottomActions`, three pagination `bottom` modifiers, one hardcoded `z-index: 39`.

**Gates**: both repos `tsc` clean, lint 0 errors, all layout audits clean (`bottom-offset`, `sticky-offsets`, `html-wrappers`, `inline-styles`, `code-quality`, `theme-drift`, `primitive-child-wrappers`), appkit + `functions/lib` rebuilt. The linked `node_modules/@mohasinac/appkit` copy had drifted badly (Root Cause #28) and was manually resynced — worth checking before trusting any consumer typecheck.

**Not done**: no visual pass. `npm run dev` was never started (Rule #10). The plan's verification list — mobile detail/cart/listing, the desktop 400px scroll-reveal where both controls appear together, footer bottom at both widths, `/admin` mobile showing exactly one tab bar — still needs a human.

**Noted, not touched**: `audit-sieve-field-schema-parity` fails with 61 violations, all in `products.repository.ts`, from a concurrent session's `ProductDocument` edits. Root Cause numbering: this session's entry is **#71**, because a concurrent session had already claimed #70 for public-projection work.


---
### S-public-projection — site-settings deny-list → allow-list, store-document leak in public HTML, settings wiring fixes (2026-08-24)

**Trigger**: "i see that site settings is a public exposed api, which can also have our secrets can we move our secrets such as integrations or other details to a secure or masked api ? or handle this properly", then "do migrations and proper wiring as well".

**The named worry was already safe** — `credentials.*` (26 API keys) *was* stripped, and `firestore.rules` denies all client access, so HTTP is the only surface. The mechanism was the problem: `GET /api/site-settings` (unauthenticated, `s-maxage=600`) spread the document and deleted three keys, so anything nobody deleted was public.

**What was actually exposed publicly**: `gst.gstin` + registered legal name/address · all 25 `commissions.*` (`gatewayFeePercent`, `payoutHoldDays`, `minPayoutAmount`, listing/featured/promoted fees) · `laborRate.hourlyRate` · `emi.surchargeSellerSharePercent` · `featureFlags.adminCheckoutBypass` · **`adSettings.providerCredentials` unmasked** (the *admin* ads endpoint masks it) and every draft/paused/scheduled ad · `integrations`, `platformLimits`, `tagline`, `favicon`. The last four groups were written by the admin form but **never declared on `SiteSettingsDocument`** — which is the whole argument for an allow-list: a deny-list keyed on a TS interface cannot see undeclared fields.

**More severe, found while tracing (not in the original report)**: `StoreRepository.mapDoc` decrypts `whatsappConfig.accessToken` on every read, and **four** Server Components passed the raw `StoreDocument` into a `"use client"` component — three behind an `as unknown as StoreDetail` cast with zero runtime effect. Next.js serialised it into the RSC flight payload in public HTML: plaintext Meta WhatsApp Business token, `wabaId`, `catalogId`, `adminNotes`, `suspensionReason`, `customCommissionRate`. Sites: `/stores/[slug]/about`, the legacy `[tab]/sort/...` route, `StoreDetailLayoutView`, and — found by sweeping rather than following the report — `MarketplaceHomepageView`, which fed raw docs as `initialItems` to the `"use client"` `FeaturedStoresSection`, i.e. **the same leak on the homepage**. Three hand-rolled store allow-lists collapsed into one shared adapter.

**Three live bugs surfaced by the "proper wiring" pass**:
1. The admin form wrote `credentials.razorpaySecret`; the server reads `razorpayKeySecret`. **The Razorpay secret was unsettable through the UI.** The seed had always written the correct name, so a seeded install worked and an admin-edited one didn't. Also surfaced `razorpayWebhookSecret`, `googleMapsApiKey`, `googlePlaceId` (all consumed, none previously editable) and dropped `smtpPassword` (zero consumers — email is Resend).
2. `AdminSiteSettingsView` saved the banner under `announcementBar.text`; `AnnouncementBar` and the homepage read `.message`. **Admin announcements saved successfully and never displayed.** `link`/`backgroundColor` were collected with no renderer at all — `link` is now wired and seeded.
3. `audit-schema-base-fields` advertised a `// audit-schema-base-ok:` marker in its own failure output that was **never implemented** *and* is banned by `audit-no-suppression-comments` — following its advice produced a different failing audit. Corrected its guidance to "use a `type` alias"; shipped zero markers.

**Also**: deleted 7 unauthenticated server actions returning raw documents (`listStoresAction` returned a whole page of decrypted stores; `getSiteSettingsAction` returned the singleton *including* the credentials blob — its own test file is titled "no auth guard"); deleted the duplicate `PATCH /api/site-settings` (no `permission`, unlike its `PUT /api/admin/site` twin) and folded its side effects into that PUT; stripped the raw `enc:v1:` blobs from the admin GET response; removed the dead `siteSettingsUpdateSchema`.

**Deliberately NOT adopted**: validating the admin PUT through `siteSettingsUpdateSchema`. It declares a stale subset of the document and uses wrong `legalPages` key names — Zod strips unknown keys and `updateSingleton` writes via Firestore `.update()`, which replaces nested maps wholesale, so the first admin save would have **wiped every `featureFlags.listingTypes` flag**. Used a top-level key allow-list instead.

**Files**: new `appkit/src/_internal/server/features/{site-settings,stores}/adapters.ts`, `_internal/shared/features/site-settings/{types,fees}.ts`; `computeBuyerEmiQuote` split out of `computeEmiSchedule` (the latter keeps the seller/platform split and stays server-only, with `surchargeSellerSharePercent` **required** so a caller can't silently record a zero seller share); `scripts/audit-public-projection-parity.mjs` (strict-zero, registered) — both its rules were verified by reintroducing the real bugs and confirming they fire; `appkit/scripts/migrate-settings-field-renames.mjs` (idempotent, `--dry-run`).

**Runtime-vs-type trap worth remembering**: the cart/checkout client props were *already* typed narrowly (`StoreAddonsRates`), but the SSR pages passed `settings.commissions` whole. Structural typing accepts a wider object, so a narrow prop type strips nothing at runtime — only `toBuyerFacingFees()` does.

**Concurrent-session note**: a parallel session was editing `appkit/src/features/admin/schemas/firestore.ts`, `AdminSiteSettingsView.tsx`, `scripts/run-audits.mjs`, `BottomActions.tsx` and `auctionSettlement.ts` throughout. Work was paused once to let their `legalPages` migration land. Both repos typechecked at 0 non-test errors after each of my stages; any residual error in `auctionSettlement.ts` is theirs.
